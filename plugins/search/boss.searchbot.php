...
/**
 * @BOSS - Плагин поиска в контенте JoiBoss CCK
 * @version 1.0.2 RE
...
$_PLUGINS->registerFunction('onSearch', 'botSearchBoss');
...
function botSearchBoss($text, $phrase = '', $ordering = ''){
	...
	if(!isset($_PLUGINS->_search_plugin_params['boss'])){
		// load plugin params info
		$plugin = null;
		$query = "SELECT `params` FROM #__plugins WHERE `element` = 'boss.searchbot' AND `folder` = 'search'";
		$database->setQuery($query);
		$database->loadObject($plugin);
		// save query to class variable
		$_PLUGINS->_search_plugin_params['boss'] = $plugin;
	}

	// pull query data from class variable
	$plugin = $_PLUGINS->_search_plugin_params['boss'];
