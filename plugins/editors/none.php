...
$_PLUGINS->registerFunction('onInitEditor', 'botNoEditorInit');
$_PLUGINS->registerFunction('onGetEditorContents', 'botNoEditorGetContents');
$_PLUGINS->registerFunction('onEditorArea', 'botNoEditorEditorArea');
...
function botNoEditorEditorArea($name, $content, $hiddenField, $width, $height, $col, $row){
	...
	foreach($results as $result){
		if($result[0]){
			$buttons[] = '<img src="' . _LPATH_SITE . '/plugins/editors-xtd/' . $result[0] . '" onclick="insertAtCursor( document.adminForm.' . $hiddenField . ', \'' . $result[1] . '\' )" alt="' . $result[1] . '"/>';
		}
	}
