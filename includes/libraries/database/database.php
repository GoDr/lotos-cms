...
class database{
	...
	public function __construct($host = 'localhost', $user = 'root', $pass = '', $db = '', $table_prefix = '', $debug = 0, $port = null, $socket = null){
		...
		if(!function_exists('mysqli_connect')){
			include _LPATH_ROOT . '/templates/system/offline.php';
			exit();
		}
	...
	public static function getInstance($ignoreMainframe = false){
		...
		if(!$ignoreMainframe && class_exists('MainFrame') && MainFrame::hasInstance()){
			self::$_instance = MainFrame::getInstance()->getDBO();
		}
		...
		if(self::$_instance === NULL){
			...
			if($instance->getErrorNum()){
				include_once(_LPATH_ROOT . DS . 'configuration.php');
				include(_LPATH_ROOT . DS . 'templates/system/offline.php');
				exit();
			}
...
class mosDBTable{
	...
	function hit($oid = null){
		...
		if(@LCore::getCfg('enable_log_items')){
			$now = date('Y-m-d');
