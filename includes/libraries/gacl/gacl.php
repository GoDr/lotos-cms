...
class gacl{
	...
	function gacl($db = null){
		...
		$this->_mos_add_acl('administration', 'install', 'users', 'administrator', 'plugins', 'all');
		$this->_mos_add_acl('administration', 'install', 'users', 'super administrator', 'plugins', 'all');
		$this->_mos_add_acl('administration', 'edit', 'users', 'super administrator', 'plugins', 'all');
		$this->_mos_add_acl('administration', 'edit', 'users', 'administrator', 'plugins', 'all');
...
//				$groups_map_table = $this->_db_table_prefix.'axo_groups_map';
	...
	function get_group_children_tree($root_id = null, $root_name = null, $inclusive = true){
		...
		for($i = $n - 1; $i >= 0; $i--){
			...
			$list[$i] = LHtml::makeOption($tree[$i]->group_id, $shim . $twist . $tree[$i]->name);
