...
class mosPageNav{
	...
	function getLimitBox($link = null){
		$limits = array();
		for($i = 5; $i <= 30; $i += 5){
			$limits[] = LHtml::makeOption($i);
		}
		$limits[] = LHtml::makeOption('50');
		$limits[] = LHtml::makeOption('100');
		$limits[] = LHtml::makeOption('150');
		$limits[] = LHtml::makeOption('5000', _PN_ALL);

		if($link){
			$link = $link . "&amp;limit=[selector]&amp;limitstart=" . $this->limitstart;
			$link = LSef::getUrlToSef($link);
			$link = str_replace('[selector]', "' + this.options[selectedIndex].value + '", $link);

			return LHtml::selectList($limits, 'limit', 'class="inputbox" size="1" onchange="document.location.href=\'' . $link . '\';"', 'value', 'text', $this->limit);
		} else{
			$html = LHtml::selectList($limits, 'limit', 'class="inputbox" size="1" onchange="document.adminForm.submit();"', 'value', 'text', $this->limit);
	...
	function writePagesLinks($link = null){
		if($link){
			...
			if($this_page > 1){
				...
				if(!$this->prev_exist){
					$mainframe->addCustomHeadTag('<link rel="prev" href="' . LSef::getUrlToSef($link . '&amp;limitstart=' . $page) . '" />');
					$this->prev_exist = 1;
				}

				$txt .= '<li class="first_page"><a href="' . LSef::getUrlToSef("$link&amp;limitstart=0") . '">' . _PN_START . '</a></li> ';
				$txt .= '<li class="back"><a href="' . LSef::getUrlToSef("$link&amp;limitstart=$page") . '">' . _PN_PREVIOUS . '</a></li> ';
			} else{
				$txt .= '<li class="first_page">' . _PN_START . '</li> ';
				$txt .= '<li class="back">' . _PN_PREVIOUS . '</li> ';
			}
			...
			for($i = $start_loop; $i <= $stop_loop; $i++){
				...
				if($i == $this_page){
					...
					$txt .= '<li><a href="' . LSef::getUrlToSef($link . '&amp;limitstart=' . $page) . '" class="pagenav"><strong>' . $i . '</strong></a></li>';
				}
			}
			...
			if($this_page < $total_pages){
				...
				if(!$this->next_exist){
					$mainframe->addCustomHeadTag("<link rel='next' href='" . LSef::getUrlToSef($link . '&amp;limitstart=' . $page) . "' />");
					$this->next_exist = 1;
				}

				$txt .= '<li class="next"><a href="' . LSef::getUrlToSef($link . '&amp;limitstart=' . $page) . '">' . _PN_NEXT . '</a> </li>';
				$txt .= '<li class="last_page"><a href="' . LSef::getUrlToSef($link . '&amp;limitstart=' . $end_page) . '">' . _PN_END . '</a></li>';
			} else{
				$txt .= '<li class="next">' . _PN_NEXT . '</li> ';
				$txt .= '<li class="last_page">' . _PN_END . '</li>';
			}
			return $txt . '</ul>';
		} else{
			$html = '';
