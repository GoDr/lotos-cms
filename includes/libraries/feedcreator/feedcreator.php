...
define("TIME_ZONE", LCore::getCfg('feed_timeoffset'));
...
class Yandex extends FeedCreator{
	function createFeed(){
		...
		for($i = 0; $i < count($this->items); $i++){
			...
			//if(!LCore::getCfg('sef')) $this->items[$i]->link = _LPATH_SITE.'/'.$this->items[$i]->link;
			$feed .= "<link>" . htmlspecialchars($this->items[$i]->link) . "</link>\n";
			$feed .= "<description>" . $this->items[$i]->getDescription() . "</description>\n";
			if($this->items[$i]->author != ""){
				$feed .= "<author>" . htmlspecialchars($this->items[$i]->author) . "</author>\n";
			...
			if(is_array(@$this->items[$i]->images)){
				foreach($this->items[$i]->images as $image){
					if(function_exists('getimagesize')){
						$type = @getimagesize(_LPATH_ROOT . $image);
					} else{
						$type = array();
					}
					$feed .= "<enclosure url=\"" . _LPATH_SITE . $image . "\" type=\"" . $type['mime'] . "\"/>\n";
				}
			}
			if($this->items[$i]->date != ""){
				$itemDate = new FeedDate($this->items[$i]->date);
