...
function mosLoadComponent($name){
	...
	include (_LPATH_ROOT . DS . "components/com_$name/$name.php");
}
...
class PageModel{
	...
	function MainBody(){
		...
		// Browser Check
		$browserCheck = 0;
		if(isset($_SERVER['HTTP_USER_AGENT']) && isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], _LPATH_SITE) !== false){
			$browserCheck = 1;
		}
        ...
        if ((LSef::getOption() != '' and LSef::getOption() != 'com_frontpage')){
            ...
            if($mainframe->getCfg('mmb_mainbody_off') == 0){
                ...
                $_PLUGINS->trigger('onMainbody', array(&$_body));
            }
        }elseif(LCore::getCfg('mainbody')){
            $_body = $GLOBALS['_MOS_OPTION']['jqueryplugins'] . $GLOBALS['_MOS_OPTION']['buffer'];
	...
	function ShowHead($params = array('js'=> 1, 'css'=> 1)){
		...
		if($mainframe->getCfg('generator_off') == 0){
			$mainframe->addMetaTag('Generator', LVersion::get('CMS') . ' - ' . LVersion::get('COPYRIGHT'));
		}
		...
		if($mainframe->getCfg('mtage_base') == 1){
			...
			$theURI = str_replace(_LPATH_SITE . '/', '', $theURI);
			echo '<base href="' . LSef::getUrlToSef(ampReplace($theURI)) . '" />' . "\r\n";
			//echo '<base href="'._LPATH_SITE.'" />'."\r\n";
		}

		if($my->id || $mainframe->get('joomlaJavascript')){
			?>
		<script src="<?php echo _LPATH_SITE; ?>/includes/js/joomla.javascript.js"></script>
		<?php
		}
		...
		if(!$mainframe->getCfg('disable_favicon')){
			...
			$icon = _LPATH_ROOT . '/images/' . $favicon;
			if(!file_exists($icon)){
				$icon = _LPATH_SITE . '/images/favicon.ico';
			} else{
				$icon = _LPATH_SITE . '/images/' . $favicon;
			}
	...
	function syndicate_header(){
		...
		database::getInstance()->setQuery($query)->loadObject($row);
		...
		if($live_bookmark){
			...
			$link_file = _LPATH_SITE . '/index2.php?option=com_rss&feed=' . $live_bookmark . '&no_html=1';
			...
			if($check){
				...
				$check = database::getInstance()->loadResult();
				if($check > 0){
					$show = 0;
				}
			}
