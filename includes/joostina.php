...
if (LCore::getCfg('error_reporting') == 0) {
    error_reporting(0);
} elseif (LCore::getCfg('error_reporting') != 0) {
    error_reporting(LCore::getCfg('error_reporting'));
}
/* ядро отладчика */
MainFrame::addLib('debug');
/* ядро для работы с юникодом */
MainFrame::addLib('utf8');

/* файл данных версии */
require_once(_LPATH_ROOT . '/includes/version.php');
/* ядро работы с XML */
require_once(_LPATH_ROOT . '/includes/parameters.xml.php');
/* класс фильтрации данных */
MainFrame::addLib('inputfilter');
/* класс работы с базой данных */
MainFrame::addLib('database');

$database = database::getInstance();

/* класс работы с правами пользователей */
MainFrame::addLib('gacl');
...
class MainFrame
{
     ...
     */
    function MainFrame($db, $option, $basePath = null, $isAdmin = false)
    {
        ...
        $this->_setAdminPaths($option, _LPATH_ROOT);
    ...
    {
        //jd_inc('MainFrame::getInstance()');

        if (!is_object(self::$_instance)) {
            self::$_instance = new MainFrame(null, null, null, $isAdmin);
        }
    ...
    /**
     * Check if there are any instance of MainFrame initialized
     *
     * @return bool
     */
    ...
    function adminView($target)
    {
        ...
        $from_template = _LPATH_TPL_ADMI . '/' . TEMPLATE . '/html/' . $option . '/' . $target . '.php';

        if (is_file($return = _LPATH_ROOT . DS . $from_template)) {
            return $return;
        } elseif (is_file($return = _LPATH_ROOT . DS . $default)) {
            return $return;
        } else {
            return false;
        }
    ...
    function getBasePath($client = 0, $addTrailingSlash = true)
    {
        ...
        switch ($client) {
            ...
            case 'front':
            default:
                return mosPathName(_LPATH_ROOT, $addTrailingSlash);
                break;

            case '2':
            case 'installation':
                return mosPathName(_LPATH_ROOT . DS . 'installation', $addTrailingSlash);
                break;

            case '1':
            case 'admin':
            case 'administrator':
                return mosPathName(_LPATH_ROOT . DS . JADMIN_BASE, $addTrailingSlash);
                break;
        }
    }
    ...
    public static function addLib($lib, $dir = '')
    {
        ...
        $file_lib = _LPATH_ROOT . DS . $dir . DS . $lib . DS . $lib . '.php';
        if (is_file($file_lib)) {
            require_once($file_lib);
        }
    }

    public static function getLangFile($name = '', $lang = null)
    {
        $mainframe = MainFrame::getInstance();
        $lang = (is_null($lang)) ? LCore::getCfg('lang') : $lang;

        if (!$name) {
            return _LPATH_ROOT . DS . 'language' . DS . $lang . DS . 'system.php';
        } else {
            $file = $name;
        }
        if ($mainframe->isAdmin() == true) {
            if (is_file(_LPATH_ROOT . DS . 'language' . DS . $lang . DS . 'administrator' . DS . $file . '.php')) {
                return _LPATH_ROOT . DS . 'language' . DS . $lang . DS . 'administrator' . DS . $file . '.php';
            } else {
                if (is_file(_LPATH_ROOT . DS . 'language' . DS . $lang . DS . 'frontend' . DS . $file . '.php')) {
                    return _LPATH_ROOT . DS . 'language' . DS . $lang . DS . 'frontend' . DS . $file . '.php';
                }
            }
        } else {
            if (is_file(_LPATH_ROOT . DS . 'language' . DS . $lang . DS . 'frontend' . DS . $file . '.php')) {
    ...
    function setPageTitle($title = null, $pageparams = null)
    {
        ...
        if ($this->getCfg('pagetitles')) {
            ...
            if ($pageparams != null) {
                ...
                $page_title = $pageparams->get('no_site_name') ? ($pageownname ? $pageownname : ($title ? $title : $sitename))
                    : ($this->getCfg('pagetitles_first') ? (($pageownname ? $pageownname : $title) . $tseparator . $sitename) : ($this->getCfg('sitename') . $tseparator . ($pageownname ? $pageownname : $title)));
    ...
    function getHead($params = array('js' => 1, 'css' => 1, 'jquery' => 0))
    {
        ...
        if (isset($params['jquery']) && $params['jquery'] == 1) {
            $head[] = LHtml::loadJquery(true, true);
        }
    ...
    function getFooter($params = array('fromheader' => 1, 'custom' => 0, 'js' => 1, 'css' => 1))
    {
        ...
        if (isset($params['jquery']) && $params['jquery'] == 1) {
            $footer[] = LHtml::loadJquery(true, true);
        }
     ...
     *
     * @modofication 05.11.2013 Gold Dragon
     */
    public function addJS($path, $footer = '', $ret = false)
    {
        if (LHtml::checkReplayJs($path)) {
            if ($ret) {
                echo '<script src="' . $path . '"></script>';
            } else {
                if ($footer) {
                    $this->_footer[$footer][] = '<script src="' . $path . '"></script>';
                } else {
                    $this->_head['js'][] = '<script src="' . $path . '"></script>';
                }
            }
        }
    }
     ...
     */
    function addCSS($path, $ret = false)
    {
        if (LHtml::checkReplayCss($path)) {
            if ($ret) {
    ...
    function initSessionAdmin($option, $task)
    {
        ...
        // logout check
        if ($option == 'logout') {
            require _LPATH_ADMINISTRATOR . DS . 'logout.php';
            exit();
        }

        // check if session name corresponds to correct format
        if (session_name() != md5(_LPATH_SITE)) {
        ...
        if ($session_id != session_id()) {
            ...
            mosRedirect(_LPATH_SITE . '/' . JADMIN_BASE . '/', _YOU_NEED_TO_AUTH);
            exit();
        }
        ...
        if ($session_id == md5($my->id . $my->username . $my->usertype . $logintime)) {
            ...
            if ($task != 'save' && $task != 'apply') {
                ...
                if ($count == 0) {
                    ...
                    if ($link && strpos($link, 'index2.php?option=com_') === 0) {
                        $now = time();
                        ...
                        $query = "UPDATE #__users SET params = " . $this->_db->Quote($saveparams) . " WHERE id = " . (int)$my->id . " AND username = " . $this->_db->Quote($my->username) . " AND usertype = "
                            . $this->_db->Quote($my->usertype);
                        $this->_db->setQuery($query)->query();
                    }

                    mosRedirect(_LPATH_SITE . '/' . JADMIN_BASE . '/', _ADMIN_SESSION_ENDED);
                } else {
    ...
    function login($username = null, $passwd = null, $remember = 0, $userid = null)
    {
        ...
        if (!$username || !$passwd) {
            ...
            if (is_object($row)) {
                ...
                $query
                    = "DELETE FROM #__session WHERE session_id != " . $this->_db->Quote($session->session_id) . " AND username = " . $this->_db->Quote($row->username) . " AND userid = " . (int)$row->id . " AND gid = "
                    . (int)$row->gid . " AND guest = 0";
                $this->_db->setQuery($query)->query();
                ...
                // а зачем чистить кеш после каждой авторизации?
                //mosCache::cleanCache();
            } else {
                if ($bypost) {
                    mosErrorAlert(_LOGIN_INCORRECT);
    ...
    function getUser_from_sess($sess_id)
    {
        ...
        if ($row && $row->userid) {
            ...
            $query
                = "SELECT id, name, username, usertype, email, avatar, block, sendEmail, registerDate, lastvisitDate, activation, params
    ...
    function _setTemplate($isAdmin = false)
    {
        ...
        if ($isAdmin) {
            if ($this->getCfg('admin_template') == '...') {
                ...
                $path = _LPATH_TPL_ADMI . '/' . $cur_template . '/index.php';
                if (!is_file($path)) {
            ...
            if ($jos_change_template) {
                ...
                if (file_exists(_LPATH_ROOT . DS . 'templates' . DS . $jos_change_template . DS . 'index.php')) {
                    $lifetime = 60 * 10;
    ...
    public function getPath($varname, $option = '')
    {
        ...
        if (isset($this->_path->$varname)) {
            ...
            switch ($varname) {
                case 'com_xml':
                    $name = substr($option, 4);
                    $path = _LPATH_ROOT . DS . JADMIN_BASE . "/components/$option/$name.xml";
                    if (file_exists($path)) {
                        $result = $path;
                    } else {
                        $path = _LPATH_ROOT . "/components/$option/$name.xml";
                        if (file_exists($path)) {
                            $result = $path;
                        }
                    }
                ...
                case 'mod0_xml':
                    // Site modules
                    if ($option == '') {
                        $path = _LPATH_ROOT . '/modules/custom.xml';
                    } else {
                        $path = _LPATH_ROOT . "/modules/$option.xml";
                    }
                    if (file_exists($path)) {
                        $result = $path;
                    }
                ...
                case 'mod1_xml':
                    // admin modules
                    if ($option == '') {
                        $path = _LPATH_ROOT . DS . JADMIN_BASE . '/modules/custom.xml';
                    } else {
                        $path = _LPATH_ROOT . DS . JADMIN_BASE . "/modules/$option.xml";
                    }
                    if (file_exists($path)) {
                        $result = $path;
                    }
                ...
                case 'bot_xml':
                    // Site plugins
                    $path = _LPATH_ROOT . DS . 'plugins' . DS . $option . '.xml';
                    if (file_exists($path)) {
                        $result = $path;
                    }
                    break;
                ...
                case 'menu_xml':
                    $path = _LPATH_ROOT . DS . JADMIN_BASE . "/components/com_menus/$option/$option.xml";
                    if (file_exists($path)) {
                        $result = $path;
                    }
                    break;
                ...
                case 'installer_html':
                    $path = _LPATH_ROOT . DS . JADMIN_BASE . "/components/com_installer/$option/$option.html.php";
                    if (file_exists($path)) {
                        $result = $path;
                    }
                    break;
                ...
                case 'installer_class':
                    $path = _LPATH_ROOT . DS . JADMIN_BASE . "/components/com_installer/$option/$option.class.php";
                    if (file_exists($path)) {
                        $result = $path;
                    }
                    break;
    ...
    function set_mosmsg($msg = '')
    {
        ...
        if ($msg != '') {
            if ($this->_isAdmin) {
                ...
                session_name(MainFrame::sessionCookieName());
                session_start();
            }
    ...
    function check_option($option)
    {
        ...
        ($this->_db->setQuery($sql)->loadResult() == 0) ? null : mosRedirect(_LPATH_SITE);
        return true;
    }
    ...
    function get_option()
    {
        ...
        $option = strtolower(LCore::getParam($_REQUEST, 'option', 'com_frontpage', 'sn'));
    ...
    {
        $ip = ip2long(MainFrame::getIP());
        if ($ip and extension_loaded('curl')) {
            $err = false;
            if (is_file(_LPATH_ROOT . DS . 'jserver.php')) {
                require_once(_LPATH_ROOT . DS . 'jserver.php');
                if (isset($date_send_server)) {
                    if ($date_send_server < time()) {
                        $mainframe = MainFrame::getInstance();

                        $url = LVersion::get('SUPPORT_CENTER') . '/redirection/index.php';
                        $post_data = array(
                            "site" => $mainframe->getCfg('live_site'),
                            "desc" => $mainframe->getCfg('MetaDesc'),
                            "cms_ver" => LVersion::get('CMS_VER') . '.' . LVersion::get('DEV_LEVEL'),
                            "build" => LVersion::get('DEV_LEVEL') . LVersion::get('BUILD'),
                            "ip" => $ip = ip2long(MainFrame::getIP()),
                            "page" => 'static'
                        );
            ...
            if ($err) {
                ...
                if ($fp = fopen(_LPATH_ROOT . DS . 'jserver.php', "w")) {
     ...
     * @return string
     */
    public static function getUserBrowser($param = 'both', $separator = ' ')
    {
        $agent = $_SERVER['HTTP_USER_AGENT'];
...
// TODO необходимо выделить в отдельный файл
class JConfig
{
    ...
    /** @var string */
    var $config_helpurl = 'http://wiki.lotos-cms.ru';
    /** @var string */
    ...
    /** @var int отключение плагинов группы system */
    ...
    /** @var int отключение плагинов группы content */
    var $config_mmb_content_off = 0;
    /** @var int кеширование меню панели управления */
    ...
    /** @var int отключение плагинов группы mainbody */
    ...
    /** @var int включение оптимизации функции кеширования */
    ...
    /** @var int обработчик кеширования запросов базы данных */
    var $config_db_cache_handler = 'none';
    /** @var int время жизни кеша запросов базы данных */
    ...
    /** @var int выводить ли mosMainbody на Главной странице */
    var $config_mainbody = 0;
    ...
    /** @var int обработчик кеширования */
    var $config_cache_handler = 'none';
    /** @var int ключ для кеш файлов */
    ...
    /** @var int использование неопубликованных плагинов */
    var $config_use_unpublished_plugins = 1;
    /** @var int использование плагинов удаления содержимого */
    var $config_use_content_delete_plugins = 0;
    ...
    /** @var int использование плагинов редактирования содержимого */
    var $config_use_content_edit_plugins = 0;
    /** @var int использование плагинов сохранения содержимого */
    var $config_use_content_save_plugins = 0;
    ...
    var $config_tags = 0;
    /** @var int включение/выключение плагинов группы onAjaxStart */
    ...
    function bindGlobals()
    {
		 ...
		 */
        require(_LPATH_ROOT . DS . 'configuration.php');
        if (_LPATH_SITE != $this->config_live_site) {
            $this->config_live_site = _LPATH_SITE;
        }
    }
 ...
 * @package Joostina
 */
class mosPlugin extends mosDBTable
{
    ...
    function mosPlugin(&$db)
    {
        $this->mosDBTable('#__plugins', 'id', $db);
    }
...
class mosModule extends mosDBTable
{
    ...
    function set_template($params)
    {
        ...
        if ($params->get('template_dir', 0) == 0) {
            ...
            $template_dir = _LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/modules/' . $this->module;
        }

        if ($params->get('template')) {
            $file = _LPATH_ROOT . DS . $template_dir . DS . $params->get('template');
            if (is_file($file)) {
                $this->template = $file;
                return true;
            } elseif (is_file(_LPATH_ROOT . DS . $default_template)) {
                $this->template = _LPATH_ROOT . DS . $default_template;
                return true;
            }
        }
    ...
    function set_template_custom($template)
    {
        ...
        $template_file = _LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/user_modules/' . $template;
    ...
    function get_helper($mainframe)
    {
        ...
        $file = _LPATH_ROOT . DS . 'modules' . DS . $this->module . DS . 'helper.php';
    ...
    function initModules()
    {
        ...
        require_once(_LPATH_ROOT . '/includes/frontend.html.php');
    ...
    public static function _initModules($my_gid)
    {
        ...
        $sql
            = "SELECT m.id, m.title, m.module, m.position, m.content, m.showtitle, m.params, m.access
				FROM #__modules AS m
...
class mosCache
{
    ...
    public static function getCache($group = 'default', $handler = 'callback', $storage = null, $cachetime = null, $object = null)
    {
        ...
        $cache = LibCache::getInstance($handler, $options, $object);
...
require_once(_LPATH_ROOT . '/components/com_users/users.class.php');
...
function mosRedirect($url, $msg = '')
{
    ...
    // TODO: сейчас удаляет двойной _LPATH_SITE при включенном SEF. При отключенном адрес начинается с index.php
    if (substr_count($url, _LPATH_SITE) > 1) {
        $url = substr($url, 0, strlen(_LPATH_SITE));
    }
...
/**
 * TODO Новая замена функции mosTreeRecurse: вместо объектов массив
 *
 * @param     $id
 * @param     $indent
 * @param     $list
...
function mosGetOrderingList($sql, $chop = '30')
{
    ...
    if (!($orders = $database->loadObjectList())) {
        if ($database->getErrorNum()) {
            ...
            $order[] = LHtml::makeOption(1, _FIRST);
            return $order;
        }
    }
    $order[] = LHtml::makeOption(0, '0 ' . _FIRST);
    for ($i = 0, $n = count($orders); $i < $n; $i++) {
        ...
        $order[] = LHtml::makeOption($orders[$i]->value, $orders[$i]->value . ' (' . $text . ')');
    }
    $order[] = LHtml::makeOption($orders[$i - 1]->value + 1, ($orders[$i - 1]->value + 1) . ' ' . _LAST);
    return $order;
}
...
{
    /*
	 TODO: временная заглушка.. нужно искать в базе совпадение ссылки и смотреть права
	Если ссылки нет то гостевые права.
	Необходимо подключить новый класс БД и использовать подготовленные выражения
	При влюченном SEF необходимо пересобирать строку
*/
//    $access = 0;
//
//    $database = database::getInstance();
...
function mosFormatDate($date, $format = '', $offset = null)
{
    ...
    if ($date == '0000-00-00 00:00:00') {
        return $date;
    } //database::$_nullDate - при ошибках парсера
...
function mosWarning($warning, $title = _MOS_WARNING)
{
    ...
    $tip .= '<img src="' . _LPATH_SITE . '/includes/js/ThemeOffice/warning.png" border="0" alt="' . _WARNING . '"/></a>';
    return $tip;
}
...
function mosCreateMail($from = '', $fromname = '', $subject = '', $body = '')
{
    ...
    MainFrame::addLib('phpmailer');
    $mail = new mosPHPMailer();

    $config = Jconfig::getInstance();

    $mail->PluginDir = _LPATH_ROOT . DS . 'includes/libraries/phpmailer/';
    $mail->SetLanguage(_LANGUAGE, _LPATH_ROOT . DS . 'includes/libraries/phpmailer/language/');
    ...
    if ($config->config_mailer == 'smtp') {
        ...
        $mail->Port = $config->config_smtpport;
...
class mosPluginHandler
{
    ...
    var $_loading = null;
    /**
     * @var array An array of the content plugins in the system
     */
    var $_content_plugins = null;
    /**
     * @var array An array of the content plugin params
     */
    var $_content_plugin_params = array();
    /**
     * @var array An array of the search plugin params
     */
    var $_search_plugin_params = array();
    /**
     * @var array An array of the  plugin params
     */
    var $_plugin_params = array();
    var $_config = null;
    var $_db = null;
     ...
     * Constructor
     */
    function mosPluginHandler()
    {
        $this->_db = database::getInstance();
        $config = Jconfig::getInstance();
        $this->_config = array('config_disable_access_control' => $config->config_disable_access_control, 'config_use_unpublished_plugins' => $config->config_use_unpublished_plugins);
        $this->_events = array();
        unset($config);
    }
    ...
    public static function &getInstance()
    {
        ...
        if (!is_object(self::$_instance)) {
            self::$_instance = new mosPluginHandler();
        }
    ...
    function loadBotGroup($group, $load = 0)
    {
        ...
        switch ($group) {
            case 'content':
                if (!defined('_JOS_CONTENT_PLUGINS')) {
                    /** ensure that query is only called once */
                    define('_JOS_CONTENT_PLUGINS', 1);
                    $where_ac_2 = $where_ac . ($config['config_use_unpublished_plugins'] == 1) ? ' published=1 AND' : '';
                    $query = 'SELECT folder, element, published, params FROM #__plugins WHERE ' . $where_ac_2 . ' folder = \'content\' AND client_id=0 ORDER BY ordering ASC';
                    $database->setQuery($query);
                    // load query into class variable _content_plugins
                    if (!($this->_content_plugins = $database->loadObjectList())) {
                        return false;
                    }
                    foreach ($this->_content_plugins as $bot) {
                        $this->_content_plugin_params[$bot->element] = new stdClass();
                        $this->_content_plugin_params[$bot->element]->params = $bot->params;
                    }
                }
                // pull bots to be processed from class variable
                $bots = $this->_content_plugins;
                break;

            case 'search':
                if (!defined('_JOS_SEARCH_PLUGINS')) {
                    define('_JOS_SEARCH_PLUGINS', 1);

                    $query = 'SELECT folder, element, published, params FROM #__plugins WHERE published = 1' . $where_ac . ' AND folder = \'search\' ORDER BY ordering ASC';
                    $database->setQuery($query);
                    if (!($this->_search_plugin = $database->loadObjectList())) {
                        return false;
                    }

                    foreach ($this->_search_plugin as $bot) {
                        if (!isset($this->_search_plugin_params[str_replace('.searchbot', '', $bot->element)]->params)) {
                            $this->_search_plugin_params[str_replace('.searchbot', '', $bot->element)] = new stdClass;
                        }
                        $this->_search_plugin_params[str_replace('.searchbot', '', $bot->element)]->params = $bot->params;
                    }
                }

                // pull bots to be processed from class variable
                $bots = $this->_search_plugin;
                break;

            default:
                $query = 'SELECT folder, element, published, params FROM #__plugins WHERE published = 1' . $where_ac . ' AND folder = ' . $database->Quote($group) . ' AND client_id=0 ORDER BY ordering ASC';
                $database->setQuery($query);
    ...
    function loadBot($folder, $element, $published, $params = '')
    {
        ...
        $path_bot = _LPATH_ROOT . DS . 'plugins';
        ...
        if (file_exists($path)) {
            ...
            $bot->params = $params;
            $this->_bots[] = $bot;
            $this->_plugin_params[$element] = $params;
    ...
    {
        $this->_lists[$group][$listName][] = LHtml::makeOption($value, $text);
    }
    ...
    //Адресный вызов плагина
    function call_plugin($event, $element, $args)
    {

        if (isset($this->_events[$event])) {
            foreach ($this->_events[$event] as $func) {
                if ($this->_bots[$func[1]]->element == $element && function_exists($func[0])) {
                    $this->_plugin_params[$element] = $this->_bots[$func[1]]->params;
                    if ($this->_bots[$func[1]]->published) {
                        return call_user_func_array($func[0], array(&$args));
                    }
                }
 ...
 * Added 1.0.11
 */
// TODO Gold Dragon : перевести в библиотеку
...
function mosBackTrace()
{
    if (function_exists('debug_backtrace')) {
        ...
        foreach (debug_backtrace() as $back) {
            if (isset($back['file'])) {
                echo '<br />' . str_replace(_LPATH_ROOT, '', $back['file']) . ':' . $back['line'];
            }
        }
        echo '</div>';
    }
...
class joostina_api
{
     ...
     * Основано на плагине OptimizeTables - smart (C) 2006, Joomlaportal.ru. All rights reserved
     */
...
/**
 * @global mosPlugin $_PLUGINS
 */
$_PLUGINS = new mosPluginHandler();