...
class modules_html{
	...
	function module($module, $params){
		...
		if($module->content != '' && $firebots){
			...
			$results = $_PLUGINS->trigger('onPrepareContent', array(&$row, &$params, 0), true);
	...
	function module2($module, $params){
		...
		$path = _LPATH_ROOT . DS . 'language' . DS . $config->config_lang . DS . 'frontend' . DS . $module->module . '.php';
		$path_def = _LPATH_ROOT . DS . 'language/russian/frontend' . DS . $module->module . '.php';
	...
	function modoutput_feed(&$module, &$params, $moduleclass_sfx){
		...
		require_once (_LPATH_ROOT . '/includes/domit/xml_domit_rss.php');
	...
	function modoutput_xhtml($module, $params, $moduleclass_sfx, $type = 0){
		...
		echo '<aside class="moduletable' . $moduleclass_sfx . '" id="module_' . $module->id . '">';
		...
		if($type){
			...
			include (_LPATH_ROOT . DS . 'modules' . DS . $module->module . '.php');
			if(isset($content)){
				echo $content;
			}
		}
	...
	function CustomContent(&$module, $params){
		...
		if($firebots){
			$row = $module;
			$row->text = $module->content;
			$results = $_PLUGINS->trigger('onBeforeDisplayContent', array(&$row, &$params, 0));
			echo trim(implode("\n", $results));
			$module->content = $row->text;
		}
		...
		if($firebots){
			$results = $_PLUGINS->trigger('onAfterDisplayContent', array(&$row, &$params, 0));
			echo trim(implode("\n", $results));
