...
<div class="jstProfile">
	...
	<div class="jstProfile_menu">
		<ul class="menu_userInfo">
			<?php
			$tabs = $_PLUGINS->trigger('userProfileTab', array($user));
			foreach($tabs as $tab){
				$id = isset($tab['id']) ? ' id="' . $tab['id'] . '"' : '';
				$class = isset($tab['class']) ? ' class="' . $tab['class'] . '"' : '';
				$title = isset($tab['title']) ? $tab['title'] : $tab['name'];
				?>
				<li><a title="<?php echo $title?>" href="<?php echo LSef::getUrlToSef($tab['href']) ?>" <?php echo $id . $class?>><?php echo $tab['name'] ?></a></li><?php
			}
			?>
		</ul>
	</div>
	<div class="plugins_area">
		<?php
		//Вывод плагинов
		$_PLUGINS->call_plugin('userProfile', $plugin_page, $user);
		?>
	</div>
</div>