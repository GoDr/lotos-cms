...
class mosUser extends mosDBTable{
	...
	public static function get_avatar($user){
		...
		$avatar_file = _LPATH_ROOT . '/images/avatars/' . $user->avatar;
	...
	function update_avatar($id = null, $img, $del = null){
		...
		if($del){
			if(file_exists($file_name = _LPATH_ROOT . DS . 'images' . DS . 'avatars' . DS . $img)){
				unlink($file_name);
			}
		}
	}
...
class userHelper{
	...
	public static function _load_core_js(){
		$mainframe = MainFrame::getInstance();
		$mainframe->addJS(_LPATH_SITE . '/components/com_users/js/com_users.js', 'custom');
	}

	public static function _load_jquery_form(){
		LHtml::loadJqueryPlugins('jquery.form', false, false, 'js');
	}
				...
				$.post(//---post:begin
					...
					function onAjaxSuccess(data) {
						...
						$('#current_<?php echo $form_params->img_field;?>_img').fadeOut(1000, function () {
							$('#current_<?php echo $form_params->img_field;?>_img').html('<img class="avatar" src="<?php echo _LPATH_SITE;?>/<?php echo $form_params->img_path;?>/' + data + '" />');
							//Скрываем индикатор
	...
	<?php if($state != 'upload'){ ?>
		<div id="current_<?php echo $form_params->img_field;?>">
			<div class="current_img" id="current_<?php echo $form_params->img_field;?>_img">
				<img class="avatar" src="<?php echo _LPATH_SITE;?>/<?php echo $form_params->img_path;?>/<?php echo $obj->$field;?>"/>
		...
		<div id="current_<?php echo $form_params->img_field;?>">
			<div class="current_img" id="current_<?php echo $form_params->img_field;?>_img">
				<img class="avatar" src="<?php echo _LPATH_SITE;?>/<?php echo $form_params->default_img;?>"/>
			</div>
			<div class="indicator" id="indicate_<?php echo $form_params->img_field;?>">&nbsp;</div>
	...
	public static function _build_img_upload_form($obj, $form_params){
		...
		if(!$mainframe->isAdmin()){
			$action = LSef::getUrlToSef($action);
		}
				...
				$('#<?php echo $form_params->img_field;?>_uploadForm').ajaxSubmit({
					...
					success:function (data) {
						...
						if (data) {
							...
							//$('#current_<?php echo $form_params->img_field;?>_img').fadeOut(1000);
							$('#current_<?php echo $form_params->img_field;?>_img').fadeOut(1000, function () {
								$('#current_<?php echo $form_params->img_field;?>_img').html('<img class="avatar" src="<?php echo _LPATH_SITE;?>/<?php echo $form_params->img_path;?>/' + data + '?r=' + Math.random() + '" />');
							});
							$('#current_<?php echo $form_params->img_field;?>_img').fadeIn(1000, function () {
...
class mosUserParameters extends mosParameters{
	...
	function _form_editor_list($name, $value, $node, $control_name){
		...
		array_unshift($editors, LHtml::makeOption('', _SELECT_EDITOR));

		return LHtml::selectList($editors, '' . $control_name . '[' . $name . ']', 'class="inputbox"', 'value', 'text', $value);
	}
}
...
class userPlugins{
	...
	function get_plugins($group = 'profile'){
		...
		$plugins = $_PLUGINS->loadBotGroup($group, 1);
		$this->_group = $group;
