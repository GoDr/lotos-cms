...
?>
<script>
	var _comuser_url = '<?php echo _LPATH_SITE;?>/components/com_users';
...
function userList($gid, $limit, $limitstart = 0){
	...
	if($params->get('template_dir')){
		$template_dir = _LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/com_users/userlist';
	}
	if(is_file($template_file = _LPATH_ROOT . DS . $template_dir . DS . $template)){
		include_once($template_file);
	} else{
		include_once(_LPATH_ROOT . DS . $template_dir . DS . 'default.php');
	}
...
function lostPassForm($option){
	...
	if($user_config->get('template_dir')){
		$template_dir = _LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/com_users/lostpass';
	}
...
function registerForm($option, $useractivation){
	...
	if(!$params->get('template')){
		if($type){
			if(is_file(_LPATH_ROOT . DS . 'components' . DS . 'com_users' . DS . 'view' . DS . 'registration' . DS . $type . '.php')){
				$template = $type . '.php';
			}
	...
	include (_LPATH_ROOT . DS . 'components' . DS . 'com_users' . DS . 'view' . DS . 'registration' . DS . $template);
...
function saveRegistration(){
	...
	if($mainframe->getCfg('useractivation') == 1){
		$email_info['message'] = sprintf(_USEND_MSG_ACTIVATE, $email_info['name'],
			...
			_LPATH_SITE . "/index.php?option=com_users&task=activate&activation=" . $row->activation,
			_LPATH_SITE, $email_info['username'], $pwd);
	} else{
		$email_info['message'] = sprintf(_USEND_MSG, $email_info['name'], $mainframe->getCfg('sitename'), _LPATH_SITE);
	}
	...
	if($mainframe->getCfg('useractivation') == 1){
		...
		if(!$params->get('template')){
			...
			if($group_name){
				if(!is_file(_LPATH_ROOT . DS . 'components' . DS . 'com_users' . DS . 'view' . DS . 'after_registration' . DS . $group_name . '.php')){
					$template = $group_name . '.php';
				}
			}
		}

		include (_LPATH_ROOT . DS . 'components' . DS . 'com_users' . DS . 'view' . DS . 'after_registration' . DS . $template);
		return;
