...
class search_html{
	...
	<form action="<?php echo LSef::getUrlToSef('index.php?option=com_search')?>" method="get" name="searchForm" id="searchForm">
	...
	<?php
		...
		$link = _LPATH_SITE . "/index.php?option=$option&amp;searchword=$cleanWord&amp;searchphrase=$searchphrase&amp;ordering=$ordering";
		//if($total>0){
					...
					<fieldset>
						<div>
							...
							if($row->href){
								...
								if($row->browsernav == 1){
									?>
										<a href="<?php echo LSef::getUrlToSef($row->href); ?>" target="_blank">
											<?php
								} else{
									?>
											<a href="<?php echo LSef::getUrlToSef($row->href); ?>">
												<?php
								}
							}
							echo $row->title;
						...
						<?php
						if(LCore::getCfg('showCreateDate')){
							?>
	...
	public static function conclusion($searchword, $pageNav){
		...
		$link = _LPATH_SITE . "/index.php?option=$option&amp;searchword=$searchword&amp;searchphrase=$searchphrase&amp;ordering=$ordering";
		echo '<div class="navigation">' . $pageNav->writePagesLinks($link) . '</div>';
	}
		...
		if(count($items->items['com_boss']) > 0){
			foreach($groups as $key => $group){
				foreach($items->items[$key] as $item){
					...
					$item->text = Text::word_limiter(LHtml::cleanText($item->text), 25);
					?>
