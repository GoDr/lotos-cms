...
class poll_html{
	...
	public static function showResults($poll, $votes, $first_vote, $last_vote, $pollist, $params){
		MainFrame::getInstance()->addCSS(_LPATH_SITE . '/components/com_poll/css/poll.css');
		?>
	...
	<table class="com_poll">
		...
		for($i = 0, $n = $_n; $i < $n; $i++){
			...
			<tr class="<?php echo $tabclass_arr[$tabcnt]; ?>">
				...
				?>
				<td class="poll_graph">&nbsp;
					<img src="<?php echo _LPATH_SITE; ?>/components/com_poll/images/blank.png" class="<?php echo $tdclass; ?>" height="<?php echo $polls_barheight; ?>" width="<?php echo $width; ?>" alt=""/>
				</td>
			</tr>
			<?php
			$tabcnt = 1 - $tabcnt;
