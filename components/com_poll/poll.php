...
function pollAddVote($uid){
	...
	if($redirect){
		mosRedirect(LSef::getUrlToSef('index.php?option=com_poll&task=results&id=' . $uid), _THANKS);
	} else{
		echo '<h3>' . _THANKS . '</h3>';
		echo '<form action="" method="GET">';
		echo '<span class="button"><input class="button" type="button" value="' . _BUTTON_RESULTS . '" onClick="window.location=\'' . LSef::getUrlToSef('index.php?option=com_poll&task=results&id=' . $uid ) . '\'"></span>';
		echo '</form>';
	}
}
...
function pollresult($uid){
	...
	*/
	...
	// dropdown output
	$link = LSef::getUrlToSef('index.php?option=com_poll&amp;task=results&amp;id=\' + this.options[selectedIndex].value + \'');
