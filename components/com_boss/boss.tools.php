...
function createImageAndThumb(
    ...
    $font = _LPATH_ROOT . "/components/com_boss/font/verdana.ttf";
    ...
    /* МИНИ-ЭСКИЗ */
    if ($type == "gif") {
        $dst_t_img = imagecreate($dst_thumb_w, $dst_thumb_h);
    } //для GIF
    else {
...
function Txt2Png($text, $directory)
{
    ...
    $filenameforpng = _LPATH_ROOT . "/images/boss/$directory/email/" . $png2display . ".png";
    $filename = _LPATH_SITE . "/images/boss/$directory/email/" . $png2display . ".png";
...
function installNewDirectory($installPlugins = 1)
{
    ...
    $errors = '';
    $database->setQuery(
        "INSERT INTO `#__boss_config` ( `name`, `slug`, `contents_per_page_front`, `contents_per_page`, `root_allowed`, `send_email_on_new`, `send_email_on_update`, `auto_publish`, `fronttext`, `email_display`, `display_fullname`, `rules_text`, `content_duration`, `submission_type`, `nb_contents_by_user`, `allow_attachement`, `allow_contact_by_pms`, `allow_comments`, `secure_comment`, `secure_new_content`, `use_content_plugin`, `show_rss`, `template`, `comment_sys`, `rating`) VALUES
        ( 'directory', 'dir', 7, 20, 1, 1, 1, 1, 'Текст приветствия\r\n', 0, 1, 'Это правила...\r\n', 30, 0, -1, 0, 0, 1, 1, 1, 1, 0, 'default', 1, 'defaultRating');"
    );
    $database->query();
    ...
    if (!is_dir(_LPATH_ROOT . "/images/boss/")) {
        @mkdir(_LPATH_ROOT . "/images/boss/");
        @copy(_LPATH_ROOT . "/images/index.html", _LPATH_ROOT . "/images/boss/index.html");
    };

    if (!is_dir(_LPATH_ROOT . "/images/boss/$id/")) {
        @mkdir(_LPATH_ROOT . "/images/boss/$id/");
        @copy(_LPATH_ROOT . "/images/index.html", _LPATH_ROOT . "/images/boss/$id/index.html");
        @copy(_LPATH_ROOT . "/administrator/components/com_boss/cron.php", _LPATH_ROOT . "/images/boss/$id/cron.php");
    };

    if (!is_dir(_LPATH_ROOT . "/images/boss/$id/categories/")) {
        @mkdir(_LPATH_ROOT . "/images/boss/$id/categories/");
        @copy(_LPATH_ROOT . "/images/index.html", _LPATH_ROOT . "/images/boss/$id/categories/index.html");
    };

    if (!is_dir(_LPATH_ROOT . "/images/boss/$id/contents/")) {
        @mkdir(_LPATH_ROOT . "/images/boss/$id/contents/");
        @copy(_LPATH_ROOT . "/images/index.html", _LPATH_ROOT . "/images/boss/$id/contents/index.html");
    };

    if (!is_dir(_LPATH_ROOT . "/images/boss/$id/email/")) {
        @mkdir(_LPATH_ROOT . "/images/boss/$id/email/");
        @copy(_LPATH_ROOT . "/images/index.html", _LPATH_ROOT . "/images/boss/$id/email/index.html");
    };

    if (!is_dir(_LPATH_ROOT . "/images/boss/$id/fields/")) {
        @mkdir(_LPATH_ROOT . "/images/boss/$id/fields/");
        @copy(_LPATH_ROOT . "/images/index.html", _LPATH_ROOT . "/images/boss/$id/fields/index.html");
    };

    if (!is_dir(_LPATH_ROOT . "/images/boss/$id/files/")) {
        @mkdir(_LPATH_ROOT . "/images/boss/$id/files/");
        @copy(_LPATH_ROOT . "/images/index.html", _LPATH_ROOT . "/images/boss/$id/files/index.html");
    };

    if (!is_dir(_LPATH_ROOT . "/images/boss/$id/lang/")) {
        @mkdir(_LPATH_ROOT . "/images/boss/$id/lang/");
        @copy(_LPATH_ROOT . "/images/index.html", _LPATH_ROOT . "/images/boss/$id/lang/index.html");
    };

    if (!is_dir(_LPATH_ROOT . "/images/boss/$id/js/")) {
        @mkdir(_LPATH_ROOT . "/images/boss/$id/js/");
        @copy(_LPATH_ROOT . "/images/index.html", _LPATH_ROOT . "/images/boss/$id/js/index.html");
        /*
        $f = fopen (_LPATH_ROOT . "/images/boss/$id/js/front.js", "w");
        fclose($f);
        $f = fopen (_LPATH_ROOT . "/images/boss/$id/js/admin.js", "w");
        fclose($f);
        */
    };
...
function backup_table_structure($directory, $file, $table)
{
    ...
    $content = '$query = "DROP TABLE IF EXISTS `' . str_replace(LCore::getCfg('dbprefix') . 'boss_' . $directory . '_', '#__boss_".$directory."_', $table) . '`";' . "\n";
    $content .= '$database->setQuery($query);' . "\n";
    $content .= '$database->query();' . "\n\n";
    $result = $database->getTableCreate(array($table));
    $result = implode(' ', $result);
    $result = str_replace(LCore::getCfg('dbprefix') . 'boss_' . $directory . '_', '#__boss_".$directory."_', $result);
...
function backup_table_data($directory, $file, $table)
{
    ...
    if ($count > 0) {
        ...
        while ($count > 0) {
            ...
            $content = '$query = "INSERT INTO `' . str_replace(LCore::getCfg('dbprefix') . 'boss_' . $directory . '_', '#__boss_".$directory."_', $table) . '` VALUES ';
            $first = true;
...
{
    require_once _LPATH_ROOT . '/includes/libraries/filesystem/folder.php';
