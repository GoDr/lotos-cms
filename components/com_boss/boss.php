...
require_once(_LPATH_ROOT . '/components/com_boss/boss.html.php');
require_once(_LPATH_ROOT . '/components/com_boss/boss.class.php');
require_once(_LPATH_ROOT . '/components/com_boss/boss.tools.php');

// активация кеша
$cache = mosCache::getCache('com_boss');

$task = (isset($frontpageConf->task)) ? $frontpageConf->task : LCore::getParam($_REQUEST, 'task', "front", 's');
$text_search = LCore::getParam($_REQUEST, 'text_search', "", 's');
$name_search = LCore::getParam($_REQUEST, 'name_search', "", 's');
$limitstart = LCore::getParam($_REQUEST, 'limitstart', 0, 'i');
$userid = LCore::getParam($_REQUEST, 'userid', $my->id, 'i');
$catid = LCore::getParam($_REQUEST, 'catid', 0, 'i');
$contentid = LCore::getParam($_REQUEST, 'contentid', 0, 'i');
$order = LCore::getParam($_REQUEST, 'order', 0, 'i');
$mode = LCore::getParam($_REQUEST, 'mode', 'email', 's');
$tag = LCore::getParam($_REQUEST, 'tag', '', 's');
$alpha = LCore::getParam($_REQUEST, 'alpha', '', 'u');
$directory = (isset($frontpageConf->directory)) ? (int)$frontpageConf->directory : LCore::getParam($_REQUEST, 'directory', 0, 'i');
...
if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/css/boss_' . $template_name . '.css')){
    $boss_css = _LPATH_TPL_FRONT_S . '/' . TEMPLATE . '/css/boss_' . $template_name . '.css';
}else{
    $boss_css = _LPATH_TPL_COM_S . '/com_boss/' . $template_name . '/css/boss.css';
}


$mainframe->addCustomHeadTag('<link rel="stylesheet" href="' . $boss_css . '" type="text/css" />');
...
if ($task != 'rss') {
    ...
    $last_cron_date = null;
    $fileCron = _LPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory . DS . 'cron.php';
    if (is_file($fileCron)) {
        require($fileCron);
...
switch ($task) {
    case 'calendar':
        $results = boss_calendar($template_name, $limitstart, $order);
    ...
    case 'delete_comment':
        ...
        mosRedirect(LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;contentid=" . $contentid . "&amp:catid=" . $catid . "&amp;directory=" . $directory));
        break;
...
function boss_calendar($template_name, $limitstart, $order)
{
    // загружаем языковой файл
    $_lang = LLang::getLang('com.boss');

    $mainframe = MainFrame::getInstance();
    $_db = LCore::getDB();

    $directory = LCore::getParam($_REQUEST, 'directory', 0, 'i');
    $year = LCore::getParam($_REQUEST, 'year', date("Y"), 'i');
    $month = LCore::getParam($_REQUEST, 'month', date("m"), 'i');
    $day = LCore::getParam($_REQUEST, 'day', date("d"), 'i');

    // Проверяем передана ли привязка к модулю календаря (id mod_calendar)
    $module_id = LCore::getParam($_REQUEST, 'modid', 0, 'i');
    $catid = '';
    $catval = array();
    $catif = '';
    // получение настроек модуля
    if ($module_id) {
        $row = $_db->selectCell("SELECT `params` FROM `#__modules` WHERE `id` = ?", $module_id);
        ...
        if (!empty($catid)) {
            ...
            $catif = ' LEFT JOIN `#__boss_' . $params['directory'] . '_content_category_href` AS cch ON cn.id = cch.content_id';
    ...
    $url = "index.php?option=com_boss&amp;task=calendar&amp;directory=" . $directory . "&amp;year=" . $year . "&amp;month=" . $month . "&amp;day=" . $day;

    $sql = 'SELECT cn.id
            FROM #__boss_' . $directory . '_contents as cn
            ' . $catif . '
            WHERE YEAR(cn.date_created) = ?
                AND MONTH(cn.date_created) = ?
                AND DAY(cn.date_created) = ?
                AND cn.published = ?
                ' . $catid . '
            ';

    $tagContentIds = $_db->getCacheSql('selectCol', $sql, $year, $month, $day, 1, $catval);
...
function show_search($catid, $directory, $template_name)
{
    ...
    $database = database::getInstance();
    $fields_searchable = $database->setQuery("SELECT * FROM #__boss_" . $directory . "_fields " . "WHERE searchable = 1 AND published = 1 " . $where)->loadObjectList();
    ...
    $paths[0]->link = LSef::getUrlToSef('index.php?option=com_boss&amp;directory=' . $directory);
...
function show_category($catid, $text_search, $name_search, $order, $limitstart, $directory, $template_name)
{
    ...
    $paths[$nb]->link = LSef::getUrlToSef('index.php?option=com_boss&amp;order=' . $order . '&amp;directory=' . $directory);
    ...
    ob_start();
    $params = boss_helpers::show_list(
        $cat_name, $cat_description, $url, "show_category", $search, $text_search, $name_search, $order, $catid, $limitstart, 0, $jDirectoryHtmlClass, $directory, $template_name, array(), $category->content_types
    );
    $params['page_body'] = ob_get_contents();
    ob_end_clean();
    // Dynamic Page Title
...
function show_message_form($contentid, $mode, $directory, $template_name)
{
    ...
    $content = LibArray::ArrayToObject($content);
...
function send_message($mode, $directory)
{
    ...
    $url = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;contentid=$contentid&amp;directory=$directory");
    ...
    if ($mode == 1) {
        ...
        $_PLUGINS->loadBotGroup('com_boss');
        $results = $_PLUGINS->trigger('onSendPMS', array($userid, $my->id, $title, $body), false);
    } else {
...
function show_content($contentid, $catid, $directory, $template_name)
{
    ...
    // TODO Gold Dragon : определить необходимость
    ...
    $q .= "LEFT JOIN  #__boss_" . $directory . "_content_category_href AS cch ON a.id = cch.content_id " . "LEFT JOIN #__users as u ON a.userid = u.id " . "LEFT JOIN #__boss_" . $directory
        . "_categories as c ON cch.category_id = c.id " . "LEFT JOIN #__boss_" . $directory
    ...
    $paths[$nb] = new stdClass();
    $paths[$nb]->text = $conf->name;
    $paths[$nb]->link = LSef::getUrlToSef('index.php?option=com_boss&amp;directory=' . $directory);
...
function write_content($contentid, $catid, $directory, $template_name)
{
    ...
    if (($contentid == 0) && ($my->id != "0") && ($conf->nb_contents_by_user != -1)) {
        ...
        if ($nb >= $conf->nb_contents_by_user) {
            ...
            mosRedirect(LSef::getUrlToSef("index.php?option=com_boss&amp;directory=$directory"), $redirect_text);
        }
    }
    ...
    if (($conf->submission_type == 1) && ($my->id == "0")) {
        ...
        //get fields
        $database->setQuery("SELECT * FROM #__boss_" . $directory . "_fields WHERE `published` = 1 ORDER BY `ordering`, `fieldid`");
        ...
        if ($errorMsg == "") {
            ...
            $database->setQuery("SELECT p.*,u.* FROM #__users as u " . "WHERE u.id=" . $uid);
...
function save_content($directory)
{
    ...
    if ($conf->secure_new_content == 1 && $my->id == 0) {
        ...
        if ($captcha_keystring !== $captcha) {
            ...
            $url = LSef::getUrlToSef("index.php?option=com_boss&task=write_content&catid=$catid&amp;directory=$directory&errorMsg=$errorMsg");
    ...
    if (($conf->submission_type == 0) && ($my->id == 0)) {
        ...
        if (isset($errorMsg)) {
            $catid = (int)mosGetParam($_POST, 'category', 0);
            $url = LSef::getUrlToSef("index.php?option=com_boss&task=write_content&catid=$catid&amp;directory=$directory");
            echo "<form name='form' action='" . $url . "' method='post'>";
    ...
    if ((($conf->send_email_on_new == 1) && ($isUpdateMode == 0)) || (($conf->send_email_on_update == 1) && ($isUpdateMode == 1))) {
        ...
        mosMail(LCore::getCfg('mailfrom'), LCore::getCfg('fromname'), LCore::getCfg('mailfrom'), $subject, $body, 1);
    }
    return true;
}
...
function manage_expiration($directory, $conf, $fileCron, $template_name)
{
    if ($conf->expiration == 1) {
        ...
        $database = database::getInstance();
        $q = "SELECT c.id, u.email, c.name " . "FROM #__boss_" . $directory . "_contents as c, " . "#__users as u " . "WHERE DATE_SUB(date_created, INTERVAL " . $conf->recall_time . " DAY) < (CURDATE() - "
            . $conf->content_duration . ") " . "AND u.id = c.userid";
        ...
        if (isset($contents)) {
            foreach ($contents as $content) {
                if ($conf->recall == 1) {
                    ...
                    $link = LSef::getUrlToSef('index.php?option=com_boss&amp;directory=' . $directory . '&amp;task=expiration&amp;contentid=' . $content->id);
...
function show_expiration($contentid, $directory, $template_name)
{
    ...
    $query
        ...
        . "_content_category_href as cch ON cch.content_id = a.id \n" . "LEFT JOIN #__users as u ON a.userid = u.id \n"
...
function extend_expiration($contentid, $directory)
{
    ...
    mosCache::cleanCache('com_boss');
    mosRedirect(LSef::getUrlToSef("index.php?option=com_boss&amp;directory=$directory"), BOSS_CONTENT_RESUBMIT);
...
function delete_content($contentid, $directory, $template_name)
{
    ...
    if ($my->id == "0") { // user not logged in
        ...
        if ($mode == "confirm") {
            ...
            mosRedirect(LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_user&amp;directory=$directory", ''));
        } else {
...
function show_profile($userid, $directory, $template_name)
{
    ...
    if ($userid == "0") {
        ...
        $database->setQuery("SELECT * FROM #__boss_" . $directory . "_fields as f " . "WHERE f.published = 1");
        $fields = $database->loadObjectList();
...
function save_profile($directory)
{
    ...
    mosRedirect(LSef::getUrlToSef("index.php?option=com_boss&amp;directory=$directory", BOSS_UPDATE_PROFILE_SUCCESSFULL));
}
...
function login_form($directory, $template_name)
{
    ...
    $jDirectoryHtmlClass->directory = $directory;
    $jDirectoryHtmlClass->template_name = $template_name;
    $jDirectoryHtmlClass->displayLoginForm(LSef::getUrlToSef('index.php?option=com_boss&amp;directory=' . $directory));
}
...
function emailsend($directory)
{
    ...
    if ($database->loadObject($content)) {
        ...
        // link sent in email
        $link = LSef::getUrlToSef('index.php?option=com_boss&task=show_content&contentid=' . $contentid . '&directory=' . $directory);
        if (trim(strpos($link, 'index.php')) === '0') {
            $link = _LPATH_SITE . '/' . $link;
        }

        // message text
        $msg = sprintf(BOSS_EMAIL_MSG, $yourname, $youremail, LCore::getCfg('sitename'), $subject, $link);
...
function show_rss($catid, $directory)
{
    ...
    $info['link'] = htmlspecialchars(_LPATH_SITE);
    ...
    $info['file'] = LCore::getCfg('cachepath') . '/' . $filename;
    ...
    if ($catid == 0) {
        ...
        $info['link'] = LSef::getUrlToSef("index.php?option=com_boss&amp;directory=$directory");
        $info['rsslink'] = LSef::getUrlToSef("index.php?option=com_boss&amp;task=rss&amp;no_html=1&amp;directory=$directory");
        $search = "1";
    } else {
        // get category-name: #__boss_".$directory."_category
        ...
        $info['link'] = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_category&amp;catid=$catid&amp;directory=$directory");
        $info['rsslink'] = LSef::getUrlToSef("index.php?option=com_boss&amp;task=rss&amp;catid=$catid&amp;no_html=1&amp;directory=$directory");

        $linkTarget = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_category&amp;catid=$catid&amp;directory=$directory");
    ...
    $database->setQuery(
        "SELECT a.*, p.name as parent, p.id as parentid, c.name as cat, c.id as catid, u.username as user, " . "FROM #__boss_" . $directory . "_contents as a " . "LEFT JOIN #__users as u ON a.userid = u.id "
        . "LEFT JOIN #__boss_" . $directory . "_categories as c ON a.category = c.id "
    ...
    foreach ($contents as $content) {
        $linkTarget = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;catid=$catid&ampcontentid=" . $content->id . "&amp;directory=$directory");
        ...
        $item_description = LHtml::cleanText($item_description);
...
function boss_show_cached_result($params)
{
    ...
    if (isset($params['page_body'])) {
        echo '<section class="boss_content">' . $params['page_body'] . '</section>';
    }
}
