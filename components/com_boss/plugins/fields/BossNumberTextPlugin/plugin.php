...
class BossNumberTextPlugin{
	...
	function getDetailsDisplay($directory, $content, $field, $field_values, $conf){
		...
		if(!empty($value)){
			...
			$return .= cutLongWord($value, $field->maxlength);
	...
	function getFormDisplay($directory, $content, $field, $field_values, $nameform = 'adminForm', $mode = "write"){
		...
		elseif($mode == "search"){
			$return = BOSS_FROM . "<input class='boss_required' mosReq='1' id='" . $field->name . "_min' type='text' test='number' mosLabel='" . $strtitle . "' name='" . $field->name . "_min' size='$field->size' maxlength='$field->maxlength' $read_only value='$value' /> \n";
			$return .= BOSS_TO . "<input class='boss_required' mosReq='1' id='" . $field->name . "_max' type='text' test='number' mosLabel='" . $strtitle . "' name='" . $field->name . "_max' size='$field->size' maxlength='$field->maxlength' $read_only value='$value' />\n";
		}
		else
