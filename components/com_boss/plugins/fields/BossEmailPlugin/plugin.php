 ...
 * @Date      26.07.2013
 * @see       http://wiki.lotos-cms.ru/index.php/BossEmailPlugin
 */
...
class BossEmailPlugin
{
    ...
    public function getDetailsDisplay($directory, $content, $field, $field_values, $conf)
    {
        ...
        if ($value != "") {
            switch ($config) {
                case 2:
                    $emailForm = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_message_form&amp;mode=0&amp;contentid=" . $content->id . "&amp;directory=" . $directory);
    ...
    {
        $return = LCore::getParam($_POST, $field->name, "", 'sn');
        return $return;
    }
    ...
    {
        $fieldId = $field->fieldid;
        $email_display = LCore::getParam($_POST, "email_display", 0, 'i');
    ...
    {
        $search = '';
        $value = LCore::getParam($_REQUEST, $fieldName, "", 'sn');
        if (!empty($value)) {
