 ...
 * @Date      22.07.2013
 * @see       http://wiki.lotos-cms.ru/index.php/BossDirectoryHrefPlugin
 */
...
class BossDirectoryHrefPlugin
{
    ...
    public static function getDetailsDisplay($directory, $content, $field, $field_values, $conf)
    {
        ...
        if ($value != "") {
            ...
            $value = str_replace($tmp[1], LSef::getUrlToSef($tmp[1]), $value);
            $return .= $value;
    ...
    {
        $return = LCore::getParam($_POST, $field->name . "_href" . $field->fieldid, '', 'sn');
        return $return;
    }
    ...
    {
        $directory = LCore::getParam($_REQUEST, 'sel_dir', 0, 'i');
        $fieldid = LCore::getParam($_REQUEST, 'fieldid', 0, 'sn');

        if ($directory == 0) {
            return;
        }
        $id_cat = LCore::getParam($_REQUEST, 'id_cat', 'directory_htef_category' . $fieldid, 'sn');

        require_once(_LPATH_ROOT . '/administrator/components/com_boss/admin.boss.html.php');
        ...
        foreach ($rows as $row) {
            ...
            array_push($list, LibArray::ArrayToObject($row));
            $children[$pt] = $list;
        }
    ...
    function loadHref()
    {
        ...
        echo '<a href="' . _LPATH_SITE . '/index.php?option=com_boss&task=show_content&contentid=' . $sel_cont . '&catid=' . $sel_cat . '&directory=' . $sel_dir . '">' . $name . '</a>';
    }
}
