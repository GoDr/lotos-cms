...
<div class="thumbsContainer">
	<?php
	if(file_exists(_LPATH_ROOT . $pic)){
		?>
		<a class="thumb_link" href="<?php echo $full; ?>" title="<?php echo $title; ?>" rel="thumb_set_<?php echo $content->id; ?>">
			<img class="thumb_img" src="<?php echo _LPATH_SITE . $pic; ?>" alt="<?php echo htmlspecialchars(stripslashes(cutLongWord($content->name)), ENT_QUOTES); ?>"/>
		</a>
		<?php
	} else{
        $nopic_dir = _LPATH_TPL_COM_S . "/com_boss/" . $conf->template . "/images/";
		if((BOSS_NOPIC != "") && (file_exists(_LPATH_ROOT . $nopic_dir . BOSS_NOPIC))){
			?>
			<img src="<?php echo $nopic_dir . BOSS_NOPIC; ?>" alt="nopic"/>
			<?php
		} else{
			?>
			<img src="<?php echo $nopic_dir . nopic . gif; ?>" alt="nopic"/>
			<?php
		}
	}
	?>
...
function galleryDetailsDisplay($directory, $content, $images, $conf_fields, $conf){
	...
	foreach($images as $image){
		$title = $image['signature'] ? $image['signature'] : $galleryTitle;
		//$style = 'display:block';
		$src = '/images/boss/' . $directory . '/contents/gallery/thumb/' . $image['file'];
