 ...
 * @Date      28.07.2013
 * @see       http://wiki.lotos-cms.ru/index.php/BossFileMultiPlugin
 */
...
class BossFileMultiPlugin
{
    ...
    {
        $params = array();
        $params['css'] = _LPATH_SITE . '/components/com_boss/plugins/fields/BossFileMultiPlugin/css/plugin.css';
        return $params;
    }
    ...
    private function displayFileLink($directory, $content, $field, $field_values, $row, $template, $type = "admin", $field_conf)
    {
        ...
        $size = '';
        if ($show_size) {
            $size = round(@filesize(_LPATH_ROOT . "/images/boss/" . $directory . "/files/" . $filename) / 1024, 2);
            $size = $size ? $size . ' Кб' : '';
        ...
        if ($show_img) {
            ...
            $image = '<img src=' . _LPATH_SITE . '/components/com_boss/plugins/fields/BossFileMultiPlugin/images/' . $show_img_size . '/' . $image . " alt=" . $ext . ' align="middle" border="0" />';
        }
        ...
        if ($filename) {
            if ($type == "front") { //отображение ссылки на фронте
                ...
                if ($counter) {
                    $url = _LPATH_SITE . '/ajax.index.php?option=com_boss&act=plugins&task=run_plugins_func&directory=' . $directory . '&class=BossFileMultiPlugin&function=download&file=' . $filename . '&cid='
                        . $content->id . '&fname=' . $fieldname;
                    $counterPrint = '<span class="boss_file boss_file_counter">' . BOSS_PLG_COUNTER . ' <span>' . $downloads . '</span>.</span>';
                } else {
                    $url = _LPATH_SITE . "/images/boss/" . $directory . "/files/" . $filename;
                    $counterPrint = '';
                }
                ...
                $size = round(@filesize(_LPATH_ROOT . "/images/boss/" . $directory . "/files/" . $filename) / 1024, 2);
                $return .= "<img src=\"" . _LPATH_TPL_ADMI_S . "/" . $template . "/images/file_ico/" . $image . "\" alt=\"" . $ext . "\" align=\"middle\" border=\"0\" />&nbsp;"
                    . "<a title='" . BOSS_DOWNLOAD_FILE . "' href=\"" . _LPATH_SITE . "/images/boss/" . $directory . "/files/" . $filename . "\" target=\"_blank\">"
    ...
    public function getFormDisplay($directory, $content, $field, $field_values, $nameform = 'adminForm', $mode = "write")
    {
        ...
        $mainframe->addJS(_LPATH_SITE . '/components/com_boss/plugins/fields/BossFileMultiPlugin/js/script.js');
        $mainframe->addCSS(_LPATH_SITE . '/components/com_boss/plugins/fields/BossFileMultiPlugin/css/plugin.css');
                ...
                $return
                    .= "
                        <input type='hidden' name='boss_file[" . $i . "][file]' value='" . $row['file'] . "' />
                            &nbsp;&nbsp;&nbsp;"
                    . self::displayFileLink($directory, $content, $field, $field_values, $row, TEMPLATE, 0, 'admin')
            ...
            foreach ($file_fields as $file_field) {
                ...
                if (is_array($files) && count($files) > 0) {
                    foreach ($files as $file) {
                        @unlink(_LPATH_ROOT . "/images/boss/" . $directory . "/files/" . $file->file);
                    }
                }
            }
        }
    ...
    public function getEditFieldOptions($row, $directory, $fieldimages, $fieldvalues)
    {
        ...
        $_show_img_size_op[] = LHtml::makeOption(16, '16x16');
        $_show_img_size_op[] = LHtml::makeOption(24, '24x24');
        $_show_img_size_op[] = LHtml::makeOption(32, '32x32');
        $_show_img_size_op[] = LHtml::makeOption(48, '48x48');
        $_show_img_size_op[] = LHtml::makeOption(64, '64x64');
        $_show_img_size = LHtml::selectList($_show_img_size_op, 'show_img_size', 'class="inputbox"', 'value', 'text', $_show_img_size);
        ...
        $return
            ...
            <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
                ...
                <tr>
                    ...
                    <td>' . LHtml::yesnoRadioList('counter', 'class="inputbox"', $_counter) . '</td>
                    <td>' . boss_helpers::bossToolTip(BOSS_PLG_ENABLE_COUNTER_LONG) . '</td>
                </tr>

                <tr>
                    <td>' . BOSS_PLG_SHOW_IMG . '</td>
                    <td>' . LHtml::yesnoRadioList('show_img', 'class="inputbox"', $_show_img) . '</td>
                    <td>' . boss_helpers::bossToolTip(BOSS_PLG_SHOW_IMG_LONG) . '</td>
                </tr>

                <tr>
                    <td>' . BOSS_PLG_SHOW_IMG_LINK . '</td>
                    <td>' . LHtml::yesnoRadioList('show_img_link', 'class="inputbox"', $_show_img_link) . '</td>
    ...
    public function download()
    {
        ...
        mosRedirect(_LPATH_SITE . "/images/boss/" . $directory . "/files/" . $file);
    }
}