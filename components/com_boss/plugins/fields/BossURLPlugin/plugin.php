...
class BossURLPlugin{
	...
	function getDetailsDisplay($directory, $content, $field, $field_values, $conf){
		...
		$linkObj = '';
		if((isset($field->link_image)) && (file_exists(_LPATH_ROOT . "/images/boss/$directory/fields/" . $field->link_image))){
			$linkObj .= "<img src='" . _LPATH_SITE . "/images/boss/$directory/fields/" . $field->link_image . "' />";
		}
		...
		if($value != ""){
            $host = (preg_match("#^http[s]?://#", $value)) ? '' : 'http://';
			$return .= "<a href='" . $host . $value . "' target='_blank'>$linkObj</a>";
		}
	...
	function onFormSave($directory, $contentid, $field, $isUpdateMode){
		$return = trim(mosGetParam($_POST, $field->name, ""));
        return $return;
	}
	...
	function getEditFieldOptions($row, $directory, $fieldimages, $fieldvalues){
		...
		$return .= '
                    ...
                    <td>
                        <img src="' . _LPATH_SITE . '/images/boss/$directory/fields/' . $row->link_image . '"
