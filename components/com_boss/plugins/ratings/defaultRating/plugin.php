...
class defaultRating extends mosDBTable{
	...
	function save_vote($directory){
		...
		if($conf->allow_ratings == 1){
			...
			if(($my->id == "0" && $conf->allow_unregisered_comment == 0)){
				mosRedirect(LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;&contentid=" . $row->contentid . "&amp;directory=$directory"), "");
				return;
			}
			...
			if(($nb > 0)){
				mosRedirect(LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;&contentid=" . $row->contentid . "&amp;directory=$directory"), BOSS_ALREADY_VOTE);
				return;
			}
			...
			mosRedirect(LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;&contentid=" . $row->contentid . "&amp;directory=$directory"), BOSS_THANKS_FOR_YOUR_VOTE);
		} else{
			mosRedirect(LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;&contentid=" . $row->contentid . "&amp;directory=$directory"), "");
		}
	}
	...
	function displayVoteForm($content, $directory, $conf){
		if($conf->allow_ratings){
			...
			if($my->id == 0 && $conf->allow_unregisered_comment == 0){
				...
				$target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=save_vote&amp;directory=$directory");
				?>
	...
	function displayVoteResult($content, $directory, $conf){
		if($conf->allow_ratings){
			...
			for($i = 1; $i <= 5; $i++){
				if($result >= $i){
					echo '<img src="' . _LPATH_SITE . '/components/com_boss/plugins/ratings/defaultRating/images/star_10.png" alt="star_10" align="middle" />';
				} else if($result >= $i - 0.5){
					echo '<img src="' . _LPATH_SITE . '/components/com_boss/plugins/ratings/defaultRating/images/star_05.png" alt="star_05" align="middle"/>';
				} else{
					echo '<img src="' . _LPATH_SITE . '/components/com_boss/plugins/ratings/defaultRating/images/star_00.png" alt="star_00" align="middle" />';
				}
			}
			$this->displayNumVotes($content);
			return true;
