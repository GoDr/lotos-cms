...
class defaultComment{
    ...
    private function displayButDelete($content, $directory, $reviews){
        ...
        if ($conf->allow_rights) {
            ...
            if ($rights->allow_me('delete_comments', $my->groop_id)) {
                echo '<a href="'.LSef::getUrlToSef("index.php?option=com_boss&amp;task=delete_comment&amp;contentid=".$content->id."&amp;catid=".$content->catid."&amp;directory=".$directory."&amp;id=".$reviews->id) .'" onClick="return window.confirm(\''.BOSS_DELETE_COMMENT.'\')">'._DELETE.'</a>';
            }
        }
    }
	...
	public function displayReviews($content, $directory, $conf, $reviews){
		...
		if(($conf->allow_comments == 1) && (isset($reviews))){
			foreach($reviews as $review){
				include(_LPATH_ROOT . '/components/com_boss/plugins/comments/defaultComment/template/review.php');
			}
		}
	}
	...
	public function displayAddReview($directory, $content, $conf){
		...
		if($conf->allow_comments == 0){
			...
			$target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=save_review&amp;directory=$directory");
			?>
		<form action="<?php echo $target;?>" method="post" name="saveForm">
			<?php include(_LPATH_ROOT . '/components/com_boss/plugins/comments/defaultComment/template/addreview.php');?>
		...
		if($conf->allow_comments == 1){
			...
			if($conf->secure_comment == 1 && $my->id == 0){
				...
				if($captcha_keystring !== $captcha){
					$link = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;&contentid=" . $row->contentid . "&amp;directory=$directory");
			...
			if(($my->id == "0" && $conf->allow_unregisered_comment == 0)){
				mosRedirect(LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;contentid=" . $row->contentid . "&amp;directory=$directory&amp;catid=$catid"), "");
				return;
			}
