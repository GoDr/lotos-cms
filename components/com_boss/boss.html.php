...
class boss_html
{
    ...
    **********************************************************************************************/
     ...
     *
     * @see http://wiki.lotos-cms.ru/index.php/Boss_html::displayAutor
     */
    private function displayAutor($content, $type = null)
    {
        if (LCore::getCfg('showAuthor') or $type) {
            $type = (empty($type)) ? LCore::getCfg('author_name') : $type;
            switch ($type) {
                case '1':
                ...
                case '2':
                    ...
                    break;
                case '3':
                    $_str = '<a href="' . LSef::getUrlToSef('index.php?option=com_users&amp;task=profile&amp;id=' . $content->user_id) . '">' . $content->user_fio . '</a>';
                    break;
                case '4':
                    $_str = '<a href="' . LSef::getUrlToSef('index.php?option=com_users&amp;task=profile&amp;id=' . $content->user_id) . '">' . $content->user . '</a>';
                    break;
                default:
                    $_str = null;
            }
     ...
     *
     * @param int  $catid : данные контента
     * @param int $indx   : показывать ли ссылку на весь каталог
     *                    0  - не показывать
     *                    -1 - использовать языковую константу BOSS_HOME
     *                    1  - использовать название каталога
     * @param bool $last  : делать ли последний пункт меню (на текущую страницу) активным
     *
     * @see http://wiki.lotos-cms.ru/index.php/Boss_html::displayPathway
     */
    private function displayPathway($catid = 0, $indx = -1, $last = true)
    {
        ...
        while($_id){
            $sql = "SELECT b.parent, b.name
                    ...
                    LEFT JOIN `#__boss_" . $this->directory . "_categories` AS d ON b.parent = d.id
                    WHERE b.id = ?";
        ...
        while($result_array){
            ...
            if(is_null($_tmp['id'])){
                ...
                //TODO Gold Dragon : временное решение
                ob_start();
                $this->displayDirectoryName();
                $directory_name = ob_get_contents();
                ob_end_clean();

                $result .= '<li><a href="'.LSef::getUrlToSef('index.php?option=com_boss&task=show_all&directory='.$this->directory).'">' . (($indx < 0) ? $_lang['BOSS_HOME'] : $directory_name) . '</a></li>';
            }else{
                // Делать ли последнюю ссылку активной
                if(!sizeof($result_array) and $last){
                    $result .= '<li>'.$_tmp['name'].'</li>';
                }else{
                    $result .= '<li><a href="'.LSef::getUrlToSef('index.php?option=com_boss&task=show_category&catid='.$_tmp['id'].'&directory='.$this->directory).'">'.$_tmp['name'].'</a></li>';
                }
            }
        }
    ...
    **********************************************************************************************/
    ...
    function displayList()
    {
        if ($this->task == 'show_frontpage') {
            if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/frontpage.php')){
                include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/frontpage.php');
            }else{
                include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/frontpage.php');
            }
        } else {
            if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/list.php')){
                include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/list.php');
            }else{
                include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/list.php');
            }
        }
    }
    ...
    function displayWriteForm()
    {
        ...
        ?>
        <script src="<?php echo _LPATH_SITE; ?>/includes/js/overlib_mini.js"></script>
        <?php $this->jsJumpmenu(); ?>
        <script><!--//-->
            <![CDATA[//><!--
            //*** Parametres
        ...
        if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/write.php')){
            include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/write.php');
        }else{
            include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/write.php');
        }
    }

    function displayEmailForm($content)
    {
        ?>
        <script>
        ...
        <?php $target = LSef::getUrlToSef("index2.php?option=com_boss&amp;task=emailsend"); ?>
        <form action="<?php echo $target; ?>" name="frontendForm" method="post" onSubmit="return submitbutton();">
            <?php
            if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/emailform.php')){
                include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/emailform.php');
            }else{
                include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/emailform.php');
            }
            ?>
    ...
    {
        ?>
        <?php echo '<img src="' . _LPATH_TPL_COM_S . '/com_boss/'  . $this->template_name . '/images/warning.gif" alt="warning" border="0" align="center">'; ?>
    ...
    {
        if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/rules.php')){
            include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/rules.php');
        }else{
            include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/rules.php');
        }
    }
    ...
    public function displayMessageForm($content, $user, $mode, $allow_attachement)
    {
        ...
        <fieldset id="boss_fieldset">
            ...
            <?php $target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=send_message&amp;mode=" . $mode . "&amp;directory=" . $directory); ?>
    ...
    function displayConfirmation()
    {
        ...
        $target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=delete_content&amp;contentid=" . $content->id . "&amp;mode=confirm&amp;directory=$directory");
        $link_yes = "<a href='" . $target . "'>" . BOSS_YES_DELETE . "</a>";
        $target = LSef::getUrlToSef("index.php?option=com_boss&amp;directory=$directory");
        $link_no = "<a href='" . $target . "'>" . BOSS_NO_DELETE . "</a>";
        if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/confirmation.php')){
            include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/confirmation.php');
        }else{
            include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/confirmation.php');
        }
    }
    ...
    function recurseCategories($id, $level, $children, $showChildren = 1, $showDesc = 1, $showImg = 1)
    {
        ...
        if (@$children[$id]) {
            ...
            foreach ($children[$id] as $row) {
                ...
                $link = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_category&amp;catid=" . $row->id . "&amp;order=0&amp;directory=$directory");
                if ($level == 0) {
                    ?>
                    <div class="boss_maincat">
                    <?php
                    if ($showImg == 1) {
                        if (file_exists(_LPATH_ROOT . "/images/boss/$directory/categories/" . $row->id . "cat.jpg")) {
                            echo '<a href="' . $link . '"><img class="imgcat" src="' . _LPATH_SITE . '/images/boss/' . $directory . '/categories/' . $row->id . 'cat.jpg" alt="' . $row->name . '" /></a>';
                        } else {
                            echo '<a href="' . $link . '"><img class="imgcat" src="' . _LPATH_TPL_COM_S . '/com_boss/' . $this->template_name . '/images/default.png" alt="' . $row->name . '" /></a>';
                        }
                    }?>
    ...
    {
        if (!is_dir(_LPATH_TPL_COM . '/com_boss/' . $this->template_name)) {
            echo '<script>';
            echo 'alert(\'Установите хотя-бы один шаблон, желательно чтобы это был шаблон "default".\')';
            echo '</script>';
            return;
        }
        if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/front.php')){
            include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/front.php');
        }else{
            include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/front.php');
        }
    }
    ...
    function displayLoginForm($return)
    {
        ...
        $link_login = LSef::getUrlToSef("index.php?option=login");
        $return = LSef::getUrlToSef($return);

        ?>
        <form action="<?php echo $link_login; ?>" method="post" name="login" id="login">

            <?php
            if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/login.php')){
                include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/login.php');
            }else{
                include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/login.php');
            }
            ?>

            <input type="hidden" name="lang" value="<?php echo LCore::getCfg('lang'); ?>"/>
            <?php echo $validate; ?>
    ...
    function displaySearchFields()
    {
        ...
        foreach ($fields_searchable as $fsearch) {
            if (($catid == 0) || (strpos($fsearch->catsid, ",$catid,") !== false) || (strpos($fsearch->catsid, ",-1,") !== false)) {
                ...
                $title = $return->title;
                $input = $return->input;
                if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/search_field.php')){
                    include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/search_field.php');
                }else{
                    include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/search_field.php');
                }
            }
        }
    }
    ...
    function displayFilter($showName = 0)
    {
        if ($this->conf->filter) {
            return;
        } elseif (!is_file(_LPATH_ROOT . '/components/com_boss/plugins/filters/' . $this->conf->filter . '/plugin.php')) {
            return;
        }
    ...
    function displaySearch()
    {
        ...
        $this->jsJumpmenu();
        $action = LSef::getUrlToSef('index.php?option=com_boss&directory=' . $this->directory . '&task=show_result&catid=' . $this->category->id);
        ?>
        <form action="<?php echo $action; ?>" name="adminform" method="post">
            <?php
            if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/search.php')){
                include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/search.php');
            }else{
                include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/search.php');
            }
            ?>
        </form>
    <?php
    ...
    function displaySearchForm()
    {
        ...
        <form action="index.php" method="get">
            ...
            ?>
            <input name="text_search" id="text_search" maxlength="20" class="inputbox" type="text" size="20"
    ...
    function displayContentTitle($content, $link = true)
    {
        $directory = $this->directory;
        if ($link == true) {
            $linkTarget = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;contentid=" . $content->id . "&amp;catid=" . $content->catid . "&amp;directory=$directory");
    ...
    /**
     * Выводит теги при отображении статьи
     *
     * @param bool $separator : если TRUE - будут удалены запятые
     *
     * @return bool|string
     *
     * @modification GoDr 29.01.2015
     */
    function displayTags($separator=false)
    {
        if (!empty($this->tags)) {
            if($separator){
                return BOSS_TAGS . str_replace(',', '', $this->tags);
            }else{
     ...
     * @param bool $separator : если TRUE - будут удалены запятые
     * @return bool|string
	 * @modification GoDr 20.11.2014
     */
    function displayListTags($content, $separator=false)
    {
        if (isset($this->tags[$content->id])) {
            if($separator){
                return BOSS_TAGS . str_replace(',', '', $this->tags[$content->id]);
            }else{
    ...
    function displayContentEditDelete($content)
    {
        ...
        $target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=write_content&amp;contentid=" . $content->id . "&amp;content_types=" . $content->type_content . "&amp;catid=" . $content->catid . "&amp;directory=" . $directory);
        $edit = "<a href='" . $target . "'>" . BOSS_CONTENT_EDIT . "</a> &nbsp;";
        $target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=delete_content&amp;contentid=" . $content->id . "&amp;catid=" . $content->catid . "&amp;directory=$directory");
    ...
    function PrintIcon($content, $text = null)
    {
        /*
          ...
          if ($params->get('print') && !$hide_js) {
              ...
              if ($params->get('icons')) {
                  $image = mosAdminMenus::ImageCheck('printButton.png', '/images/system/', null, null, _PRINT, 'print' . $cpr_i);
                  $cpr_i++;
              } else {
        ...
        */
        ...
        $params->set('print', 1);
        $params->set('icons', 1);
        $print_link = _LPATH_SITE . '/index2.php?option=com_boss&amp;task=show_content&amp;contentid=' . $content->id . '&amp;popup=1&amp;directory=' . $directory;

        LHtml::PrintIcon(null, $params, 0, $print_link);
        /*
        if(isset($text)){
    ...
    function EmailIcon($content, $text = null)
    {
        ...
        if (1) {
            ...
            $link = _LPATH_SITE . '/index2.php?option=com_boss&amp;task=emailform&amp;contentid=' . $content->id . "&amp;directory=$directory";

            if (isset($text)) {
                $image = $text;
            } else {
                if (1) {
                    $image = LAdminMenu::ImageCheck('emailButton.png', '/images/system/', NULL, NULL, BOSS_CMN_EMAIL, BOSS_CMN_EMAIL);
                } else {
    ...
    function displayCategoryImage($content)
    {
        ...
        if (($catid == 0) || (!file_exists(_LPATH_ROOT . '/images/boss/' . $directory . '/categories/' . $catid . 'cat_t.jpg'))) {
            echo '<img  class="imgheading" src="' . _LPATH_TPL_COM_S . '/com_boss/' . $this->template_name . '/images/default.png" alt="default" />';
        } else {
            echo '<img  class="imgheading" src="' . _LPATH_SITE . '/images/boss/' . $directory . '/categories/' . $catid . 'cat_t.jpg" alt="' . $name . '" />';
        }
    }
        ...
        if ($type > 1) {
            switch ($type) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    $link = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_category&amp;catid=" . $content->catid . "&amp;directory=" . $this->directory);
                    echo '<a href="' . $link . '" >' . $content->cat . '</a>';
                    break;
                case 3:
                    $link = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_category&amp;catid=" . $content->catid . "&amp;directory=" . $this->directory);
                    if ($content->parentid and $content->parent) {
                        $link_parent = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_category&amp;catid=" . $content->parentid . "&amp;directory=" . $this->directory);
    ...
    function displayContentBy($content)
    {
        ...
        if ($content->user_id != 0) {
            echo BOSS_FROM;
            $target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_user&amp;userid=" . $content->user_id . "&amp;directory=$directory");
            ...
            foreach ($fieldsgroup as $key => $f) {
                ...
                if ($return->value != "") {
                    ...
                    if ($hrefNum > 0 && $hrefNum == $i) {
                        $link = LSef::getUrlToSef('/index.php?option=com_boss&task=show_content&contentid=' . $content->id . '&catid=' . $content->catid . '&directory=' . $this->directory);
                        $href = '<a href="' . $link . '">';
                        $hrefEnd = '</a>';
                    } else {
                        $hrefEnd = $href = '';
                    ...
                    if ($return->title != "" and $return->value != "" and $titleEnable == 1) {
                        echo '<div class="field_block"><div class="field_title">' . $return->title . '</div>' . $divider . '<div class="field_value">' . $return->value . '</div></div>';
                    } else {
                        echo $return->value;
                    }
                    echo $hrefEnd;
    ...
    function displayCatImage()
    {
        ...
        if (($catid == 0) || (!file_exists(_LPATH_ROOT . '/images/boss/' . $directory . '/categories/' . $catid . 'cat.jpg'))) {
            echo '<img  class="imgheading" src="'._LPATH_TPL_COM_S . '/com_boss/' . $this->template_name . '/images/default.png" alt="default" />';
        } else {
    ...
    function displayCatTitle($showDefImg = 1)
    {
        ...
        if ($showDefImg > 0 and is_readable(_LPATH_ROOT . '/images/boss/' . $directory . '/categories/' . $catid . 'cat_t.jpg')) {
            echo '<img  class="imgheading" src="' . _LPATH_SITE . '/images/boss/' . $directory . '/categories/' . $catid . 'cat_t.jpg" alt="' . $name . '" />' . $name;
        } elseif ($showDefImg == 0  and is_readable(_LPATH_ROOT . '/images/boss/' . $directory . '/categories/' . $catid . 'cat_t.jpg')) {
            echo '<img  class="imgheading" src="' . _LPATH_TPL_COM_S . '/com_boss/' . $this->template_name . '/images/default_t.gif" alt="default" />' . $name;
        } else {
            echo $name;
        }
    }
    ...
    function displayAdvancedSearchLink()
    {
        ...
        $directory = $this->directory;
        ?>
        <a href="<?php echo LSef::getUrlToSef("index.php?option=com_boss&amp;task=search&amp;catid=$catid&amp;directory=$directory"); ?>"><?php echo BOSS_ADVANCED_SEARCH; ?></a>
    <?php
    ...
    function displayOrderOption()
    {
        ...
        $imgUrl = _LPATH_TPL_COM_S . '/com_boss/' . $this->template_name . '/images/' . $img;
        $link = LSef::getUrlToSef($url . $order_url . "&amp;direction=" . $direction);

        if (isset($fields)) {
            $this->jsJumpmenu(); ?>
            <select name="order" size="1" onchange="jumpmenu('parent',this)">
                <option
                    value="<?php echo LSef::getUrlToSef($url . "&amp;order=0&amp;direction=" . $direction); ?>" <?php
                if ($order == "0") {
                    echo "selected='selected'";
                } ?>>
                    <?php echo BOSS_DATE; ?>
                ...
                <?php foreach ($fields as $s) {
                    if ($s->sort == 1) {
                        ?>
                        <option
                            value="<?php echo LSef::getUrlToSef($url . "&amp;order=" . $s->fieldid . "&amp;direction=" . $direction); ?>" <?php
                        if ($order == $s->fieldid) {
                            echo "selected='selected'";
                        } ?>>
            ...
            foreach ($this->contents as $content) {
                ...
                if ($listTemplate == 'frontpage') {
                    ...
                    if ($content->featured == 1) {
                        $tpl = 'featured_item.php';
                        //$tpl = $this->viewsPlugin->contentViews($content, $this->views);
                    } else {
                        $tpl = 'list_item.php';
                    }
                }
                if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/' . $tpl)){
                    include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/' . $tpl);
                }else{
                    include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/' . $tpl);
                }
            }
        }
    }
    ...
    function displayShowOther($content)
    {
        ...
        if ($content->user_id != 0) {
            $target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_user&amp;userid=" . $content->userid . "&amp;directory=$directory");
            ...
            if (($my->id == $row->userid) && ($update_possible == 1)) {
                ...
                <div>
                    <?php
                    $target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=write_content&amp;contentid=$content->id" . "&amp;directory=$directory");
                    echo "<a href='" . $target . "'>" . BOSS_CONTENT_EDIT . "</a>";
                    echo "&nbsp;";
                    $target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=delete_content&amp;contentid=$content->id" . "&amp;directory=$directory");
    ...
    function displayPms($content, $private = 0)
    {
        ...
        if (($content->userid != 0) && ($conf->allow_contact_by_pms == 1)) {
            ...
            $pmsForm = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_message_form&amp;mode=1&amp;contentid=" . $content->id . "&amp;directory=$directory");
    ...
    function displayContent($content, $unique = 1)
    {
        $conf = $this->conf;
        $mainframe = MainFrame::getInstance();
        if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/content.php')){
            include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/content.php');
        }else{
            include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/content.php');
        }
        ...
        if (isset($fields)) {
            ...
            foreach ($fields as $field) {
                ...
                if ((strpos($field->catsid, ",$catid,") !== false) || (strpos($field->catsid, ",-1,") !== false)) {
                    ...
                    if(file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/write_field.php')){
                        include(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $this->template_name . '/write_field.php');
                    }else{
                        include(_LPATH_TPL_COM . '/com_boss/' . $this->template_name . '/write_field.php');
                    }
                }
            }
        }
    ...
    function displayCategoriesSelectSearch()
    {
        ...
        <select name='category_choose' onchange="jumpmenu('parent',this)">
            <option
                value="<?php echo LSef::getUrlToSef("index.php?option=com_boss&amp;task=search&amp;catid=0&amp;directory=$directory"); ?>" <?php if ($catid == 0) {
                echo 'selected="selected"';
    ...
    function displayContentTypesSelect($task = 'write_content')
    {
        ...
        <select class='boss_required inputbox' name='content_types_choose' onchange="jumpmenu('parent',this)">
            ...
            ?>
            <option
                value='<?php echo LSef::getUrlToSef("index.php?option=com_boss$linkoption&amp;content_types=0&amp;directory=" . $this->directory); ?>'><?php echo BOSS_SELECT_CONTENT_TYPE2; ?></option>
            <?php
            foreach ($content_types as $row) {
                ...
                ?>
                <option
                    value="<?php echo LSef::getUrlToSef("index.php?option=com_boss$linkoption&amp;content_types=" . $row->id . "&amp;directory=" . $this->directory); ?>"
    ...
    function displayCaptchaImage()
    {
        ...
        <img id="captchaimg" alt="<?php echo _PRESS_HERE_TO_RELOAD_CAPTCHA ?>"
             onclick="document.saveForm.captchaimg.src='<?php echo _LPATH_SITE; ?>/includes/libraries/kcaptcha/index.php?session=<?php echo MainFrame::sessionCookieName() ?>&' + new String(Math.random())"
             src="<?php echo _LPATH_SITE; ?>/includes/libraries/kcaptcha/index.php?session=<?php echo MainFrame::sessionCookieName() ?>"/>
    <?php
    ...
    function displayFormBegin($mode = "write")
    {
        ...
        if ($mode == "write") {
            $target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=save_content&amp;directory=$directory");
        } else {
            $target = LSef::getUrlToSef("index.php?option=com_boss&amp;task=send_arrange&amp;directory=$directory");
        }
        $catid = $this->category->id;
    ...
    function displayWriteLink()
    {
        ...
        if (!isset($catid)) {
            $link_write_content = LSef::getUrlToSef("index.php?option=com_boss&amp;task=write_content&amp;directory=$directory");
        } else {
            $link_write_content = LSef::getUrlToSef("index.php?option=com_boss&amp;task=write_content&amp;catid=$catid&amp;directory=$directory");
        }
    ...
    {
        $directory = $this->directory;
        $link_show_all = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_all&amp;directory=$directory");
    ...
    {
        $directory = $this->directory;
        $link_show_user = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_user&amp;directory=$directory");
    ...
    {
        $directory = $this->directory;
        $link_show_search = LSef::getUrlToSef("index.php?option=com_boss&amp;task=search&amp;directory=$directory");
    ...
    function displayRulesLink()
    {
        ...
        $link_show_rules = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_rules&amp;directory=$directory");
    ...
    function displayLastContents()
    {
        ...
        foreach ($contents as $row) {
            ...
            <div class="boss_content_box">
                <?php
                $linkTarget = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_content&amp;contentid=" . $row->id . "&amp;catid=" . $row->category . "&amp;directory=$directory");
                $ok = 0;
                $i = 1;
                while (!$ok) {
                    if ($i < $nb_images + 1) {
                        $ext_name = chr(ord('a') + $i - 1);
                        if (is_readable(_LPATH_ROOT . "/images/boss/$directory/contents/" . $row->id . $ext_name . "_t.jpg")) {
                            $pic = _LPATH_SITE . "/images/boss/$directory/contents/" . $row->id . $ext_name . "_t.jpg";
                        } elseif (is_readable(_LPATH_ROOT . "/images/boss/$directory/contents/" . $row->id . $ext_name . "_t.png")) {
                            $pic = _LPATH_SITE . "/images/boss/$directory/contents/" . $row->id . $ext_name . "_t.png";
                        } elseif (is_readable(_LPATH_ROOT . "/images/boss/$directory/contents/" . $row->id . $ext_name . "_t.gif")) {
                            $pic = _LPATH_SITE . "/images/boss/$directory/contents/" . $row->id . $ext_name . "_t.gif";
                        } else {
                            $pic = null;
                        }
                        if (!is_null($pic)) {
                        ...
                        if ($nb_images != 0) {
                            echo "<a href='" . $linkTarget . "'><img src='" . _LPATH_TPL_COM_S . "/com_boss/" . $this->template_name . "/images/nopic.gif' alt='nopic' border='0' /></a>";
                            $ok = 1;
                        } else {
                            $ok = 1;
                        }
    ...
    {
        $link_lostpassword = LSef::getUrlToSef("index.php?option=com_users&task=lostPassword");
        echo '<a href="' . $link_lostpassword . '">' . BOSS_LOST_PASSWORD . "</a>";
    }

    function displayCreateAccountLink()
    {
        $link_create = LSef::getUrlToSef("index.php?option=com_users&task=register");
    ...
    function alpaPrint($directory, $alphabet, $alphaContent)
    {
        ...
        $catid = LCore::getParam($_GET, 'catid', '0', 'sn');

        foreach ($alphabet as $alf) {
            if (in_array($alf, $alphaContent)) {
                echo '<a class="alphaindex" href="' . LSef::getUrlToSef('index.php?option=com_boss&amp;catid=' . $catid . '&amp;task=search_alpha&amp;directory=' . $directory . '&amp;alpha=' . urlencode($alf) . $order . $direction) . '">' . $alf . '</a> ';
            } else {
                echo $alf . ' ';
            }
        }
    ...
    function show_expiration($content, $conf)
    {
        ...
        $target = LSef::getUrlToSef("index.php?option=com_boss&task=extend_expiration&contentid=$content->id");
        ?>
        <div class="renew">
