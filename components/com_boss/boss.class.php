...
class jDirectoryConf extends mosDBTable
{
    ...
    var $contents_per_page_front = null;
...
class jDirectoryCategory extends mosDBTable
{
    ...
    public static function saveCategory($directory, $conf)
    {
        ...
        if ($_POST['cb_image'] == "delete") {
            $pict = _LPATH_ROOT . "/images/boss/$directory/categories/" . $row->id . "cat.jpg";
            if (file_exists($pict)) {
                unlink($pict);
            }
            $pict = _LPATH_ROOT . "/images/boss/$directory/categories/" . $row->id . "cat_t.jpg";
            if (file_exists($pict)) {
                unlink($pict);
            }
        }
        ...
        if (isset($_FILES['cat_image']) and !$_FILES['cat_image']['error']) {
            $path = _LPATH_ROOT . "/images/boss/$directory/categories/";
            createImageAndThumb(
                $_FILES['cat_image']['tmp_name'], $_FILES['cat_image']['name'], $path, $row->id . "cat.jpg", $row->id . "cat_t.jpg", $conf->cat_max_width, $conf->cat_max_height, $conf->cat_max_width_t,
...
class jDirectoryContent extends mosDBTable
{
    ...
    public static function newContent($directory, $conf)
    {
        ...
        //get fields
        $fields = $database->setQuery("SELECT * FROM #__boss_" . $directory . "_fields WHERE published = 1 AND (FIND_IN_SET($type_content, `catsid`) > 0 OR `catsid` = ',-1,') ORDER by ordering")
            ->loadObjectList();
        if ($database->getErrorNum()) {
            echo $database->stderr();
            return false;
    ...
    public static function saveContent($directory, $conf)
    {
        ...
        //get fields
        $fields = $database->setQuery("SELECT * FROM #__boss_" . $directory . "_fields WHERE `published` = 1 AND (FIND_IN_SET($type_content, `catsid`) > 0 OR `catsid` = ',-1,') ")->loadObjectList();
        if ($database->getErrorNum()) {
            echo $database->stderr();
            return false;
        }
    ...
    public function saveSelf($directory, $fields, $conf, $isUpdateMode = 0)
    {
        ...
        //теги
        require_once(_LPATH_ROOT . '/includes/libraries/tags/tags.php');
        ...
        if ($act != '') {
            ...
            $url = LSef::getUrlToSef("index.php?option=com_boss&task=show_content&contentid=" . $this->id . "&catid=" . $category . "&directory=" . $directory);
        }
    ...
    public static function displayContent($directory, $conf)
    {
        ...
        //get fields
        $database->setQuery(
            "SELECT * FROM #__boss_" . $directory . "_fields WHERE `published` = 1 AND (FIND_IN_SET($row->type_content, `catsid`) > 0 OR `catsid` = ',-1,') ORDER BY `ordering`, `fieldid`"
        );
    ...
    public static function listContents($directory, $conf)
    {
        ...
        $categs = $database->setQuery(
            "SELECT c.name, c.id , cch.content_id " . "FROM #__boss_" . $directory . "_categories as c, " . "#__boss_" . $directory . "_content_category_href as cch " . "WHERE cch.category_id = c.id "
        )->loadObjectList();
        if ($database->getErrorNum()) {
            echo $database->stderr();
            return false;
...
class jDirectoryField extends mosDBTable
{
    ...
    function getFieldValue($field, $content, $field_values, $directory, $conf)
    {
        ...
        if (isset($content->$fieldName) and $user->gid >= $field->access) {
    ...
    public static function showFields($directory, $conf)
    {
        ...
        $tabs = new Sliders();
        LHtml::loadJquery();
        LHtml::loadJqueryPlugins('jquery.form');
        LHtml::loadJqueryUI();
    ...
    public static function editField($directory)
    {
        ...
        $sort_direction[] = LHtml::makeOption('DESC', BOSS_CMN_SORT_DESC);
        $sort_direction[] = LHtml::makeOption('ASC', BOSS_CMN_SORT_ASC);

        $display_title_list[] = LHtml::makeOption(0, BOSS_NO_DISPLAY);
        $display_title_list[] = LHtml::makeOption(1, BOSS_DISPLAY_DETAILS);
        $display_title_list[] = LHtml::makeOption(2, BOSS_DISPLAY_LIST);
        $display_title_list[] = LHtml::makeOption(3, BOSS_DISPLAY_LIST_AND_DETAILS);

        $database->setQuery("SELECT id AS `value`, `name` AS text FROM `#__groups` ORDER BY id");
        $groups = $database->loadObjectList();
        $groups[] = LHtml::makeOption(-1, BOSS_FIELD_GUEST);
        $access = LHtml::selectList($groups, 'access', 'class="inputbox" size="4"', 'value', 'text', $row->access);

        $lists['display_title'] = LHtml::selectList($display_title_list, 'display_title', 'class="inputbox" style="width:200px;" size="1"', 'value', 'text', $row->display_title);
        $lists['required'] = LHtml::yesnoRadioList('required', 'class="inputbox" size="1"', $row->required);
        $lists['editable'] = LHtml::yesnoRadioList('editable', 'class="inputbox" size="1"', $row->editable);
        $lists['searchable'] = LHtml::yesnoRadioList('searchable', 'class="inputbox" size="1"', $row->searchable);
        $lists['sort'] = LHtml::yesnoRadioList('sort', 'class="inputbox" size="1"', $row->sort);
        $lists['sort_direction'] = LHtml::selectList($sort_direction, 'sort_direction', 'class="inputbox" size="1"', 'value', 'text', $row->sort_direction);
        $lists['published'] = LHtml::yesnoRadioList('published', 'class="inputbox" size="1"', $row->published);
        $lists['filter'] = LHtml::yesnoRadioList('filter', 'class="inputbox" size="1"', $row->filter);
        $lists['access'] = $access;

        $path = _LPATH_ROOT . "/images/boss/$directory/fields";
        $handle = opendir($path);
    ...
    public static function saveField($dir)
    {
        ...
        foreach ($directories as $directory) {
            ...
            if ($plugfield == false) {
                ...
                $tableFields = $database->getTableFields(array("#__boss_" . $directory . "_contents"));
...
class boss_helpers
{
    ...
    {
        $arr_replace_utf = array('\u0410', '\u0430', '\u0411', '\u0431', '\u0412', '\u0432', '\u0413', '\u0433', '\u0414', '\u0434', '\u0415', '\u0435', '\u0401', '\u0451', '\u0416', '\u0436', '\u0417', '\u0437',
            '\u0418', '\u0438', '\u0419', '\u0439', '\u041a', '\u043a', '\u041b', '\u043b', '\u041c', '\u043c', '\u041d', '\u043d', '\u041e', '\u043e', '\u041f', '\u043f', '\u0420', '\u0440',
            '\u0421', '\u0441', '\u0422', '\u0442', '\u0423', '\u0443', '\u0424', '\u0444', '\u0425', '\u0445', '\u0426', '\u0446', '\u0427', '\u0447', '\u0428', '\u0448', '\u0429', '\u0449',
            '\u042a', '\u044a', '\u042b', '\u044b', '\u042c', '\u044c', '\u042d', '\u044d', '\u042e', '\u044e', '\u042f', '\u044f', '\'');
        $arr_replace_cyr = array('А', 'а', 'Б', 'б', 'В', 'в', 'Г', 'г', 'Д', 'д', 'Е', 'е', 'Ё', 'ё', 'Ж', 'ж', 'З', 'з', 'И', 'и', 'Й', 'й', 'К', 'к', 'Л', 'л', 'М', 'м', 'Н', 'н', 'О', 'о', 'П', 'п', 'Р', 'р', 'С',
            'с', 'Т', 'т', 'У', 'у', 'Ф', 'ф', 'Х', 'х', 'Ц', 'ц', 'Ч', 'ч', 'Ш', 'ш', 'Щ', 'щ', 'Ъ', 'ъ', 'Ы', 'ы', 'Ь', 'ь', 'Э', 'э', 'Ю', 'ю', 'Я', 'я', '"');
    ...
    public static function addDirectoryScript($directory)
    {
        ...
        (is_file(_LPATH_ROOT . $script)) ? $mainframe->addJS(_LPATH_SITE . $script) : null;
    }
    ...
    public static function copy_folder_rf($pathFrom, $pathTo)
    {
        ...
        mosMakePath(_LPATH_ROOT, str_replace(_LPATH_ROOT, '', $pathTo));
    ...
    {
        $defaultLangPath = _LPATH_ROOT . DS . 'components' . DS . 'com_boss' . DS . 'lang' . DS;
        $directoryLangPath = _LPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory . DS . 'lang/';
        if (!$directory) {
            if (is_file($defaultLangPath . LCore::getCfg('lang') . '.php')) {
                require_once($defaultLangPath . LCore::getCfg('lang') . '.php');
            } else {
                if (is_file($defaultLangPath . 'russian.php')) {
                    require_once($defaultLangPath . 'russian.php');
                }
            }
        } else {
            if (is_file($directoryLangPath . LCore::getCfg('lang') . '.php')) {
                require_once($directoryLangPath . LCore::getCfg('lang') . '.php');
            } else {
                if (is_file($directoryLangPath . 'russian.php')) {
     ...
     * @return void
     */
    public static function loadBossPluginLang($typePlugin, $plugin)
    {
        $langPath = _LPATH_ROOT . '/components/com_boss/plugins/' . $typePlugin . '/' . $plugin . '/lang/';

        if (is_file($langPath . LCore::getCfg('lang') . '.php')) {
            require_once($langPath . LCore::getCfg('lang') . '.php');
        } elseif (is_file($langPath . 'russian.php')) {
    ...
    public static function bossToolTip($tooltip, $title = '', $image = 'tooltip.png')
    {
        ...
        $image = _LPATH_SITE . '/includes/js/ThemeOffice/' . $image;
    ...
    {
        $file = _LPATH_ROOT . "/images/boss/$directory/fields/" . basename($_FILES['uploadfile']['name']);
    ...
    public static function upload_file($directory, $folder)
    {
        ...
        //целевой файл
        $file = _LPATH_ROOT . "/images/boss/$directory/$folder/" . $filename;
        //если целевой файл есть, то даем ему уникальное имя
        if (is_file($file)) {
            $i = 0;
            $fname = $filename;
            while (file_exists(_LPATH_ROOT . "/images/boss/$directory/$folder/" . $fname)) {
                $i++;
                $fname = $i . "_" . $filename;
            }
            $file = _LPATH_ROOT . "/images/boss/$directory/$folder/" . $fname;
            $filename = $fname;
        }
        if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $file)) {
            echo $filename;
    ...
    public static function delete_pack($directory)
    {
        ...
        $file = _LPATH_ROOT . "/images/boss/$directory/$pack.zip";
        if (is_file($file)) {
            unlink($file);
        }
        if (is_file($file)) {
    ...
    public static function delete_file($directory)
    {
        ...
        $file = _LPATH_ROOT . "/images/boss/$directory/$folder/$file";
        if (is_file($file)) {
            unlink($file);
        }
        if (is_file($file)) {
    ...
    public static function get_cattree($directory, $conf, $onlyNotEmpty = 1, $mode = 'read', $isUpdateMode = 0)
    {
        ...
        if ($conf->allow_rights) {
            ...
            if ($mode == 'read') {
                ...
                if ($mode == 'write' && $isUpdateMode == 0) {
                    $action = 'create_content';
                } else {
                    if ($mode == 'write' && $isUpdateMode == 1) {
                        $action = 'edit';
                    }
                }
            }
        ...
        if ($onlyNotEmpty == 1) {
            $q = "SELECT c.*, count(*) as num_contents,a.id as not_empty, parent.id as is_parent " . "FROM #__boss_" . $directory . "_categories as c " . "LEFT JOIN #__boss_" . $directory
                . "_categories as parent ON c.id = parent.parent " . "LEFT JOIN #__boss_" . $directory . "_content_category_href as cch ON c.id = cch.category_id " . "LEFT JOIN #__boss_" . $directory
                . "_contents as a ON a.id = cch.content_id " . "WHERE  c.published = 1 " . "GROUP BY c.id " . "ORDER BY c.parent, c.ordering, c.name";
            $list = $database->setQuery($q)->loadObjectList();
                        ...
                        if ($onlyNotEmpty != 1 || $v->not_empty > 0 || ($v->is_parent > 0 && $notEmptyChild == 1)) {
                            ...
                            if ($conf->allow_rights) {
                                ...
                                if ($action == 'edit' && ($rights->allow_me('edit_all_content', $my->groop_id) || $rights->allow_me('edit_user_content', $my->groop_id))) {
                                    ...
                                    if ($rights->allow_me($action, $my->groop_id)) {
                                        $tree[$pt] = $list_temp;
                                    }
                                }
                            } else {
        ...
        if (isset($cats)) {
            foreach ($cats as $cat) {
                if ($cat->parent == $catid) {
                    ...
                    $list[$i++]->link = LSef::getUrlToSef('index.php?option=com_boss&amp;task=show_category&amp;catid=' . $cat->id . '&amp;order=' . $order . '&amp;directory=' . $directory);
                }
            }
        }
        return $list;
    ...
    public static function get_pathlist($cats, $catid, $catname, $list, $order, $directory)
    {
        ...
        if (isset($cats)) {
            ...
            $list[$i]->link = LSef::getUrlToSef('index.php?option=com_boss&amp;task=show_category&amp;catid=' . $catid . '&amp;order=' . $order . '&amp;directory=' . $directory);
            $i++;
            ...
            if ($catid != -1) {
                ...
                while ($orderlist[$current]->parent != 0) {
                    ...
                    $list[$i]->link = LSef::getUrlToSef('index.php?option=com_boss&amp;task=show_category&amp;catid=' . $orderlist[$current]->id . '&amp;order=' . $order . '&amp;directory=' . $directory);
                    $i++;
                }
            }
        }
    ...
    /**
     * @param null|string       $text          - Заголовок
     * @param null|string       $description   - Описание
     * @param null|string       $url           - Путь до контента при сортировки
     * @param                   $task
     * @param                   $search
     * @param                   $text_search
     * @param                   $name_search
     ...
     * @param                   $catid
     * @param                   $limitstart
     * @param                   $update_possible
     * @param                   $jDirectoryHtmlClass
     * @param                   $directory
     * @param null|string       $template_name - имя шаблона
     * @param array             $tagContentIds : ID категорий
     * @param int               $type_content
     *
     * @return array|bool
     */
    public static function show_list(
        $text, $description, $url, $task, $search, $text_search, $name_search, $order, $catid, $limitstart, $update_possible, $jDirectoryHtmlClass, $directory, $template_name, $tagContentIds = array(), $type_content = 0
    )
    {
        $_db = LCore::getDB();
        $my = LCore::getUser();
        ...
        $fields_searchable = $database->setQuery("SELECT * FROM #__boss_" . $directory . "_fields " . "WHERE filter = 1 AND published = 1 " . $type_content_q . " ORDER BY ordering")->loadObjectList();
        if ($database->getErrorNum()) {
            echo $database->stderr();
            return false;
        }
        ...
        if ($task == 'show_frontpage') {
            ...
            $limit = $conf->contents_per_page_front;
        } else {
        ...
        //TODO необходимо сделать настраиваемую сортировку на главной
        $ordering = ($task == 'show_frontpage') ? "a.date_created DESC" : $order_text;
        ...
        $nav_link = $url . "&amp;order=" . $order;

        $conf->update_possible = $update_possible;

        $sql
            = "SELECT g.*
                FROM #__boss_" . $directory . "_groups AS g
                WHERE g.published = ?
                AND g.template = ?";
        $groupstemp = $_db->select($sql, 1, $template_name);
        ...
        $sql
            = "SELECT g.name AS gname,f.*
                FROM #__boss_" . $directory . "_groupfields as fg
                ...
                WHERE g.published = 1 " . $groupids . "
                    AND f.published = 1
                    AND fg.type_tmpl = 'category'
    ...
    public static function arr_to_links($directory, $tags, $ds = ', ')
    {
        ...
        $return = array();
        foreach ($tags as $tag) {
            $return[] = '<a class="tag" href="' . LSef::getUrlToSef('index.php?option=com_boss&amp;task=search_tags&amp;directory=' . $directory . '&amp;tag=' . urlencode($tag)) . '">' . $tag . '</a>';
        }
    ...
    public static function loadAlphaIndex($directory)
    {
        ...
        $return['alphaContent'] = $database->setQuery(
            "SELECT DISTINCT UCASE(LEFT(`name`, 1)) FROM #__boss_" . $directory
            . "_contents WHERE (date_publish = '0000-00-00 00:00:00' OR date_publish <= NOW()) AND (date_unpublish = '0000-00-00 00:00:00' OR date_unpublish >= NOW()) AND published = '1'"
        )->loadResultArray();
...
class BossDirectory
{
    ...
    public static function removeDirectory($id)
    {
        ...
        $database->setQuery(
            "DROP TABLE IF EXISTS `#__boss_" . $id . "_categories`, " . "`#__boss_" . $id . "_contents`, " . "`#__boss_" . $id . "_content_category_href`, " . "`#__boss_" . $id . "_content_types`, " . "`#__boss_" . $id
            . "_field_values`, " . "`#__boss_" . $id . "_fields`, " . "`#__boss_" . $id . "_groupfields`, " . "`#__boss_" . $id . "_groups`, " . "`#__boss_" . $id . "_rating`, "
            . "`#__boss_" . $id . "_reviews`; "
        )->query();

        $database->setQuery("DELETE FROM `#__boss_config` WHERE `id` = $id")->query();

        boss_helpers::rmdir_rf(_LPATH_ROOT . "/images/boss/$id");
    }
}
...
class BossTemplates
{
    ...
    public static function change_template($directory, $fieldid)
    {
        ...
        $tpl = array();
        $tpl[] = LHtml::makeOption('0', BOSS_SELECT_TEMPLATE);
        foreach ($templates as $template) {
            $tpl[] = LHtml::makeOption($template, $template);
        }
        $templates = LHtml::selectList($tpl, 'template', 'class="inputbox_tpl" id="template" size="1" onchange="tpl_poz_field()"', 'value', 'text', '');

        $tpl = array();
        $tpl[] = LHtml::makeOption(0, BOSS_SELECT_TYPE_TEMPLATE);
        $tpl[] = LHtml::makeOption('category', BOSS_FORM_CATEGORY);
        $tpl[] = LHtml::makeOption('content', BOSS_LIST_CONTENTS);
        $type_tpl = LHtml::selectList($tpl, 'template_type', 'class="inputbox_tpl" id="template_type" size="1" onchange="tpl_poz_field()"', 'value', 'text', '');
    ...
    public static function save_poz($directory, $fieldid)
    {
        ...
        foreach ($pozitions as $pozition) {
            $q = "INSERT INTO #__boss_" . $directory . "_groupfields " . "(`fieldid`, `groupid`, `template`, `type_tmpl`, `ordering`) " . "VALUES " . "('" . $fieldid . "', '" . $pozition . "', '" . $template . "', '"
                . $template_type . "', '0')";
            $database->setQuery($q)->query();
    ...
    public static function editTemplate($directory, $template, $type_tmpl, $conf)
    {
        ...
        $query = "SELECT `fieldid`, `name`, `title` " . "FROM #__boss_" . $directory . "_fields WHERE published = 1 " . "ORDER BY `title` ASC";
    ...
    public static function saveTemplate($directory, $template, $type_tmpl)
    {
        ...
        foreach ($fields as $field) {
            ...
            $q = "INSERT INTO #__boss_" . $directory . "_groupfields " . "(`fieldid`,               `groupid`,      `template`,      `type_tmpl`,      `ordering`) " . "VALUES " . "('" . $field['fieldid'] . "', '"
                . $field['groupid'] . "', '" . $template . "', '" . $type_tmpl . "', '" . $field['ordering'] . "')";
            $database->setQuery($q)->query();
        }
    ...
    public static function deleteTemplate($directory)
    {
        ...
        foreach ($tid as $template) {
            if ($template != "") {
                boss_helpers::rmdir_rf(_LPATH_TPL_COM . "/com_boss/" . $template);
            }
...
class BossTemplateFields
{
    ...
    public static function saveTmplField($directory)
    {
        ...
        if ($id > 0) {
            $q = "UPDATE #__boss_" . $directory . "_groups SET " . "`name`='$name', `desc`='$desc', `template`='$template', `type_tmpl`='$type_tmpl', `published`='$published', `catsid`='$catsid' "
                . "WHERE  `id`= '$id' LIMIT 1";
            $database->setQuery($q)->query();
            if ($database->getErrorNum()) {
                echo $database->stderr();
    ...
    public static function editTmplSource($directory, $template, $source_file, $conf)
    {
        $files = array();
        $source = '';
        $path = _LPATH_TPL_COM . "/com_boss/$template";
        $files[] = "css/boss.css";
        $handle = opendir($path);
        while ($file = readdir($handle)) {
    ...
    {
        $path = _LPATH_TPL_COM . "/com_boss/$template/$source_file";
    ...
    public static function getTemplateFields($directory, $template, $type_tmpl = '')
    {
        ...
        if (count($fields) == 0) {
            ...
            if (file_exists(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $template . '/_service.php')) {
                require(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/components/com_boss/' . $template . '/_service.php');
            } else {
                require(_LPATH_TPL_COM . '/com_boss/' . $template . '/_service.php');
            }
            $q = "INSERT INTO #__boss_" . $directory . "_groups " . "(`name`, `desc`, `template`, `type_tmpl`, `published`, `catsid`) " . "VALUES ";
            $q1 = array();
...
class BossPlugins
{
    ...
    {
        $bossPlugins = array();
        $path = _LPATH_ROOT . '/components/com_boss/plugins/' . $folder . '/';
        $handle = opendir($path);
        while ($file = readdir($handle)) {
            if (($file != ".") && ($file != "..")) {
    ...
    {
        $path = _LPATH_ROOT . '/components/com_boss/plugins/' . $folder . '/' . $name . '/';
    ...
    public static function run_plugins_func($directory)
    {
        ...
        $path = _LPATH_ROOT . '/components/com_boss/plugins/fields/' . $class . '/plugin.php';
        if (is_file($path)) {
            require_once($path);
        } else {
            return false;
    ...
    public static function deletePlugin($directory)
    {
        ...
        foreach ($tid as $pluginname) {
            $bossPlugins = array();
            $path = _LPATH_ROOT . "/images/boss/$directory/plugins/$pluginname";
    ...
    public static function installPlugin($directory)
    {
        ...
        if ($ext == 'zip') {
            // Extract functions
            require_once(_LPATH_ROOT . '/administrator/includes/pcl/pclzip.lib.php');
            require_once(_LPATH_ROOT . '/administrator/includes/pcl/pclerror.lib.php');
            ...
            require_once(_LPATH_ROOT . '/includes/Archive/Tar.php');
        ...
        foreach ($directories as $dir) {
            if ($ext == 'zip') {
                // Extract functions
                $ret = $zipfile->extract(PCLZIP_OPT_PATH, _LPATH_ROOT . "/images/boss/$dir/plugins/$folder");
                if ($ret == 0) {
                    $zipfile->setError(1, 'Unrecoverable error "' . $zipfile->errorName(true) . '"');
                    return false;
                }
            } else {
                if (!$archive->extractModify(_LPATH_ROOT . "/images/boss/$dir/plugins/$folder", '')) {
                    $archive->setError(1, 'Extract Error');
                    return false;
                }
            }
            require_once(_LPATH_ROOT . "/images/boss/$dir/plugins/$folder/$name/plugin.php");
    ...
    public static function listPlugins($directory, $conf)
    {
        ...
        $path = _LPATH_ROOT . "/images/boss/$directory/plugins";
        if (!is_dir($path)) {
            mkdir($path);
        }
...
class bossFieldImages
{
    ...
    public static function deleteFieldImage($directory)
    {
        ...
        foreach ($tid as $filename) {
            if ($filename != "") {
                @unlink(_LPATH_ROOT . "/images/boss/$directory/fields/" . $filename);
            }
        }
    ...
    public static function uploadFieldImage($directory)
    {
        ...
        while (file_exists(_LPATH_ROOT . "/images/boss/$directory/fields/" . $filename)) {
            $filename = "copy_" . $filename;
        }
        is_file($userfile['tmp_name']) ? move_uploaded_file($userfile['tmp_name'], _LPATH_ROOT . "/images/boss/$directory/fields/" . $filename) : null;
...
class bossExportImport
{
    ...
    public static function showImpExpForm($directory, $conf)
    {
        ...
        $directories = BossDirectory::getDirectories();
        $packs = array();
        if (is_dir(_LPATH_ROOT . '/images/boss/' . $directory)) {
            if ($handle = opendir(_LPATH_ROOT . '/images/boss/' . $directory)) {
                while (false !== ($file = readdir($handle))) {
    ...
    public static function exportDirectory($directory)
    {
        ...
        $patch = _LPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory . DS . $pack_name;
        $patchToTemplates = _LPATH_ROOT . DS . 'templates' . DS . 'com_boss';
        $patchToPlugins = _LPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory . DS . 'plugins';
        $patchToContentFolders = _LPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory;
        ...
        //архивируем файлы
        zipFolder($patch, _LPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory . DS . $pack_name . '.zip');
    ...
    public static function importDirectory()
    {
        ...
        mosMakePath(_LPATH_ROOT . "/images", 'boss/tmp');
        //разархивируем во временную папку
        $ret = $zipfile->extract(PCLZIP_OPT_PATH, _LPATH_ROOT . "/images/boss/tmp");
        if ($ret == 0) {
        ...
        $pathFrom = _LPATH_ROOT . "/images/boss/tmp";
        //путь куда копировать
        $pathTo = _LPATH_ROOT . "/images/boss/$directory";
        ...
        is_dir($pathFrom . '/templates') ? boss_helpers::copy_folder_rf($pathFrom . '/templates', _LPATH_TPL_COM . "/com_boss") : null;
        ...
        $pathToReserve = _LPATH_ROOT . '/components/com_boss';
