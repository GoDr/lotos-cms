...
LHtml::loadJquery(false);
$mainframe->addJS(_LPATH_SITE . '/' . JADMIN_BASE . '/components/com_boss/js/function.js');

//Подключаем языковой файл 
if(file_exists($lang_file = _LPATH_ROOT . '/components/com_boss/lang/' . LCore::getCfg('lang') . '.php'))
	$lang_file = $lang_file;
else $lang_file = _LPATH_ROOT . '/components/com_boss/lang/russian.php';
...
<table class="adminlist mod_boss_admin_contents<?php echo $moduleclass_sfx ?>">
	<tr>
		<th <?php echo $colspan; ?> class="title"><?php echo $content_title ?>
			<small>( <a href="index2.php?option=com_boss&directory=<?php echo $directory ?>&act=contents<?php echo $cat_filter ?>"><?php echo $content_title_link ?></a> )</small>
		</th>
	...
	foreach($rows as $row){
        ...
        if(empty($row->userid)){
            $author = _GUEST;
        }elseif($acl->acl_check('administration', 'manage', 'users', $my->usertype, 'components', 'com_users')){
			// Ссылка на автора
            ...
            $author = htmlspecialchars($autors[$row->userid]->name, ENT_QUOTES);
        }
		...
		<tr class="row<?php echo $k; ?>">
			...
			<td class="td-state" align="center" onclick="boss_publ('img-pub-<?php echo $directory . $row->catid . $row->id; ?>', '<?php echo "act=contents&task=publish&tid=" . $row->id . "&directory=" . $directory; ?>');">
				<img id="img-pub-<?php echo $directory . $row->catid . $row->id;?>" class="img-mini-state" alt="<?php echo _PUBLISHING?>" src="<?php echo _LPATH_TPL_ADMI_S . '/' . TEMPLATE . '/images/ico/' . $img; ?>"/>
			</td>
			<?php if($display_author){ ?>
