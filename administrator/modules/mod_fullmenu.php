...
if(!defined('_JOS_FULLMENU_MODULE')){
	...
	class mosFullAdminMenu{
		...
		public static function show($usertype = ''){
			...
			if($config->config_adm_menu_cache){ // проверяем, активировано ли кеширование в панели управления
				...
				echo '<script src="' . _LPATH_SITE . '/cache/adm_menu_' . $menuname . '.js?r=' . $config->config_cache_key . '"></script>';
			...
			$installPlugins = $acl->acl_check('administration', 'install', 'users', $usertype, 'plugins', 'all');
			$editAllPlugins = $acl->acl_check('administration', 'edit', 'users', $usertype, 'plugins', 'all');
			...
			$cur_file_icons_path = _LPATH_TPL_ADMI_S . '/' . TEMPLATE . '/images/menu_ico/';
		...
		['<img src="<?php echo $cur_file_icons_path ?>preview.png"/>','<?php echo _BUTTON_LINK_IN_NEW_WINDOW ?>','<?php echo _LPATH_SITE; ?>/index.php','_blank','<?php echo _LPATH_SITE; ?>'],
		['<img src="<?php echo $cur_file_icons_path ?>preview.png"/>','<?php echo _MOD_FULLMENU_SITE_PREVIEW_IN_THIS_WINDOW ?>','index2.php?option=com_admin&task=preview',null,'<?php echo _LPATH_SITE; ?>'],
		['<img src="<?php echo $cur_file_icons_path ?>preview.png"/>','<?php echo _MOD_FULLMENU_SITE_PREVIEW_WITH_MODULE_POSITIONS ?>','index2.php?option=com_admin&task=preview2',null,'<?php echo _LPATH_SITE; ?>'],
		],
		...
		<?php
			...
			foreach($menuTypes as $menuType){
				?>['<img src="<?php echo $cur_file_icons_path ?>menus.png"/>','<?php echo $menuType['type']; ?>','index2.php?option=com_menus&menutype=<?php echo $menuType['id']; ?>',null,''],
			<?php
			}
			if($manageTrash){
				?>
		...
		<?php
			...
			($installPlugins | $editAllPlugins
			){
				?>
			[null,'<?php echo _PLUGINS ?>',null,null,'<?php echo _PLUGINS ?>',
			<?php if($editAllPlugins){ ?>
				['<img src="<?php echo $cur_file_icons_path ?>module.png"/>', '<?php echo _MOD_FULLMENU_SITE_PLUGINS ?>', "index2.php?option=com_plugins", null, '<?php echo _MOD_FULLMENU_SITE_PLUGINS ?>'],
				_cmSplit,
				['<img src="<?php echo $cur_file_icons_path ?>install.png"/>', '<?php echo _MOD_FULLMENU_PLUGINS_INSTALL_UNINSTALL ?>', 'index2.php?option=com_installer&element=plugin', null, '<?php echo _MOD_FULLMENU_PLUGINS_INSTALL_UNINSTALL ?>'],
				<?php } ?>
			],
			<?php
			}
		...
		<?php if($canConfig){ ?>
			['<img src="<?php echo $cur_file_icons_path ?>jfmanager.png"/>','<?php echo _MOD_FULLMENU_FILE_MANAGER ?>','index2.php?option=com_filemanager',null,'<?php echo _MOD_FULLMENU_FILE_MANAGER ?>'],
			...
			var myMenu =
				[
					...
					if($installPlugins | $editAllPlugins){
						?>
						_cmSplit,
						[null, '<?php echo _PLUGINS; ?>', null, null, '<?php echo $text; ?>'],
						<?php
					} // if ( $installPlugins | $editAllPlugins)
					/* Installer Sub-Menu*/
					if($installModules){
