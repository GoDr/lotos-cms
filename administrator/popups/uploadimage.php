...
$mainframe = new MainFrame($database, $option, _LPATH_ROOT, true);
...
if($directory != 'banners' && $directory != '' && !is_dir(_LPATH_ROOT . '/images/stories/' . $directory)){
	$directory = '';
}
...
if(isset($_FILES['userfile'])){
	if($directory == 'banners'){
		...
		if($directory != ''){
			...
			if(!is_dir(_LPATH_ROOT . '/images/stories/' . $directory)){
...
if($css != '' && !is_dir(_LPATH_ADMINISTRATOR . '/templates/' . $css . '/css/template_css.css')){
	$css = 'joostfree';
} else
	if($css == ''){
		$css = 'joostfree';
