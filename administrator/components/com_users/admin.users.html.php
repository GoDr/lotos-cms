...
class HTML_users{
		...
		<table class="adminlist">
			...
			for($i = 0, $n = $num; $i < $n; $i++){
				...
				<tr class="row<?php echo $k; ?>">
					...
					<td><?php echo LHtml::idBox($i, $row->id); ?></td>
					<td width="1%"><img width="25" class="miniavatar" src="<?php echo _LPATH_SITE . '/' . mosUser::get_avatar($row); ?>"/></td>
	...
	public static function edituser($row, $lists, $option, $uid, $params){
		...
		if(!defined('_JQUERY_LOADED')){
			...
			$mainframe->addJS(LCore::getCfg('live_site') . '/includes/js/jquery/jquery.js');
		}
		LHtml::loadOverlib();
		echo LHtml::loadJqueryPlugins('jquery.form', true, false);
	...
	<form action="index2.php" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
		...
		<table width="100%">
			<tr>
				...
				<td><?php echo LHtml::genderSelectList('gender', 'class="inputbox"', $row->user_extra->gender); ?> </td>
			</tr>
			<tr>
				<td class="key"><label><?php echo _C_USERS_B_DAY ?></label></td>
				<td>
					<?php echo LHtml::daySelectList('birthdate_day', 'class="inputbox"', $bday_date); ?>
					<?php echo LHtml::monthSelectList('birthdate_month', 'class="inputbox"', $bday_month, 1); ?>
					<?php echo LHtml::yearSelectList('birthdate_year', 'class="inputbox"', $bday_year); ?>
				</td>
			</tr>
			<tr>
