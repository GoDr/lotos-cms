...
//require_once (_LPATH_ROOT.'/components/com_users/users.config.php');
...
function showUsers($option){
	...
	$types[] = LHtml::makeOption('0', _com_users_SELECT_GROOP);
	$database->setQuery($query);
	$types = array_merge($types, $database->loadObjectList());
	$lists['type'] = LHtml::selectList($types, 'filter_type', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"', 'value', 'text', "$filter_type");

	// get list of Log Status for dropdown filter
	$logged[] = LHtml::makeOption(0, _com_users_SELECT_STATUS);
	$logged[] = LHtml::makeOption(1, _com_users_USER_LOGED);
	$lists['logged'] = LHtml::selectList($logged, 'filter_logged', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"', 'value', 'text', "$filter_logged");
...
function editUser($uid = '0', $option = 'users'){
	...
	if($my_group == 'super administrator' && $my->gid != 25){
		...
		$lists['gid'] = LHtml::selectList($gtree, 'gid', 'size="10"', 'value', 'text', $row->gid);
	}

	// build the html select list
	$lists['block'] = LHtml::yesnoRadioList('block', 'class="inputbox" size="1"', $row->block);
	// build the html select list
	$lists['sendEmail'] = LHtml::yesnoRadioList('sendEmail', 'class="inputbox" size="1"', $row->sendEmail);
...
function saveUser($task){
	...
	*/
	...
	if($isNew){
		...
		$message = sprintf(_NEW_USER_MESSAGE, $row->name, LCore::getCfg('sitename'), _LPATH_SITE, $row->username, $pwd);

		if(LCore::getCfg('mailfrom') != "" && LCore::getCfg('fromname') != ""){
			$adminName = LCore::getCfg('fromname');
			$adminEmail = LCore::getCfg('mailfrom');
		} else{
