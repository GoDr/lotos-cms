...
function editLink($id = 0){
	...
	$pathA = _LPATH_ROOT . '/includes/js/ThemeOffice';
	$pathL = _LPATH_SITE . '/includes/js/ThemeOffice';
	$images = array();
	$folders = array();
	$folders[] = LHtml::makeOption('/');

	$images['/'][] = LHtml::makeOption('spacer.png', ' --- ');
	ReadImages($pathA, '/', $folders, $images);

	$lists['image'] = GetImages($images, $pathL, $row);

	$options = array();
	$options[] = LHtml::makeOption('0', _FIRST_LEVEL);
...
function GetImages($images, $pathL, $row){
	...
	$getimages = LHtml::selectList($images['/'], 'admin_menu_img', 'class="inputbox" size="10" style="width:95%"' . $javascript, 'value', 'text', $admin_menu_img);
...
function categoryParentList($id, $action, $options = array()){
	...
	foreach($list as $item){
		if($this_treename){
			if($item->id != $cat->id && strpos($item->treename, $this_treename) === false){
				$options[] = LHtml::makeOption($item->id, $item->treename);
			}
		} else{
			if($item->id != $cat->id){
				$options[] = LHtml::makeOption($item->id, $item->treename);
	...
	$parent = LHtml::selectList($options, 'parent', 'class="inputbox" size="1" style="width:80%"', 'value', 'text', $cat->parent);
	return $parent;
}
