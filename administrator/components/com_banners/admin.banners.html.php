...
class HTML_banners{
	...
	function changeDisplayImage(msg) {
		if (document.adminForm.image_url.value != '') {
			...
			if (document.adminForm.image_url.value.indexOf('.swf') != -1) {
				...
				document.getElementById("flashDivText").innerHTML = '<br><?php echo _LPATH_SITE; ?>/index.php?option=com_banners&task=clk&id=' + bid;
				if (msg == 1) {
				...
				<td colspan="2" valign="top">
					...
					if($row->image_url != ''){
						if(preg_match("/.swf/", $row->image_url)){
							...
							$image_url = _LPATH_SITE . '/images/show/' . $row->image_url;
							$swfinfo = @getimagesize(_LPATH_ROOT . '/images/banners/' . $row->image_url);
							$result = "
		...
		<table width="100%" class="adminlist">
			...
			<tr>
				...
				<td colspan="2" align="left">
					<?php
					/*Hours Select*/
					echo LHtml::integerSelectList(0, 23, 1, '_publish_up_hour', 'size="1" onChange="checkPublish();" onClick="checkPublish();"', $row->publish_up_hour, "%02d");
					/*Minutes Select*/
					echo LHtml::integerSelectList(0, 55, 5, '_publish_up_minute', 'size="1" onChange="checkPublish();" onClick="checkPublish();"', $row->publish_up_minute, "%02d");
					?>
				</td>
			</tr>
			<tr>
				...
				<td colspan="2" align="left">
					<?php
					/*Hours Select*/
					echo LHtml::integerSelectList(0, 23, 1, '_publish_down_hour', 'size="1" onChange="checkPublish();" onClick="checkPublish();"', $row->publish_down_hour, "%02d");
					/*Minutes Select*/
					echo LHtml::integerSelectList(0, 55, 5, '_publish_down_minute', 'size="1" onChange="checkPublish();" onClick="checkPublish();"', $row->publish_down_minute, "%02d");
					?>
				</td>
			</tr>
	...
	<a href="<?php echo $link; ?>" title="<?php echo $text; ?>">
		<?php
        echo LAdminMenu::imageCheckAdmin($image, _LPATH_TPL_ADMI_S . '/' . TEMPLATE . '/images/system_ico/', null, null, $text);
