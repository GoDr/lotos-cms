...
function viewBanners($option){
	...
	$categories[] = LHtml::makeOption('-1', _ABP_ALLCAT);
	$categories = array_merge($categories, $banners_categories);
	$categorieslist = LHtml::selectList($categories, 'catid', 'class="inputbox" size="1" onchange="document.adminForm.submit();"', 'value', 'text', $catid);
	...
	$clients[] = LHtml::makeOption('-1', _ABP_ALLCLI);
	$clients = array_merge($clients, $banners_clients);
	$clientlist = LHtml::selectList($clients, 'cliid', 'class="inputbox" size="1" onchange="document.adminForm.submit();"', 'value', 'text', $cliid);
...
function editBanner($bannerid, $option){
	...
	// Build Client select list
	$clients[] = LHtml::makeOption('0', _ABP_SELECT_CLIENT);
	$clients = array_merge($clients, $banners_clients);
	$clientlist = LHtml::selectList($clients, 'cid', 'class="inputbox" size="1"', 'value', 'text', $banner->cid);
	...
	// Build Categories select list
	$categories[] = LHtml::makeOption('0', _ABP_SELECT_CATEGORY);
	$categories = array_merge($categories, $banners_categories);
	$categorieslist = LHtml::selectList($categories, 'tid', 'class="inputbox" size="1"', 'value', 'text', $banner->tid);

	// Imagelist
	// get list of images
	$dimension = array();
	$imgFiles = mosReadDirectory(_LPATH_ROOT . "/images/show");
	$images = array();
	$images[] = LHtml::makeOption('', _ABP_PSANIMG);
	foreach($imgFiles as $file){
		if(preg_match("/(\.bmp|\.gif|\.jpg|\.jpeg|\.png|\.swf)$/i", $file)){
			$images[] = LHtml::makeOption($file);
			// get image info
			$image_info = @getimagesize(_LPATH_ROOT . "/images/show/" . $file);
			$dimension[$file]['w'] = $image_info[0];
			$dimension[$file]['h'] = $image_info[1];
		}
	}

	$ilist = LHtml::selectList($images, 'image_url', "class=\"inputbox\" size=\"1\" onchange=\"changeDisplayImage(1);\"", 'value', 'text', $banner->image_url);
	...
	$glist = LHtml::selectList($groups, 'access', 'class="inputbox" size="1"', 'value', 'text', intval($banner->access));
...
function saveBanner($option, $task){
	...
	if($send_email == 'on'){
		...
		$link = _LPATH_SITE . '/index.php?option=com_banners&task=statistics&id=' . $banner->id . '&password=' . mosHash($banner->password);
		$result = mosMail(LCore::getCfg('mailfrom'), LCore::getCfg('fromname'), $client->email, _ABP_SUBJECT_MAIL, _ABP_BODY_MAIL . $link);
...
function viewBannerClients($option){
	...
	$states[] = LHtml::makeOption('-1', _ABP_ALLCLI);
	$states[] = LHtml::makeOption('1', _ABP_BANNERS_ATT);
	$states[] = LHtml::makeOption('0', _ABP_BANNERS_NO_PUB);

	$stateslist = LHtml::selectList($states, 'published', 'class="inputbox" size="1" onchange="document.adminForm.submit();"', 'value', 'text', $published);
...
function viewCategories($option){
	...
	// Build States select list
	$states[] = LHtml::makeOption('-1', _ABP_ALLCAT);
	$states[] = LHtml::makeOption('1', _ABP_BANNERS_ATT);
	$states[] = LHtml::makeOption('0', _ABP_BANNERS_NO_PUB);
...
function buildReccurTypeSelect($reccurtype, $args){
	$recur[] = LHtml::makeOption('0', _ABP_ALLDAYS);
	$recur[] = LHtml::makeOption('1', _ABP_EACHWEEK);
	$tosend = LHtml::selectList($recur, 'reccurtype', $args, 'value', 'text', $reccurtype);
	echo $tosend;
}
...
function doRestore($option){
	...
	require_once (_LPATH_ROOT . '/includes/domit/xml_domit_include.php');
