...
function showSummary($option, $task){
	...
	switch($field){
		case 'hits':
			$order_by = "hits $order";
			$sorts['b_agent'] = LHtml::sortIcon("$sort_base&tab=tab1", "agent");
			$sorts['b_hits'] = LHtml::sortIcon("$sort_base&tab=tab1", "hits", $order);
			$sorts['o_agent'] = LHtml::sortIcon("$sort_base&tab=tab2", "agent");
			$sorts['o_hits'] = LHtml::sortIcon("$sort_base&tab=tab2", "hits", $order);
			$sorts['d_agent'] = LHtml::sortIcon("$sort_base&tab=tab3", "agent");
			$sorts['d_hits'] = LHtml::sortIcon("$sort_base&tab=tab3", "hits", $order);
			break;

		case 'agent':
		default:
			$order_by = "agent $order";
			$sorts['b_agent'] = LHtml::sortIcon("$sort_base&tab=tab1", "agent", $order);
			$sorts['b_hits'] = LHtml::sortIcon("$sort_base&tab=tab1", "hits");
			$sorts['o_agent'] = LHtml::sortIcon("$sort_base&tab=tab2", "agent", $order);
			$sorts['o_hits'] = LHtml::sortIcon("$sort_base&tab=tab2", "hits");
			$sorts['d_agent'] = LHtml::sortIcon("$sort_base&tab=tab3", "agent", $order);
			$sorts['d_hits'] = LHtml::sortIcon("$sort_base&tab=tab3", "hits");
			break;
	}
...
function showSearches($option, $task, $showResults = null){
	...
	for($i = 0, $n = $total; $i < $n; $i++){
		...
		if($showResults){
			$results = $_PLUGINS->trigger('onSearch', array($rows[$i]->search_term));
