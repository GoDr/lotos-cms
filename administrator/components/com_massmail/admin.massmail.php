...
function messageForm($option){
	...
	$gtree = array(LHtml::makeOption(0, _ALL_USER_GROUPS));

	// get list of groups
	$lists = array();
	$gtree = array_merge($gtree, $acl->get_group_children_tree(null, 'USERS', false));
	$lists['gid'] = LHtml::selectList($gtree, 'mm_group', 'size="10"', 'value',
		'text', 0);
...
function sendMail(){
	...
	if(count($to['users']) || $gou === '0'){
		...
		$message_header = sprintf(_MASSMAIL_MESSAGE, html_entity_decode(LCore::getCfg('sitename'),
			ENT_QUOTES));
		$message = $message_header . $message_body;
		$subject = html_entity_decode(LCore::getCfg('sitename'), ENT_QUOTES) . ' / ' .
			stripslashes($subject);

		//Send email
		foreach($rows as $row){
			mosMail(LCore::getCfg('mailfrom'), LCore::getCfg('fromname'), $row->email, $subject, $message, $mode);
		}
	}
