...
class boss_show_user_menu{
	public static function editCategory($uid, $menutype, $menu, $directory){
		...
		$link = 'index.php?option=com_boss&task=show_user&directory=' . $directory;
		if($uid){
            Menus::checkOut($my->id, $uid);
		} else{
			$menu->type = 'boss_show_user';
		...
		boss_show_user_menu_html::editCategory($menu, $lists, $params);
	}
}
