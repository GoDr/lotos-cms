...
class boss_show_rules_menu{
	public static function editCategory($uid, $menutype, $menu, $directory){
		...
		$link = 'index.php?option=com_boss&task=show_rules&directory=' . $directory;
		if($uid){
            Menus::checkOut($my->id, $uid);
		} else{
			$menu->type = 'boss_show_rules';
		...
		boss_show_rules_menu_html::editCategory($menu, $lists, $params);
	}
}
