...
function viewFrontPage($option, $directory){
	...
	$query = "SELECT cc.id AS value, cc.name AS text"
		...
		. "\n INNER JOIN #__boss_" . $directory . "_categories AS cc ON cc.id = cch.category_id"
		. "\n WHERE c.frontpage = 1";
	$categories[] = LHtml::makeOption('0', _SEL_CATEGORY);
	$database->setQuery($query);
	$categories = array_merge($categories, $database->loadObjectList());
	$lists['catid'] = LHtml::selectList($categories, 'catid', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"', 'value', 'text', $catid);
	...
	$query = "SELECT c.userid, u.name"
		...
		. "\n GROUP BY u.name"
		. "\n ORDER BY u.name";
	$authors[] = LHtml::makeOption('0', _SEL_AUTHOR, 'userid', 'name');
	$database->setQuery($query);
	$authors = array_merge($authors, $database->loadObjectList());
	$lists['authorid'] = LHtml::selectList($authors, 'filter_authorid', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"', 'userid', 'name', $filter_authorid);
...
function settings($conf){
	...
	foreach($directories as $directory){
		$dirOptions[] = LHtml::makeOption($directory->id, $directory->name);
	}
	$directorylist = LHtml::selectList($dirOptions, 'directory', 'class="inputbox" size="1"', 'value', 'text', @$conf->directory);

	$pages = array();
	$pages[] = LHtml::makeOption('show_frontpage', _DIRECTORY_CONTENT);
	$pages[] = LHtml::makeOption('front', _DIRECTORY_CATEGORY);
	$pageslist = LHtml::selectList($pages, 'page', 'class="inputbox" size="1"', 'value', 'text', @$conf->page);

	// список сортировки
	$order = array();
	$order[] = LHtml::makeOption('frontpage', _SELECT_ORDER_1);
	$order[] = LHtml::makeOption('directory', _SELECT_ORDER_2);
	$orderlist = LHtml::selectList($order, 'order', 'class="inputbox" size="1"', 'value', 'text', @$conf->order);
