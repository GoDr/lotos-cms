...
require_once(_LPATH_ADMINISTRATOR . '/components/com_templates/admin.templates.class.php');
// XML library
...
if (LCore::getCfg('one_template') != '...') {
    echo joost_info(_INOGLOBAL_CONFIG_ONE_TEMPLATE_USING . ' <b>' . LCore::getCfg('one_template') . '</b>');
}
...
function viewTemplates($option, $client)
{
    ...
    if ($client == 'admin') {
        $templateBaseDir = mosPathName(_LPATH_TPL_ADMI);
    } else {
        $templateBaseDir = mosPathName(_LPATH_TPL_FRONT);
    }
...
function editTemplateSource($p_tname, $option, $client)
{
    ...
    if ($client == 'admin') {
        $file = _LPATH_ADMINISTRATOR . '/templates/' . $p_tname . '/index.php';
    } else {
        $file = _LPATH_ROOT . '/templates/' . $p_tname . '/index.php';
    }
...
function saveTemplateSource($option, $client, $task)
{
    ...
    if ($client == 'admin') {
        $file = _LPATH_ADMINISTRATOR . '/templates/' . $template . '/index.php';
    } else {
        $file = _LPATH_ROOT . '/templates/' . $template . '/index.php';
    }
...
function editTemplateCSS($p_tname, $option, $client)
{
    josSpoofCheck();
    if ($client == 'admin') {
        $file = _LPATH_ADMINISTRATOR . '/templates/' . $p_tname . '/css/template_css.css';
    } else {
        $file = _LPATH_ROOT . '/templates/' . $p_tname . '/css/template_css.css';
    }
...
function saveTemplateCSS($option, $client)
{
    ...
    if ($client == 'admin') {
        $file = _LPATH_ADMINISTRATOR . '/templates/' . $template . '/css/template_css.css';
    } else {
        $file = _LPATH_ROOT . '/templates/' . $template . '/css/template_css.css';
    }
...
/**
 * Привязка шаблона сайта к компонентам
 *
 * @param string $p_tname: имя шаблона
 * @param $option
 * @param $client
 */
function assignTemplate($p_tname, $option, $client)
{
    ...
    if ($p_tname) {
        $query = "SELECT * FROM #__modules_com WHERE moduleid = " . (int)$id;
        $database->setQuery($query);
        $lookup = $database->loadObjectList();
    }

    // Получаем HTML-список привязки
    $menulist = LAdminMenu::MenuLinks($lookup, 1, 0);

    // HTML форма
    // TODO Нет механизма сохранения в админке. Нет механизма получения нужного шаблона на фронте
