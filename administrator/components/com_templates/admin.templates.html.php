...
class HTML_templates{
		...
		<!--
			function showInfo(name, dir) {
				...
				if (document.adminForm.doPreview.checked) {
					var src = '<?php echo ($client == 'admin' ? _LPATH_TPL_ADMI : _LPATH_TPL_FRONT); ?>/'+dir+'/template_thumbnail.png';
			var html=name;
	...
	<form action="index2.php" method="post" name="adminForm">
		...
		<table class="adminlist">
			...
			for($i = 0, $n = $a; $i < $n; $i++){
				$row = &$rows[$i];
				if(LCore::getCfg('one_template') == $row->directory){
	...
	public static function editTemplateSource($template, &$content, $option, $client){
		$template_path = ($client == 'admin' ? _LPATH_TPL_ADMI : _LPATH_TPL_FRONT) . '/' . $template . '/index.php';
		...
		<table cellpadding="1" cellspacing="1" border="0" width="100%">
			<tr>
				<td width="290">
					<table class="adminheading">
						<tr>
							<th class="templates"><?php echo _TEMPLATE_EDITOR_HEADER;?></th>
						</tr>
					</table>
				</td>
	...
	public static function editCSSSource($template, &$content, $option, $client){
		$css_path = ($client == 'admin' ? _LPATH_TPL_ADMI : _LPATH_TPL_FRONT) . '/' . $template . '/css/template_css.css';
