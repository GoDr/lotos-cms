...
// при обращении к настройкам быстрых значков доступа почистим их кеш
...
function editIcon($id, $option){
	...
	$lists['ordering'] = LAdminMenu::SpecificOrdering($row, $id, $query, 1);
	...
	$lists['components'] = LAdminMenu::SpecificOrdering($row, $id = true, $query, 1); // id special handling
	...
	$lists['targets'] = LHtml::selectList($targets, 'tar_gets', 'id="tar_gets" class="inputbox" size="1"', 'value', 'text', null);
	...
	$lists['components_check'] = LHtml::selectList($ccheck, 'ccheck', 'id="ccheck" class="inputbox" size="1"', 'value', 'text', null);
	...
	if($my_group == 'siteowner'){
		...
		$lists['gid'] = LHtml::selectList($gtree, 'gid', 'class="inputbox" size="4"', 'value', 'text', $row->gid);
	}

	// display
	$display[] = LHtml::makeOption('', _DISPLAY_TEXT_AND_ICON);
	$display[] = LHtml::makeOption('1', _DISPLAY_ONLY_TEXT);
	$display[] = LHtml::makeOption('2', _DISPLAY_ONLY_ICON);

	$lists['display'] = LHtml::selectList($display, 'display', 'class="inputbox" size="1"', 'value', 'text', $row->display);
...
function saveIcon($redirect, $option){
	...
	// удаление пути
	$row->icon = str_replace(_LPATH_SITE, '', $row->icon);
...
function chooseIcon($option){
	...
	$icons = 0;
	$imgs = array();
	$folder[] = _LPATH_ADMINISTRATOR . '/images/';
	$folder[] = _LPATH_TPL_ADMI_S . '/' . $cur_template . '/images/cpanel_ico/';
