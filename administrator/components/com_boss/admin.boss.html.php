...
class HTML_boss
{
    ...
    public static function header($text, $directory, $directories, $conf)
    {
                ...
                <?php if ((empty($task) || $task == 'cancel') && !@$conf->allow_rights) { ?>
                    <th>
                        ...
                        foreach ($layouts as $key => $val) {
                            $options[] = LHtml::makeOption("index2.php?option=com_boss&directory=" . $directory . "&act=" . $act . "&layout=" . $key, $val);
                        }
                        $selected = (!empty($layout)) ? $layout : '';
                        echo LHtml::selectList($options, 'layout', 'id="layout" onchange="jumpmenu(\'parent\',this)"', 'value', 'text', $selected);
                        ?>
                    </th>
                <?php } ?>
            </tr>
        ...
        <?php
        if ($act != "contents" && $act != "plugins" && $act != "categories" && $act != "content_types") {
            echo '<br clear="all"/>';
        }
        if ($act == "categories" && $task == 'edit') {
            echo '<br clear="all"/>';
        }
        ?>
    ...
    public static function editConfiguration($row, $templates, $directory, $directories, $sort_fields, $filters, $ratings, $rights, $conf)
    {
        ...
        <script>
            ...
            function showimage() {
                ...
                document.images.preview.src = '<?php echo _LPATH_TPL_COM_S; ?>/com_boss/' + getSelectedValue('adminForm', 'template') + '/template_thumbnail.png';
            }
        ...
        <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
            ...
            <tr>
                <td><?php echo BOSS_CONTENTS_PER_PAGE_FRONT; ?></td>
                <td><input type="text" name="contents_per_page_front" value="<?php echo @$row->contents_per_page_front; ?>"/></td>
                <td><?php echo BOSS_CONTENTS_PER_PAGE_FRONT_LONG; ?></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <?php
                    $options = array();
                    $options[] = LHtml::makeOption(0, BOSS_DATE);
                    $options[] = LHtml::makeOption(1, BOSS_DATE_LAST_COMMENT);
                    foreach ($sort_fields as $s) {
                        $options[] = LHtml::makeOption($s->fieldid, $s->title);
                    }
                    $selected = (!empty($row->default_order_by)) ? $row->default_order_by : '';
                    echo LHtml::selectList($options, 'default_order_by', 'id="default_order_by"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    <?php
                    $options = array();
                    $options[] = LHtml::makeOption(2, "Backlink");
                    $options[] = LHtml::makeOption(1, BOSS_YES);
                    $options[] = LHtml::makeOption(0, BOSS_NO);
                    $selected = (!empty($row->auto_publish)) ? $row->auto_publish : '';
                    echo LHtml::selectList($options, 'auto_publish', 'id="auto_publish"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    <?php
                    $options = array();
                    $options[] = LHtml::makeOption(0, BOSS_SUBMISION_WITH_ACCOUNT_CREATION);
                    $options[] = LHtml::makeOption(1, BOSS_SUBMISSION_ALLOWED_ONLY_FOR_REGISTERS);
                    $options[] = LHtml::makeOption(2, BOSS_SUBMISSION_ALLOWED_FOR_VISITORS);
                    $selected = (!empty($row->submission_type)) ? $row->submission_type : '';
                    echo LHtml::selectList($options, 'submission_type', 'id="submission_type"', 'value', 'text', $selected);
                    ?>
                </td>
                <td>&nbsp;</td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'secure_new_content', 'id="secure_new_content"', 'value', 'text', $selected);
                    ?>
                </td>
                <td><?php echo BOSS_SECURE_NEW_CONTENT_LONG; ?></td>
            </tr>
            <tr>
                <td><?php echo BOSS_CONTENT_PLUGIN; ?></td>
                <td>
                    <?php
                    $options = array();
                    $options[] = LHtml::makeOption(1, BOSS_YES);
                    $options[] = LHtml::makeOption(0, BOSS_NO);
                    $selected = (!empty($row->use_content_plugin)) ? $row->use_content_plugin : '';
                    echo LHtml::selectList($options, 'use_content_plugin', 'id="use_content_plugin"', 'value', 'text', $selected);
                    ?>
                </td>
                <td><?php echo BOSS_CONTENT_PLUGIN_LONG; ?></td>
            </tr>
            <tr>
                ...
                <td>
                    <?php
                    $options = array();
                    $options[] = LHtml::makeOption(1, BOSS_ROOT_SUBMIT_ALLOWED);
                    $options[] = LHtml::makeOption(0, BOSS_ROOT_SUBMIT_NOT_ALLOWED);
                    $selected = (!empty($row->root_allowed)) ? $row->root_allowed : '';
                    echo LHtml::selectList($options, 'root_allowed', 'id="root_allowed"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    <?php
                    $options = array();
                    $options[] = LHtml::makeOption(0, BOSS_EMPTY_CAT_ALLOW);
                    $options[] = LHtml::makeOption(1, BOSS_EMPTY_CAT_NOT_ALLOW);
                    $selected = (!empty($row->empty_cat)) ? $row->empty_cat : '';
                    echo LHtml::selectList($options, 'empty_cat', 'id="empty_cat"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'send_email_on_new', 'id="send_email_on_new"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'send_email_on_update', 'id="send_email_on_update"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'show_rss', 'id="show_rss"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    ...
                    foreach ($filters as $filter) {
                        $options[] = LHtml::makeOption($filter->value, $filter->text);
                    }
                    $selected = (!empty($row->filter)) ? $row->filter : '';
                    echo LHtml::selectList($options, 'filter', 'id="filter"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    ...
                    foreach ($templates as $tpl) {
                        $options[] = LHtml::makeOption($tpl, $tpl);
                    }
                    $selected = (!empty($row->template)) ? $row->template : '';
                    echo LHtml::selectList($options, 'template', 'id="template" onchange="showimage()"', 'value', 'text', $selected);
                    $tmpl_tmp = (!empty($row->template)) ? $row->template : 'default';
                    ?>
                    <img class="ml10" src="<?php echo _LPATH_TPL_COM_S . "/com_boss/" . $tmpl_tmp . "/template_thumbnail.png"; ?>"  name="preview" border="1" alt="<?php echo $tmpl_tmp; ?>"/>
                </td>
                <td>
        ...
        <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'display_fullname', 'id="display_fullname"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'allow_attachement', 'id="allow_attachement"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'allow_contact_by_pms', 'id="allow_contact_by_pms"', 'value', 'text', $selected);
                    ?>
                </td>
        ...
        <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'allow_ratings', 'id="allow_ratings"', 'value', 'text', $selected);
                    ?>
                </td>
                <td>&nbsp;</td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    foreach ($ratings as $rating) {
                        $options[] = LHtml::makeOption($rating->value, $rating->text);
                    }
                    $selected = (!empty($row->rating)) ? $row->rating : '';
                    echo LHtml::selectList($options, 'rating', 'id="rating"', 'value', 'text', $selected);
                    ?>
                </td>
                <td>&nbsp;</td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'allow_comments', 'id="allow_comments"', 'value', 'text', $selected);
                    ?>
                </td>
                <td>&nbsp;</td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'secure_comment', 'id="secure_comment"', 'value', 'text', $selected);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    <?php
                    $options = array();
                    $options[] = LHtml::makeOption(1, BOSS_REVIEWS_SYS_IN);
                    //Если установлен jComments, то разрешаем его включить.
                    if (is_file(_LPATH_ROOT . "/components/com_jcomments/jcomments.php")) {
                        $options[] = LHtml::makeOption(0, BOSS_REVIEWS_SYS_OUT);
                    }
                    $selected = (!empty($row->comment_sys)) ? $row->comment_sys : '';
                    echo LHtml::selectList($options, 'comment_sys', 'id="comment_sys"', 'value', 'text', $selected);
                    ?>
                </td>
                <td><?php echo BOSS_REVIEWS_SYS; ?></td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'allow_unregisered_comment', 'id="allow_unregisered_comment"', 'value', 'text', $selected);
                    ?>
                </td>
                <td><?php echo BOSS_ALLOW_UNREG_COMMENTS; ?></td>
            </tr>
        ...
        <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'expiration', 'id="expiration"', 'value', 'text', $selected);
                    ?>
                </td>
                <td>&nbsp;</td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'recall', 'id="recall"', 'value', 'text', $selected);
                    ?>
                </td>
                <td>&nbsp;</td>
            </tr>
        ...
        <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
            <tr>
                ...
                <td>
                    ...
                    echo LHtml::selectList($options, 'allow_rights', 'id="allow_rights"', 'value', 'text', $selected);
                    ?>
                </td>
            </tr>
            <tr>
            ...
            foreach ($children[$id] as $row) {
                ...
                <tr class="row<?php echo($num & 1); ?>">
                    ...
                    <td align="center">
                        ...
                        if (file_exists($pict)) {
                            echo '<img src="' . _LPATH_SITE . '/images/boss/' . $directory . '/categories/' . $row->id . 'cat_t.jpg"/>';
                        } else {
                            echo '<img src="' . _LPATH_TPL_COM_S . '/com_boss/' . $template_name . '/images/default.png"/>';
                        }
                        ?>
                    </td>
                    ...
                    <td><?php $tmpl = $row->template ? $row->template : '';
                        echo $tmpl; ?></td>
        ...
        <form action="index2.php" method="get" name="adminForm">
            <div class="fr mb20">
                ...
                <select name="catid" id="catid" onchange="document.adminForm.submit();">
                    <option value="0" <?php if (!isset($cat)) {
                        echo selected;
            ...
            <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
                <tr>
                    ...
                    <th><?php echo BOSS_CONTENT_TYPES; ?></th>
                    <th><?php echo BOSS_AUTOR; ?></th>
                    <th><?php echo BOSS_TH_PUBLISH; ?></th>
                    <th><?php echo BOSS_TH_CATEGORY; ?></th>
                    <th><?php echo BOSS_TH_DATE; ?></th>
                </tr>
                <?php
                $k = 0;
                for ($i = 0; $i < count($rows); $i++) {
                    ...
                    <tr class="row<?php echo $k; ?>">
                        ...
                        <td align="right"><?php echo $row->id; ?></td>
                        <td>
                            <?php HTML_boss::displayLinkText($name, "index2.php?option=com_boss&act=contents&task=edit&&directory=" . $directory . "&tid[]=" . $row->id); ?>
                            <br>
                            <?php echo _LPATH_SITE; ?>/index.php?option=com_boss&amp;task=show_content&amp;catid=<?php echo HTML_boss::getIdCategory($categs, $row->id); ?>&amp;contentid=<?php echo $row->id; ?>&amp;directory=<?php echo $directory; ?>
                        </td>
    ...
    public static function getIdCategory($categs, $content_id)
    {
            ...
            <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
                <tr>
                    ...
                    <td colspan="2">
                        <?php
                        $options = array();
                        $options[] = LHtml::makeOption(1, BOSS_PUBLISH);
                        $options[] = LHtml::makeOption(0, BOSS_NO_PUBLISH);
                        $selected = (!empty($row->published)) ? $row->published : '';
                        echo LHtml::selectList($options, 'published', 'id="published"', 'value', 'text', $selected);
                        ?>
                        &nbsp;&nbsp;<?php echo BOSS_FROM ?>&nbsp;
                ...
                <tr>
                    ...
                    <td>
                        ...
                        echo LHtml::selectList($options, 'frontpage', 'id="frontpage"', 'value', 'text', $selected);
                        ?>
                    </td>
                    <td>&nbsp;</td>
                <tr>
                <tr>
                    ...
                    <td>
                        ...
                        echo LHtml::selectList($options, 'featured', 'id="featured"', 'value', 'text', $selected);
                        ?>
                    </td>
                    <td>&nbsp;</td>
                <tr>
    ...
    public static function displayCategory($row, $cats, $directory, $directories, $templates, $comtentTypes, $rights, $conf)
    {
        ...
        <form action="index2.php" method="post" name="adminForm" id="adminForm" class="adminForm"
            ...
            <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
                ...
                <tr>
                    <td><?php echo BOSS_SUBCAT; ?></td>
                    <td>
                        <?php echo LHtml::yesnoSelectList('subcat', 'id="subcat"', $row->subcat); ?>
                        <?php echo LHtml::toolTip('', BOSS_SUBCAT_FULL); ?>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php
                        $a_pic = _LPATH_ROOT . "/images/boss/$directory/categories/" . @$row->id . "cat.jpg";
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php echo mosToolTip(BOSS_CATEGORY_CONTENT_TYPE); ?>
                    </td>
                </tr>
                <tr>
                    <td><?php echo BOSS_TEMPLATE; ?></td>
                    <td>
                        <?php
                        $options = array();
                        $options[] = LHtml::makeOption(0, BOSS_TEMPLATE_SELECT);
                        foreach ($templates as $tpl) {
                            $options[] = LHtml::makeOption($tpl, $tpl);
                        }
                        $selected = (!empty($row->template)) ? $row->template : '';
                        ...
                        <img class="ml10" src="<?php echo _LPATH_TPL_COM_S . "/com_boss/" . $tmpl_tmp . "/template_thumbnail.png"; ?>"
    ...
    public static function showField($row, $directory, $plugin, $tpl = array())
    {
        ...
        <li style="display: block;" id="<?php echo $row->fieldid; ?>_li">
            ...
            <div class="block" id="div_field_<?php echo $row->fieldid; ?>">
                ...
                <img class="field_img" src="<?php echo _LPATH_SITE . $plugin->getFieldIcon(); ?>" alt="<?php echo $plugin->name; ?>"/>
                ...
                <ul id="form_builder_toolbox">
                    ...
                    foreach ($plugins as $key => $plugin) {
                        ...
                        if (method_exists($plugin, 'getFieldIcon')) {
                            ...
                            if (is_file(_LPATH_ROOT . $icon)) {
                                $style = 'style="background-image:url(' . _LPATH_SITE . $icon . ');"';
                            }
                        }
    ...
    public static function editfield($row, $lists, $plug, $types, $directory, $directories, $task, $fnames)
    {
        ...
        <form action="index2.php?option=com_boss" method="POST" name="fieldForm" id="fieldForm">
            ...
            <table class="adminform">
                ...
                <tr>
                    <td width="45%"><?php echo BOSS_FIELD_ACCESS; ?></td>
                    <td width="45%"><?php echo $lists['access']; ?></td>
                    <td><?php echo boss_helpers::bossToolTip(BOSS_FIELD_ACCESS_TIP); ?></td>
                </tr>
            </table>
            <?php
            $tabs->endTab();
                ...
                if (isset($templates)) {
                    ...
                    foreach ($templates as $tpl) {
                        ...
                        <tr class="row<?php echo $k; ?>">
                            ...
                            <td><?php echo $tpl; ?></td>
                            <td align="center">
                                <img src="<?php echo _LPATH_TPL_COM_S . "/com_boss/" . $tpl . "/template_thumbnail.png"; ?>"
                                     border="1"/>
                            </td>
                            <td align="center">
                                <a href="<?php echo _LPATH_SITE; ?>/administrator/index2.php?option=com_boss&directory=<?php echo $directory; ?>&act=templates&task=edit_tmpl&template=<?php echo $tpl; ?>&type_tmpl=category">
                                    <img src="<?php echo _LPATH_TPL_COM_S . "/com_boss/" . $tpl . "/template_category.png"; ?>" border="1"
                                         title="<?php echo BOSS_EDIT_CAT_TMPL; ?>"/>
                                </a>
                            </td>
                            <td align="center">
                                <a href="<?php echo _LPATH_SITE; ?>/administrator/index2.php?option=com_boss&directory=<?php echo $directory; ?>&act=templates&task=edit_tmpl&template=<?php echo $tpl; ?>&type_tmpl=content">
                                    <img src="<?php echo _LPATH_TPL_COM_S . "/com_boss/" . $tpl . "/template_content.png"; ?>" border="1"
                                         title="<?php echo BOSS_EDIT_CONTENT_TMPL; ?>"/>
                                </a>
                            </td>
                            <td align="center">
                                <a href="<?php echo _LPATH_SITE; ?>/administrator/index2.php?option=com_boss&directory=<?php echo $directory; ?>&act=templates&task=list_tmpl_fields&template=<?php echo $tpl; ?>">
                                    <img src="<?php echo _LPATH_SITE; ?>/administrator/components/com_boss/images/16x16/template_fields.png"
                                         title="<?php echo BOSS_EDIT_TMPL_FIELDS; ?>"/>
                                </a>
                            </td>
                            <td align="center">
                                <a href="<?php echo _LPATH_SITE; ?>/administrator/index2.php?option=com_boss&directory=<?php echo $directory; ?>&act=templates&task=edit_tmpl_source&source_file=&template=<?php echo $tpl; ?>">
                                    <img src="<?php echo _LPATH_SITE; ?>/administrator/components/com_boss/images/16x16/code.png"
    ...
    public static function editTemplate($directory, $directories, $template, $type_tmpl, $positions, $groupfields, $fields, $cats, $conf)
    {
        ...
        <form action="index2.php" method="post" name="adminForm">
            ...
            <div style="text-align: center;">
                <img src="<?php echo _LPATH_TPL_COM_S . "/com_boss/$template/$img"; ?>"/>
            </div>
            <input type="hidden" name="option" value="com_boss"/>
            <input type="hidden" name="act" value="templates"/>
            ...
            <table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        echo LHtml::selectList($options, 'template', 'id="template"', 'value', 'text', $selected);
                        ?>
                    </td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    ...
                    <td>
                        <?php
                        $options = array();
                        $options[] = LHtml::makeOption("content", BOSS_TH_CONTENT_TMPL);
                        $options[] = LHtml::makeOption("category", BOSS_TH_CAT_TMPL);
                        $selected = (!empty($field->type_tmpl)) ? $field->type_tmpl : '';
                        echo LHtml::selectList($options, 'type_tmpl', 'id="type_tmpl"', 'value', 'text', $selected);
                        ?>
                    </td>
                    <td>&nbsp;</td>
                </tr>
    ...
    public static function editTmplSource($directory, $directories, $template, $source_file, $files, $source, $conf)
    {
        HTML_boss::header(BOSS_EDIT_TEMPLATE_FIELD, $directory, $directories, $conf);
        ?>
        <form action="index2.php" method="post" name="adminForm">
    ...
    public static function listPlugins($directory, $directories, $plugins, $used, $conf)
    {
        ...
        <form action="index2.php" method="post" name="filterForm">
            <div class="fr mb20">
                ...
                <?php
                $options = array();
                $options[] = LHtml::makeOption('', BOSS_ALL);
                $options[] = LHtml::makeOption(0, BOSS_NOTUSED);
                $options[] = LHtml::makeOption(1, BOSS_USED);
                $selected = (!empty($used)) ? $used : '';
                echo LHtml::selectList($options, 'used', 'id="used" onchange="document.filterForm.submit();"', 'value', 'text', $selected);
                ?>
                            ...
                            <td>
                                <?php if ($plugin['folder'] != 'fields') {
                                    ...
                                    if (method_exists($bossPlugin, 'displaySettingsForm'))
                                        echo '<a href="' . _LPATH_SITE . '/administrator/index2.php?option=com_boss&act=plugins&task=edit&directory=' . $directory . '&folder=' . $plugin['folder'] . '&plugin=' . $plugin['file'] . '">' . BOSS_EDIT . '</a>';
                                }
                                ?>
                            </td>
                        </tr>
                ...
                if (isset($fieldimages)) {
                    ...
                    foreach ($fieldimages as $fieldimage) {
                        ...
                        <tr class="row<?php echo $k; ?>">
                            ...
                            <td>
                                <img src="<?php echo _LPATH_SITE . "/images/boss/$directory/fields/$fieldimage"; ?>"
                                     border="1"/>
                            </td>
                            <td>
                                <?php echo $fieldimage; ?>
    ...
    public static function showImpExpForm($directory, $directories, $packs, $conf)
    {
        ...
        <table width="100%">
            <tr>
                <td width="50%">
                    <fieldset>
                        <legend><?php echo BOSS_EX_HEADER; ?></legend>
                                                ...
                                                <div id="<?php echo $id; ?>">
                                                    <table>
                                                        <tr>
                                                            ...
                                                            <td><a href="<?php echo _LPATH_SITE; ?>/images/boss/<?php echo $directory . '/' . $pack; ?>"><?php echo BOSS_DOWNLOAD_FILE . " " . $pack; ?></a></td>
            ...
            <tr>
                <td width="50%">
                    <br/>
                    <fieldset>
                        <legend><?php echo BOSS_IM_HEADER; ?></legend>
