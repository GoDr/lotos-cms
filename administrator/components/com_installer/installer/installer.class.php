...
class mosInstaller{
	...
	function downloadPackage($url, $target = false){
		$base_Dir = mosPathName(_LPATH_ROOT . DS . 'media');
	...
	function extractArchive(){
		...
		if(preg_match('/.zip$/', $archivename)){
			// Extract functions
			require_once (_LPATH_ADMINISTRATOR . '/includes/pcl/pclzip.lib.php');
			require_once (_LPATH_ADMINISTRATOR . '/includes/pcl/pclerror.lib.php');
	...
	function parseFiles($tagName = 'files', $special = '', $specialError = '', $adminFiles = 0){
		...
		if($tagName == 'media'){
			$installTo = mosPathName(_LPATH_ROOT . '/images/stories');
		} elseif($tagName == 'languages'){
			foreach($files as $file){
				$lang_path = $file->getAttribute('folder');
				$installTo = mosPathName(_LPATH_ROOT . DS . $lang_path);
				$fil = $file->getText();
	 ...
	 * Used by Components/Modules/Plugin Installer
	 * @return boolean True on success, False on error
	 */
...
function cleanupInstall($userfile_name, $resultdir){
	if(file_exists($resultdir)){
		deldir($resultdir);
		unlink(mosPathName(_LPATH_ROOT . '/media/' . $userfile_name, false));
	}
}
