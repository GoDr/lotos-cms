...
class mosInstallerModule extends mosInstaller{
	...
	function install($p_fromdir = null){
		...
		$this->elementDir(mosPathName(_LPATH_ROOT . ($client == 'admin' ? '/' . JADMIN_BASE : '') . '/modules/'));
		...
		if(!$id){
			...
			$this->setError(1, _PLUGIN . ' "' . $this->elementName() . '" ' . _ALREADY_EXISTS);
			return false;
		}
	...
	function uninstall($id, $option, $client = 0){
		...
		if(count($modules)){
			...
			if(!$row->client_id){
				$basepath = _LPATH_ROOT . '/modules/';
			} else{
				$basepath = _LPATH_ADMINISTRATOR . '/modules/';
			}
