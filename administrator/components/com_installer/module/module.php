...
function showInstalledModules($_option){
	...
	$select[] = LHtml::makeOption('', _ALL);
	$select[] = LHtml::makeOption('0', _SITE_MODULES);
	$select[] = LHtml::makeOption('1', _ADMIN_MODULES);
	$lists['filter'] = LHtml::selectList($select, 'filter', 'class="inputbox" size="1" onchange="document.adminForm.submit();"', 'value', 'text', $filter);
	if($filter == null){
		$and = '';
	} else
		if(!$filter){
	...
	for($i = 0; $i < $n; $i++){
		...
		if($row->client_id == "1"){
			$moduleBaseDir = mosPathName(mosPathName(_LPATH_ROOT) . JADMIN_BASE . DS . 'modules');
		} else{
			$moduleBaseDir = mosPathName(mosPathName(_LPATH_ROOT) . 'modules');
		}
