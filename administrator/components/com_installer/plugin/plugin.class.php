 ...
 */
class mosInstallerPlugin extends mosInstaller{
	...
	function install($p_fromdir = null){
		...
		$this->elementDir(mosPathName(_LPATH_ROOT . DS . 'plugins' . DS . $folder));
		...
		if($this->parseFiles('files', 'plugin', _NO_FILES_OF_PLUGINS) === false){
		...
		// Insert plugin in DB
		$query = "SELECT id FROM #__plugins WHERE element = " . $database->Quote($this->elementName());
		...
		if(!$id){
			...
			$this->setError(1, sprintf(_COM_INSTALLER_PLUGIN_EXIST, $this->elementName()));
			return false;
		}
	...
	function uninstall($id, $option, $client = 0){
		...
		$id = intval($id);
		$query = "SELECT name, folder, element, iscore FROM #__plugins WHERE id = " . (int)
		$id;
		$database->setQuery($query);
		...
		if($database->getErrorNum()){
			HTML_installer::showInstallMessage($database->stderr(), _UNINSTALL_ERROR, $this->returnTo($option, 'plugin', $client));
			exit();
		}
		if($row == null){
			HTML_installer::showInstallMessage(_WRONG_ID, _UNINSTALL_ERROR, $this->returnTo($option, 'plugin', $client));
			exit();
		}

		if(trim($row->folder) == ''){
			HTML_installer::showInstallMessage(_BAD_DIR_NAME_EMPTY, _UNINSTALL_ERROR, $this->returnTo($option, 'plugin', $client));
			exit();
		}

		$basepath = _LPATH_ROOT . DS . 'plugins' . DS . $row->folder . DS;
		...
		if($row->iscore){
			HTML_installer::showInstallMessage($row->name . ' - ' . _IS_PART_OF_CMS, _UNINSTALL_ERROR, $this->returnTo($option, 'plugin', $client));
			exit();
		}

		$query = "DELETE FROM #__plugins WHERE id = " . (int)$id;
		$database->setQuery($query);
		if(!$database->query()){
			$msg = $database->stderr;
			die($msg);
