...
class mosInstallerTemplate extends mosInstaller{
	...
	function install($p_fromdir = null){
		...
		$this->elementDir(mosPathName(_LPATH_ROOT . ($client == 'admin' ? DS . JADMIN_BASE : '') . DS . 'templates' . DS . strtolower(str_replace(" ", "_", $this->elementName()))));
		$base = mosPathName(_LPATH_ROOT . ($client == 'admin' ? '/' . JADMIN_BASE : '') . '/templates');
	...
	function uninstall($id, $option, $client = 0){
		...
		// Delete directories
		$path = _LPATH_ROOT . ($client == 'admin' ? '/' . JADMIN_BASE : '') . '/templates/' . $id;
	...
	function cleanAfterError(){
		...
		josSpoofCheck(null, null, 'request');
		// Delete directories
		$path = _LPATH_ROOT . ($client == 'admin' ? '/' . JADMIN_BASE : '') . '/templates/' . $this->elementName();
