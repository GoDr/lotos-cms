...
class HTML_templates{
		...
		<!--
			function showInfo(name, dir) {
				...
				if (document.adminForm.doPreview.checked) {
					var src = '<?php echo _LPATH_SITE . ($client == 'admin' ? '/' . JADMIN_BASE : ''); ?>/templates/'+dir+'/template_thumbnail.png';
			var html=name;
	...
	<form action="index2.php" method="post" name="adminForm">
		<table class="adminheading">
			<tr>
				<th class="templates">
					<?php echo _TEMPLATES?>
					<small>
						<small>[ <?php echo $client == 'admin' ? _CONTROL_PANEL : _SITE; ?> ]</small>
					</small>
				</th>
		...
		<table class="adminlist">
			...
			for($i = 0, $n = $_n; $i < $n; $i++){
				...
				<tr class="row<?php echo $k; ?>">
					...
					<td align="left">
						<a onmouseover="showInfo('<?php echo $row->name; ?>','<?php echo $row->directory; ?>')" onmouseout="return nd();"><?php echo $row->name; ?></a>
					</td>
					<?php
					if($client == 'admin'){
						?>
		...
		<input type="hidden" name="element" value="template"/>
	...
	function editTemplateSource($template, &$content, $option, $client){
		$template_path = _LPATH_ROOT . ($client == 'admin' ? '/' . JADMIN_BASE : '') . '/templates/' . $template . '/index.php';
		/* подключение js файла codepress*/
		if(LCore::getCfg('codepress')) LHtml::loadCodepress();
		?>
	<script>

		function ch_apply() {
			SRAX.get('tb-apply').className = 'tb-load';
			<?php if(LCore::getCfg('codepress')) echo 'document.adminForm.filecontent.value=codearea.getCode();';?>
			dax({
	...
	<form action="index2.php" method="post" name="adminForm" id="adminForm" <?php if(LCore::getCfg('codepress')) echo 'onsubmit="document.adminForm.filecontent.value=codearea.getCode();document.adminForm.submit();"';?>>
		<table cellpadding="1" cellspacing="1" border="0" width="100%">
			<tr>
				<td width="290">
					<table class="adminheading">
						<tr>
							<th class="templates">HTML-редактор шаблона</th>
						</tr>
					</table>
				</td>
		...
		<table class="adminform">
			...
			;?>
			<tr>
				<td><textarea style="width:100%;height:600px" cols="130" rows="35" name="filecontent"><?php echo $content; ?></textarea></td>
			</tr>
			<?php };?>
		</table>
	...
	function editCSSSource($template, &$content, $option, $client){
		$css_path = _LPATH_ROOT . ($client == 'admin' ? '/' . JADMIN_BASE : '') . '/templates/' . $template . '/css/template_css.css';
		/* подключение js файла codepress*/
		if(LCore::getCfg('codepress')) LHtml::loadCodepress();
		?>
	<script>
		...
		<table cellpadding="1" cellspacing="1" border="0" width="100%">
			<tr>
				<td width="280">
					<table class="adminheading">
						<tr>
							<th class="templates"><?php echo _CSS_TEMPLATE_EDITOR?></th>
						</tr>
					</table>
				</td>
