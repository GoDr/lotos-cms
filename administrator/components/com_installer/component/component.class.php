...
class mosInstallerComponent extends mosInstaller{
	...
	function install($p_fromdir = null){
		...
		$this->elementDir(mosPathName(_LPATH_ROOT . DS . 'components' . DS . strtolower("com_" . str_replace(" ", "", $this->elementName())) . DS));
		$this->componentAdminDir(mosPathName(_LPATH_ADMINISTRATOR . DS . 'components' . DS . strtolower('com_' . str_replace(' ', '', $this->elementName()))));
	...
	public function uninstall($cid, $option, $client = 0){
		...
		$filesindir = mosReadDirectory(mosPathName(_LPATH_ADMINISTRATOR . DS . 'components' . DS . $row->option), '.xml$');
		if(count($filesindir) > 0){
			...
			foreach($filesindir as $file){
				...
				if(!$xmlDoc->loadXML(_LPATH_ADMINISTRATOR . DS . 'components' . DS . $row->option . DS . $file, false, true)){
					return false;
				}
				...
				if(!is_null($uninstallfile_elemet)){
					...
					if(!is_null($uninstall_file) && file_exists(_LPATH_ADMINISTRATOR . DS . 'components' . DS . $row->option . DS . $uninstall_file)){
						require_once (_LPATH_ADMINISTRATOR . DS . 'components' . DS . $row->option . DS . $uninstall_file);
		...
		if(trim($row->option)){
			$result = 0;
			$path = mosPathName(_LPATH_ADMINISTRATOR . DS . 'components' . DS . $row->option);
			if(is_dir($path)){
				$result |= deldir($path);
			}
			$path = mosPathName(_LPATH_ROOT . DS . 'components' . DS . $row->option);
			if(is_dir($path)){
				$result |= deldir($path);
			}
			return $result;
	...
	function cleanAfterError(){
		...
		josSpoofCheck();
		$basepath = mosPathName(_LPATH_ROOT . DS . 'components' . DS . strtolower("com_" . str_replace(" ", "", $this->elementName())));
		$adminpath = mosPathName(_LPATH_ADMINISTRATOR . DS . 'components' . DS . strtolower("com_" . str_replace(" ", "", $this->elementName())));
		;
			...
			if($files_element->hasChildNodes()){
				...
				if(count($files) != 0){
					foreach($files as $file){
						if($adminFiles == 1){
							if(file_exists(_LPATH_ADMINISTRATOR . DS . $file->getText())){
								unlink(_LPATH_ADMINISTRATOR . DS . $file->getText());
							}
						} else{
							if(file_exists(_LPATH_ROOT . DS . $file->getText())){
								unlink(_LPATH_ROOT . DS . $file->getText());
							}
						}
					}
				}
