...
function x_apply(){
	...
	$msg = sprintf(_COM_PLUGINS_APPLY, $row->name);
	return $msg;
}
...
function x_publish($id = null){
	...
	$query = "SELECT published FROM #__plugins WHERE id = " . (int)$id;
	...
	$query = "UPDATE #__plugins SET published = " . (int)$state . " WHERE id = " . $id . " " . "\n AND ( checked_out = 0 OR ( checked_out = " . (int)$my->id . " ) )";
	$database->setQuery($query);
	if(!$database->query()){
		return 'error-db';
	} else{
