...
class HTML_modules{
	 ...
	 */
	public static function showPlugins($rows, $client, $pageNav, $lists, $search){
        $my = LCore::getUser();
	...
	<form action="index2.php" method="post" name="adminForm">
		<table class="adminheading">
			<tr>
				<th class="plugins"><?php echo _PLUGINS?></th>
		...
		<table class="adminlist">
			<tr>
				...
				<th class="title"><?php echo _PLUGIN_NAME?></th>
			...
			for($i = 0, $n = $_c; $i < $n; $i++){
				$row = &$rows[$i];
				mosMakeHtmlSafe($row);
				$link = 'index2.php?option=com_plugins&client=' . $client . '&task=editA&hidemainmenu=1&id=' . $row['id'];
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="center" <?php echo ($row['checked_out'] && ($row['checked_out'] != $my->id)) ? null : 'onclick="ch_publ(' . $row['id'] . ',\'com_plugins\');" class="td-state"';?>>
	 ...
	 */
	public static function editPlugin($row, $lists, $params){
		LHtml::loadOverlib();
		$row->nameA = '';
		if($row->id){
	...
	<script>
		...
		function submitbutton(pressbutton) {
			...
			if (form.name.value == "") {
				alert("<?php echo _NO_PLUGIN_NAME?>");
			} else if (form.element.value == "") {
				alert("<?php echo _NO_PLUGIN_FILENAME?>");
			} else {
				submitform(pressbutton);
			}
		}
	...
	<table class="adminheading">
		<tr>
			<th class="plugins"><?php echo _SITE_PLUGIN?>:
				<small><?php echo $row->id ? _EDITING : _CREATION; ?></small><?php echo $row->nameA; ?></th>
		</tr>
	</table>
			...
			<tr valign="top">
				<td width="60%" valign="top">
					<table class="adminform">
						<tr>
							<th colspan="2"><?php echo _PLUGIN_DETAILS?></th>
						<tr>
						<tr>
						...
						<tr>
							<td valign="top" class="key"><?php echo _USE_THIS_PLUGIN_FILE?>:</td>
							<td><input class="text_area" type="text" name="element" size="35" value="<?php echo $row->element; ?>"/>.php</td>
						</tr>
						<tr>
							<td valign="top" class="key"><?php echo _PLUGIN_ORDER?>:</td>
