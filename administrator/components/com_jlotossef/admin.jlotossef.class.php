...
class JLotosSefClass
{
     ...
     *
     * @modification 07.02.2014 Gold Dragon
     */
    private static function defaultConfig(){
        // Делать резервную копию таблицы ссылок перед очисткой
        //  0 - Нет
        //  1 - Да. Перезаписывать файл
        //  2 - Да. Создавать новый файл
        self::$config['pack_ref'] = '2';
        // Делать резервную копию таблицы дубликатов перед очисткой
        //  0 - Нет
        //  1 - Да. Перезаписывать файл
        //  2 - Да. Создавать новый файл
        self::$config['pack_dup'] = '1';
        // путь до архивных файлов
        self::$config['pack_path'] = '/images/backups/';
        // префикс архивных фалов
        self::$config['pack_pref'] = 'sef_';
        // Формат файла
        //  0 : Текстовый файл
        //  1 : SQL INSERTs
        //  2 : SQL REPLACEs
    ...
    public static function saveDup()
    {
        ...
        $row_ref['url'] = str_replace(_LPATH_SITE, '', LCore::getParam($_POST, 'ref_url', '', 'sn'));
        $row_ref['sef'] = str_replace(_LPATH_SITE, '', LCore::getParam($_POST, 'ref_sef', '', 'sn'));

        $row_dup['url'] = str_replace(_LPATH_SITE, '', LCore::getParam($_POST, 'dup_url', '', 'sn'));
        $row_dup['sef'] = str_replace(_LPATH_SITE, '', LCore::getParam($_POST, 'dup_sef', '', 'sn'));
        if (empty($row_dup['sef']) or $row_dup['sef'] == '/') {
            ...
            $row_dup['id'] = LCore::getParam($_POST, 'id', 0, 'i');
    ...
    public static function saveRef()
    {
        ...
        $row['url'] = str_replace(_LPATH_SITE, '', LCore::getParam($_POST, 'url', '', 'sn'));
        if ($row['url'][0] != '/') {
            $row['url'] = '/' . $row['url'];
        }
        // нормализуем ссылку
        ...
        $row['sef'] = str_replace(_LPATH_SITE, '', LCore::getParam($_POST, 'sef', '', 'sn'));
        if (empty($row['sef']) or $row['sef'] == '/') {
            ...
            $row['id'] = LCore::getParam($_POST, 'id', 0, 'i');
    ...
    {
        if (empty(self::$lang)) {
            self::$lang = LLang::getLang('com.jlotossef');
        }
        return self::$lang;
    }
    ...
    public static function getCfg($key = null)
    {
        if (empty(self::$config)) {
            ...
            // Если база пуста, загружает значения по умолчанию
            if(empty($rows)){
                self::defaultConfig();
            }
    ...
    public static function pageConfiguration()
    {
        ...
        $_tmp_array = array();
        $_tmp_array[] = LHtml::makeOption('0', $lang['JLSEF_CFG_PACK_REF_VAL0']);
        $_tmp_array[] = LHtml::makeOption('1', $lang['JLSEF_CFG_PACK_REF_VAL1']);
        $_tmp_array[] = LHtml::makeOption('2', $lang['JLSEF_CFG_PACK_REF_VAL2']);
        $data['pack_ref'] = LHtml::selectList($_tmp_array, 'pack_ref', 'class="inputbox" size="1"', 'value', 'text', $_config['pack_ref']);

        // Создавать резервную копию таблицы дубликатов перед очисткой
        $_tmp_array = array();
        $_tmp_array[] = LHtml::makeOption('0', $lang['JLSEF_CFG_PACK_DUP_VAL0']);
        $_tmp_array[] = LHtml::makeOption('1', $lang['JLSEF_CFG_PACK_DUP_VAL1']);
        $_tmp_array[] = LHtml::makeOption('2', $lang['JLSEF_CFG_PACK_DUP_VAL2']);
        $data['pack_dup'] = LHtml::selectList($_tmp_array, 'pack_dup', 'class="inputbox" size="1"', 'value', 'text', $_config['pack_dup']);
        ...
        $_tmp_array = array();
        $_tmp_array[] = LHtml::makeOption('0', $lang['JLSEF_CFG_PACK_FORMAT_VAL0']);
        $_tmp_array[] = LHtml::makeOption('1', $lang['JLSEF_CFG_PACK_FORMAT_VAL1']);
        $_tmp_array[] = LHtml::makeOption('2', $lang['JLSEF_CFG_PACK_FORMAT_VAL2']);
        $_tmp_array[] = LHtml::makeOption('3', $lang['JLSEF_CFG_PACK_FORMAT_VAL3']);
        $_tmp_array[] = LHtml::makeOption('4', $lang['JLSEF_CFG_PACK_FORMAT_VAL4']);
        $data['pack_format'] = LHtml::selectList($_tmp_array, 'pack_format', 'class="inputbox" size="1"', 'value', 'text', $_config['pack_format']);
    ...
    public static function exportLink($indx, $redirect = true)
    {
        ...
        if (($indx == 0 and self::getCfg('pack_ref')) OR ($indx == 1 and self::getCfg('pack_dup'))) {
            $table = ($indx) ? LCore::getCfg('dbprefix') . 'sef_duplicate' : LCore::getCfg('dbprefix') . 'sef_link';

            $_path = _LPATH_ROOT . self::getCfg('pack_path');
            $_db = LCore::getDB();
            ...
            $fp = fopen(_LPATH_ROOT . self::getCfg('pack_path') . $filename, 'w');
    ...
    public static function saveConfiguration($indx = false)
    {
        ...
        $rows['pack_ref'] = LCore::getParam($_POST, 'pack_ref', 0, 'i');
        $rows['pack_dup'] = LCore::getParam($_POST, 'pack_dup', 0, 'i');
        $rows['pack_path'] = LCore::getParam($_POST, 'pack_path', '/images/backups/', 's');
        $rows['pack_pref'] = LCore::getParam($_POST, 'pack_pref', 'sef_', 's');
        $rows['pack_format'] = LCore::getParam($_POST, 'pack_format', 1, 'i');
    ...
    public static function pageDefault()
    {
        ...
        $sef_files = sizeof(glob(_LPATH_SEF . '/*sef.ini'));
