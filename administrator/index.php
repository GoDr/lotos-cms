...
require_once (_LPATH_ROOT . DS . 'includes/joostina.php');
...
if(isset($_POST['submit'])){
	...
	if($pass == null){
		mosRedirect(_LPATH_SITE . '/' . JADMIN_BASE . '/', _PLEASE_ENTER_PASSWORDWORD);
		exit();
	}
	...
	if($config->config_captcha OR ((int)$config->config_admin_bad_auth >= 0 && $config->config_admin_bad_auth <= $bad_auth_count)){
		...
		if($captcha_keystring != $captcha){
			mosRedirect(_LPATH_SITE . '/' . JADMIN_BASE . '/?' . $config->config_admin_secure_code, _BAD_CAPTCHA_STRING);
	...
	*/
	...
	if(isset($my->id)){
		...
		if(strcmp($hash, $cryptpass) || !$acl->acl_check('administration', 'login', 'users', $my->usertype)){
			...
			mosRedirect(_LPATH_SITE . '/' . JADMIN_BASE . '/index.php?' . $config->config_admin_secure_code, _BAD_USERNAME_OR_PASSWORDWORD);
			exit();
		}
		...
		if(LCore::getCfg('admin_expired') === '1'){
			...
			$checktime = (LCore::getCfg('session_life_admin') ? LCore::getCfg('session_life_admin') : 1800) / 2;
	...
	$path = _LPATH_TPL_ADMI . '/' . TEMPLATE . '/login.php';
	require_once ($path);
	doGzip();
}