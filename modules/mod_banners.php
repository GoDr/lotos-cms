...
if(!function_exists('showBanners')){
	...
	function showSingleBanner($text, $moduleclass_sfx, &$banner, $mainframe){
		...
		if($banner->custom_banner_code != ""){
			...
			$image_url = _LPATH_SITE . '/images/show/' . $banner->image_url;
			...
			$title = $banner->title;
			$result = "<a href=\"" . LSef::getUrlToSef('index.php?option=com_banners&amp;task=clk&amp;id=' . $banner->id) . "\" target=\"_" . $target . "\"><img src=\"" . $image_url . "\" style=\"border:" . $border_value . "px " . $border_style . " " . $border_color . "\" vspace=\"0\" alt=\"$alt\" title=\"$title\"/></a>";
			if($text){
				$result = "<a href=\"" . LSef::getUrlToSef('index.php?option=com_banners&amp;task=clk&amp;id=' . $banner->id) . "\" target=\"_" . $target . "\" class='bantxt" . $moduleclass_sfx . "'>" . $alt . "</a><br />" . $result;
			}
			$result = '<div class="banernblok' . $moduleclass_sfx . '">' . $result . '</div>';
		} elseif(preg_match("/.swf/", $banner->image_url)){
			$image_url = _LPATH_SITE . '/images/show/' . $banner->image_url;
			$swfinfo = @getimagesize(_LPATH_ROOT . '/images/banners/' . $banner->image_url);
			$result = "
