...
class mod_menu_easy_Helper
{
    ...
    function getParams($params, $module)
    {
        ...
        if (!LCore::getCfg('disable_access_control')) {
            $access = " AND access <= ?";
        }
     ...
     * @param $moduleid
     *
     * @modification 06.12.2013 Gold Dragon
     */
    public function getHTML($params, $moduleid)
    {
        ...
        foreach ($this->_links as $link) {
            ...
            // получаем ссылку
            $link_norm = LSef::getUrlToSef($link['link']);

            // Формируем стиль
            if ($this->_link_active == 1 AND LSef::$url == $link_norm) {
                $actived = ' id="mod_menu_easy_a' . $this->_module->id . '"';
            } elseif ($this->_link_active == 2 AND LSef::$url == $link_norm) {
            ...
            if ($this->_link_type) {
                ...
                if (LSef::$url == $link_norm and $this->_link_null) {
                    $result = '<span ' . $actived . '>' . $link['name'] . '</span>';
                } else {
                    switch ($link['browserNav']) {
                        case 1: // открыть в новом окне
                            $result = '<a href="' . $link_norm . '" ' . $actived . ' target="_blank">' . $link['name'] . '</a>';
                            break;
                        case 2: // открытие во всплывающем окне
                            $result = "<a href=\"#\" ".$actived." onclick=\"javascript: window.open('" . $link_norm . "', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550'); return false\" >" . $link['name'] . "</a>";
                            break;
                        default: // открытие в текущем окне
                            $result = '<a href="' . $link_norm . '" ' . $actived . ' target="_self">' . $link['name'] . '</a>';
                            break;
                    }
                }
            } else {
                ...
                if ($this->_image_template) {
                    ...
                    $path_root = _LPATH_TPL_FRONT . '/' . $_mainframe->getTemplate() . '/images/modules/mod_menu_easy';
                    $path_site = _LPATH_TPL_FRONT_S . '/' . $_mainframe->getTemplate() . '/images/modules/mod_menu_easy';
                } else {
                    $path_root = _LPATH_ROOT . '/images/menuimages';
                    $path_site = _LPATH_SITE . '/images/menuimages';
                }
