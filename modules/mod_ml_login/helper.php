...
class mod_ml_login_Helper{
	...
	function prepare_logout_form($params){
		...
		$params->_action = LSef::getUrlToSef('index.php?option=logout');
	...
	function prepare_login_form($params){
		...
		$login_input_def = '<input type="text" name="username" id="mod_login_USER" class="inputbox" value="" />';

		$pass_label_def = '<label for="mod_login_password" id="pass_lbl">' . $params->get('ml_pass_text') . '</label>';
		$pass_input_def = '<input type="password" id="mod_login_password" name="passwd" class="inputbox" value="" />';
		...
		switch($params->get('show_login_text', 1)){
			...
			case '2':
				$params->_input_login = '<input type="text" name="username" id="mod_login_USER" class="inputbox" value="' . $params->get('ml_login_text') . '" onblur="if(this.value==\'\') this.value=\'' . $params->get('ml_login_text') . '\';" onfocus="if(this.value==\'' . $params->get('ml_login_text') . '\') this.value=\'\';" />';
				break;
	...
	function get_return($params){
		...
		$return = str_replace(_LPATH_SITE . '/', '', $return);
		$return = ampReplace($return);

		return htmlentities(LSef::getUrlToSef($return));
	}
