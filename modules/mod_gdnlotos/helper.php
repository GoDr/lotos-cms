 ...
 * @description Модуль позволяет выводить основные материалы по определённым критериям для Lotos CMS
...
class mod_gdnlotos_Helper
{
    ...
    private function getParams($params, $moduleid)
    {
        ...
        $this->css_dir = trim($params->get('css_dir', ''));
        $this->modul_link = intval($params->get('modul_link', 0));
        $this->modul_link_cat = preg_replace("#[\d]+-#", '', trim($params->get('modul_link_cat', 0)));
        ...
        $this->image_quality_s = intval($params->get('image_quality_s', 3));
        if ($this->image_quality_s > 9) {
            $this->image_quality_s = 3;
        }
        $this->image_size_b = intval($params->get('image_size_b', 100));
        $this->image_quality_b = intval($params->get('image_quality_b', 3));
        if ($this->image_quality_b > 9) {
            $this->image_quality_b = 3;
        }
        ...
        if ($this->template_dir and is_dir(_LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/modules/mod_gdnlotos/' . $this->template)) {

            $this->tpl_dir = _LPATH_TPL_FRONT . '/' . TEMPLATE . '/html/modules/mod_gdnlotos/' . $this->template;
            $this->tpl_dir_style = _LPATH_TPL_FRONT_S . '/' . TEMPLATE . '/html/modules/mod_gdnlotos/' . $this->template . '/style.css';

        } elseif (is_dir(_LPATH_TPL_MOD . '/mod_gdnlotos/' . $this->template)) {

            $this->tpl_dir = _LPATH_TPL_MOD . '/mod_gdnlotos/' . $this->template;
            $this->tpl_dir_style = _LPATH_TPL_MOD_S . '/mod_gdnlotos/' . $this->template . '/style.css';

        } else {

            $this->tpl_dir = _LPATH_TPL_MOD . '/mod_gdnlotos/default';
            $this->tpl_dir_style = _LPATH_TPL_MOD_S . '/mod_gdnlotos/default/style.css';

        }

        if ($this->css_dir) {
            $mainframe = MainFrame::getInstance();
            $mainframe->addCSS('$this->tpl_dir_style', true);
        }
        // получаем имя каталога
        $_db = LCore::getDB();
        $this->dir_name = Text::gdQuoteReplace($_db->selectCell("SELECT `name` FROM `#__boss_config` WHERE `id` = ?", $this->directory));
    }
    ...
    public function getHTML($params, $moduleid)
    {
        ...
        MainFrame::addLib('images');

        // получение параметров модуля
        $this->getParams($params, $moduleid);

        $result = $this->style . '<div>Информация отсутствует</div>';
    ...
    {
        $this->getItem($row, 'sp');
        $tpl = file_get_contents($this->tpl_dir . '/special.tpl');
    ...
    {
        $this->getItem($row, 'bs');
        $tpl = file_get_contents($this->tpl_dir . '/basic.tpl');
    ...
    private function getReference($row)
    {
        ...
        $link = LSef::getUrlToSef('index.php?option=com_boss&amp;task=show_content&amp;directory=' . $this->directory . '&amp;catid=' . $row['category_id'] . '&amp;contentid=' . $row['id']);
        $tpl = file_get_contents($this->tpl_dir . '/reference.tpl');
    ...
    private function getAssembly()
    {
        ...
        if ($this->modul_link) {
            if ($this->modul_link == 1) {
                $link = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_all&amp;directory=" . $this->directory);
            } else {
                $link = LSef::getUrlToSef("index.php?option=com_boss&amp;task=show_category&amp;directory=" . $this->directory . "&amp;catid=" . $this->modul_link_cat);
            }
            $this->script .= '$("#module_' . $this->moduleid . ' > h3").wrap(\'<a href="' . $link . '" />\');';
        }
        if ($this->image_link == 2 OR $this->modul_link) {
            $this->script = '<script>$(function() {' . $this->script . '});</script>';
        }
        $replace = array($this->style, $this->moduleclass_sfx, $this->special, $basic, $this->reference, $this->script);
        $tpl = file_get_contents($this->tpl_dir . '/assembly.tpl');
     ...
     * @return void
     *
     * @modification 20.11.2013 Gold Dragon
     */
    private function getItem($row, $file)
    {
        ...
        if ($this->item_title) {
            ...
            $tpl = file_get_contents($this->tpl_dir . '/' . $file . '.tit.tpl');
        ...
        if ($this->directory_name) {
            ...
            if ($this->directory_link AND $this->directory) {
                $link_dir = LSef::getUrlToSef('index.php?option=com_boss&amp;task=show_all&amp;directory=' . $this->directory);
                $dir_name = '<a href="' . $link_dir . '" title="' . $dir_name . '">' . $dir_name . '</a>';
            }
            $tpl = file_get_contents($this->tpl_dir . '/' . $file . '.sec.tpl');
        ...
        if ($this->category_name) {
            ...
            if ($this->category_link AND $this->directory) {
                $link_cat = LSef::getUrlToSef('index.php?option=com_boss&amp;task=show_category&amp;directory=' . $this->directory . '&amp;catid=' . $row['category_id']);
                $row['cat_name'] = '<a href="' . $link_cat . '" title="' . $row['cat_name'] . '">' . $row['cat_name'] . '</a>';
            }
            $tpl = file_get_contents($this->tpl_dir . '/' . $file . '.cat.tpl');
        ...
        if ($this->show_date) {
            ...
            $tpl = file_get_contents($this->tpl_dir . '/' . $file . '.dat.tpl');
        ...
        /***************************************************************************/
        if ($this->image) {
            if ($row['type'] == 'BossImagePlugin') {
                $path = '/images/boss/' . $this->directory . '/contents/' . $row['id'] . 'a.';
                if (is_readable(_LPATH_ROOT . $path . 'jpg')) {
                    $text_with_image = '<img src="' . _LPATH_SITE . $path . 'jpg">';
                } elseif (is_readable(_LPATH_ROOT . $path . 'gif')) {
                    $text_with_image = '<img src="' . _LPATH_SITE . $path . 'gif">';
                } elseif (is_readable(_LPATH_ROOT . $path . 'png')) {
                    $text_with_image = '<img src="' . _LPATH_SITE . $path . 'png">';
                } else {
                    $text_with_image = '';
                }
            } else {
            ...
            // Проверяем на доступность изображения
            if (!$this->check_url($tmp_img)) {
                $tmp_img = '/images/noimage.jpg';
            }

            // Проверяем где находится картинка (внешняя или с сайта
            $parse_url = parse_url($tmp_img);
            ...
            if ((isset($parse_url['host']) and $parse_url['host'] == $parse_site['host']) or !isset($parse_url['host'])) {
                if (is_readable(_LPATH_ROOT . $parse_url['path'])) {
                    $tmp_img = _LPATH_SITE . $parse_url['path'];
            ...
            if ($info_img[0] and $info_img[1]) {
                // формирование размера
                if ($this->image_prev == 'width') {
                    $img_thumb_width = ($info_img[0] > $image_size) ? $image_size : $info_img[0];
                    $coeff = $info_img[0] / $info_img[1]; // если да, то делим ширину на высоту
                    $img_thumb_height = (int)($img_thumb_width / $coeff);
                } else {
                    $img_thumb_height = ($info_img[1] > $image_size) ? $image_size : $info_img[1];
                    $coeff = $info_img[1] / $info_img[0]; // и наоборот...
            ...
            $b = preg_match('#(\.jpeg|\.jpg|\.gif|\.png)$#i', $tmp_img);
            if (!$b) {
            ...
            // формируем миниэскиз
            $src = _LPATH_SITE . '/modules/mod_gdnlotos/imgsketch.php?' . 'src=' . $tmp_img . '&w=' . $img_thumb_width . '&h=' . $img_thumb_height . '&q=' . $image_quality;
            ...
            $tpl = file_get_contents($this->tpl_dir . '/' . $file . '.img.tpl');
        ...
        if ($this->show_author) {
            ...
            $tpl = file_get_contents($this->tpl_dir . '/' . $file . '.aut.tpl');
        ...
        if ($this->text) {
            ...
            $tpl = file_get_contents($this->tpl_dir . '/' . $file . '.txt.tpl');
        ...
        if ($this->readmore) {
            ...
            $tpl = file_get_contents($this->tpl_dir . '/' . $file . '.but.tpl');
        ...
        if ($this->hits) {
            ...
            $tpl = file_get_contents($this->tpl_dir . '/' . $file . '.hit.tpl');
    ...
    private function getContent()
    {
        ...
        // Значение для SQL-запроса
        $sql_value = array();

        if ($count) {
            // дополнительный фильтр по категориям
            if (trim($this->catid)) {
                $_tmp = preg_replace('#[,]+[\s]*[\d]+#', ' OR b.category_id = ?', $this->catid);
                $_tmp = preg_replace('#[\d]+#', '?', $_tmp);

                $whereCatid = " AND ( b.category_id = " . $_tmp . " )";
            } else {
                $whereCatid = '';
            }
            ...
            if ($this->time) {
                $time = "AND UNIX_TIMESTAMP(CURRENT_TIMESTAMP)-UNIX_TIMESTAMP(date_created) <= ?";
            } else {
                $time = '';
            }
            ...
            $sql
                = "SELECT a.id,
					...
					LEFT JOIN #__boss_" . $this->directory . "_fields AS f ON f.name = ?
                    WHERE  a.published = ?
                        " . $whereCatid . "
                        " . $time . "
				    ORDER BY " . $orderby . "
					LIMIT ?";
            $sql_value[] = $this->content_field;
            $sql_value[] = 1;
            if (sizeof(preg_split('#[,]+[\s]*#', $this->catid, -1, PREG_SPLIT_NO_EMPTY))) {
                $sql_value = array_merge($sql_value, preg_split('#[,]+[\s]*#', $this->catid, -1, PREG_SPLIT_NO_EMPTY));
            }
            if ($this->time) {
                $sql_value[] = $this->time * 24 * 60 * 60;
            }
            $sql_value[] = $count;
            $result = $_db->getCacheSql('select', $sql, $sql_value);
        }
        ...
        if (sizeof($result) and $result[0]['type'] == 'BossImageGalleryPlugin') {
            ...
            for ($i = 0; $i < sizeof($result); $i++) {
                $result[$i]['introtext'] = preg_replace($pattern, $replacement, $result[$i]['introtext']);
            }
        }
        ...
        if (sizeof($result) and $result[0]['type'] == 'BossYouTubePlugin') {
            ...
            $tmp = array();
            for ($i = 0; $i < sizeof($result); $i++) {
                $b = preg_match($pattern, $result[$i]['introtext'], $tmp);
                if ($b) {
                    $result[$i]['introtext'] = '<iframe src="//www.youtube.com/embed/' . $tmp[1] . '"></iframe>';
                } else {
                    $result[$i]['introtext'] = '';
                }
            }
     ...
     * @param        $html - текст
     * @param        $length - длина
     * @param string $hellip - текст который добавляется в конце
     *
     * @return string
     */
    ...
    private function getAuthor($row, $params)
    {
        if ($row['userid'] != '') {
            ...
            if ($params == '3' OR $params == '4') {
                ...
                $author_seflink = LSef::getUrlToSef($author_link);
