 ...
 * @description Четвёртая страница установки
 */
    ...
    if (!count($error)) {
       error_reporting(0);
        ...
        if (is_null($database->connect_error)) {
            ...
            if (is_readable(_LPATH_ROOT . '/installation/sql/lotos_cms.sql')) {
                $info['content'] .= '<p>&nbsp;&nbsp;&nbsp;&nbsp;&#149;&nbsp;Файл с дампом базы данных - <b><span style="color: #090">Загружен</span></b></p>';

                $sql_array = file(_LPATH_ROOT . '/installation/sql/lotos_cms.sql', FILE_SKIP_EMPTY_LINES|FILE_IGNORE_NEW_LINES);
                ...
                foreach($sql_array as $value){
                    ...
                    if(!$b){
                        ...
                        if(preg_match_all('#;\s*$#', $value, $tmp)){
                            ...
                            $b1 = preg_match("#CREATE TABLE.*?[`]+(.*?)[`+][\s]*[\(]+#ius", $sql_tmp, $a1);
                            $b2 = preg_match("#INSERT INTO.*?[`]+(.*?)[`+][\s]*[\(]+#ius", $sql_tmp, $a2);
                            if($b1){
                                $_mes = 'Создание таблицы <b>' . $a1[1] . '</b>';
                            }elseif($b2){
                                $_mes = 'Добавление данных в таблицу <b>' . $a2[1] . '</b>';
                            }else{
                                $_mes = 'Неизвестный SQL-запрос';
                            }
                            if($database->query($sql_tmp)){
                                $info['content'] .= '<p>&nbsp;&nbsp;&nbsp;&nbsp;&#149;&nbsp;' . $_mes . ' - <b><span style="color: #090">ОК</span></b></p>';
                            }else{
                                $error[] = 'Ошибка: ' . $_mes . '<br>'
            ...
            // Загрузка примеров
            if($DBSample){
                if (is_readable(_LPATH_ROOT . '/installation/sql/sample_data.sql')) {
                    $info['content'] .= '<p>&nbsp;&nbsp;&nbsp;&nbsp;&#149;&nbsp;Файл с демонстрационными данными - <b><span style="color: #090">Загружен</span></b></p>';

                    $sql_array = file(_LPATH_ROOT . '/installation/sql/sample_data.sql', FILE_SKIP_EMPTY_LINES|FILE_IGNORE_NEW_LINES);
