 ...
 * @see http://wiki.lotos-cms.ru/index.php/LLang
 *
 * @example
...
class LLang
{
    ...
    public static function getLang($expansion = null, $lang = null)
    {
        ...
        if (!is_null($expansion)) {
            ...
            if (is_null($lang) and !preg_match('#([a-z]{2})#', $lang)) {
                $lang = array_search(LCore::getCfg('lang'), self::$lang_codes);
            }

            // на всякий случай корректируем расширение если вдруг ошибка
            $type_a = array('lib_', 'sys_', 'front_', 'admin_', 'com_', 'mod_', 'plg_', 'tpl_');
            $type_b = array('lib.', 'sys.', 'front.', 'admin.', 'com.', 'mod.', 'plg.', 'tpl.');
            ...
            if (!isset(self::$language[$lang . '.' . $expansion])) {
                ...
                $pathFile = _LPATH_LANG . '/' . $lang . '/' . $expansion . '.lang.ini';
...
//            $ind = array($ind);
//        }
//        $fileNames = scandir(_LPATH_LANG . DS . self::$lang);
//
//        $expansion = array();
//
