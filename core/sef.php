 ...
 * @copyright  Авторские права (C) 2013-2015, Gold Dragon
 * @date       20.01.2015
 * @see        http://wiki.lotos-cms.ru/index.php/LSef
...
class LSef
{
    ...
    /** @var object библиотека LibIni */
    private static $_lib_ini;

    /** @var object библиотека LibUrl */
    private static $_lib_url;

    /** @var object библиотека LibText */
    ...
    public static function getInstance($cfg_sef = 0, $cfg_frontpage = 0)
    {
        ...
        self::$_lib_url = LibUrl::getInstance();
        self::$_lib_ini = LibIni::getInstance();
        self::$_lib_text = LibText::getInstance();

        // запоминаем адрес страницы
        if (!defined('IS_ADMIN')) {
            self::$url = self::getLinkNormalization(_LPATH_SITE . $_SERVER['REQUEST_URI']);
        }
        ...
        if ($cfg_sef and !defined('IS_ADMIN') and !substr_count($_SERVER['REQUEST_URI'], 'index2.php') and !isset($_REQUEST['tp'])) {
            ...
            if (file_exists(_LPATH_ROOT . DS . 'components' . DS . 'com_sef' . DS . 'sef.php')) {
                require_once(_LPATH_ROOT . DS . 'components' . DS . 'com_sef' . DS . 'sef.php');
            } else {
    ...
    private static function checkINI($option_ini)
    {
        ...
        if (in_array($option_ini, self::$sef_files)) {
            ...
            self::$_lib_ini->parse(_LPATH_SEF . DS . $option_ini);
     ...
     *
     * @param string $link  : нормальная ссылка
     * @param bool   $admin : принудительное преобразование ссылки
     *
     * @return string : sef-ссылка
     *
     * @modification 20.01.2015 Gold Dragon
     */
    public static function getUrlToSef($link, $admin = false)
    {
        // Нормализуем ссылку
        $link = self::getLinkNormalization($link);

        if ((self::$cfg_sef and !defined('IS_ADMIN') and strstr($link, 'index.php')) or $admin) {

            // Парсим URL
            self::$_lib_url->parse($link);

            // Проверяем ссылку на внешний ресурс
            $site_url = parse_url(_LPATH_SITE);
            ...
            if (self::$_lib_url->_path != 'ajax.index.php' and self::$_lib_url->_path != '/index2.php' and $host) {
                ...
                if (array_key_exists($link_hash, self::$_sef_url)) {
                    ...
                    $link_sef = $_db->selectCell('SELECT sef FROM `#__sef_link` WHERE url = ?', trim(str_replace(_LPATH_SITE, '', $link)));
                    ...
                    if (!$link_sef) {
                        ...
                        if (self::checkINI(self::getOption($link))) {
                            ...
                            if ($tmp_key !== false) {
                                if (isset($task_val_arr[$tmp_key]) and $task_val_arr[$tmp_key] != '') {
                                    ...
                                    $b = preg_match_all("#(\[\[([a-z0-9-_]*)\]\])#i", $task, $value_arr);
                                    if ($b) {
                                        // Получаем значения
                                        foreach ($value_arr[2] as $value) {
                                            $val[] = self::$_lib_url->getVar($value);
                                        }
                                        // производим замену
                                        $task = str_ireplace($value_arr[1], $val, $task);
                                    }
                            ...
                            // Добавляем "Якорь"
                            $fragment = (self::$_lib_url->_fragment) ? '#' . self::$_lib_url->_fragment : '';
                            ...
                            if ((LSef::$cfg_frontpage AND stripos($link, 'option=com_frontpage') > 0 AND !(stripos($link, 'limit'))) OR $link == 'index.php' OR $link == 'index.php?') {
                                ...
                                if (self::$_lib_url->_query) {
                                    ...
                                    foreach (self::$_lib_url->_vars as $key => $value) {
                                        $_query[] = $key . self::separator . $value;
                                    }
                        ...
                        // Транслитерация
                        $link_sef = (isset($link_sef)) ? self::$_lib_text->text_to_url($link_sef) : '';

                        // Удаляем возможные повторы слеша
                        $link_sef = preg_replace('#-/|/-#', '/', $link_sef);
                        $link_sef = preg_replace('#/\.#i', '.', $link_sef);
                        $link_sef = preg_replace('#[/-]{2,}#', '/', $link_sef);
     ...
     * @return string
     *
     * @modification 07.08.2013 Gold Dragon
     */
    private static function getLinkNormalization($link)
    {
        ...
        self::$_lib_url->parse(_LPATH_SITE);
        ...
        if (is_null(self::$_lib_url->_host) or $_host_site == self::$_lib_url->_host) {
            ...
            if (LCore::getCfg('com_frontpage_clear')
                and ($link == _LPATH_SITE . '/index.php?option=com_frontpage'
    ...
    private static function saveSef($link, $link_sef)
    {
        ...
        $link = trim(str_replace(_LPATH_SITE, '', $link));
        $link_sef = trim(str_replace(_LPATH_SITE, '', $link_sef));
