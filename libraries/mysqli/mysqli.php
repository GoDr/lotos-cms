...
/**
 * LibMySQLi - Класс для управления базой данных
 *
 * @package     LibMySQLi
 * @version     1.3
...
class LibMySQLi
{
    ...
    /** @var int - флаг кеширования */
    private $_db_caching;

    /** @var string - префикс для таблиц */
    private $_db_prefix;

    /** @var string - путь до файлов кеширования */
    ...
    private function __construct()
    {
        ...
        $this->_db_host = LCore::getCfg('host');

        $this->_db_user = LCore::getCfg('user');

        $this->_db_password = LCore::getCfg('password');

        $this->_db_name = LCore::getCfg('db');

        $this->_db_port = LCore::getCfg('port');

        $this->_db_socket = LCore::getCfg('socket');

        $this->_db_debug = LCore::getCfg('debug');

        $this->_db_caching = LCore::getCfg('caching');

        $this->_db_prefix = LCore::getCfg('dbprefix');

        $this->_db_cache_dir = LCore::getCfg('cachepath');

        $this->_db_cache_time = LCore::getCfg('cachetime');
    ...
    /**
     * Получить результат кеширования SQL-запроса
     *
     * @return mixed - данные
     * @throws Exception
     *
    ...
    public function getCacheSql()
    {
        ...
        if (method_exists(__CLASS__, $method)) {
            if (is_readable($path) and $this->_db_caching) {
                ...
                // Если файла кеша не существует, делаем запрос в БД
                $rows = $this->$method($sql, $params);
                file_put_contents($path, serialize($rows), LOCK_EX);
                return $rows;
            }
     ...
     *
     * @return array - ассоциациативный массив
     *              [server_info]    - версия сервера MySQL в виде строки
     *              [server_version] - версия сервера MySQL, представленую в виде integer
     *              [client_info]    - информация о клиенте MySQL в виде строки
     *              [client_version] - информация о клиенте MySQL, представленую в виде integer
     * @@example:
     *      Array
     *      (
     *          [server_info] => 5.5.37-log
     *          [server_version] => 50537
     *          [client_info] => mysqlnd 5.0.8-dev - 20102224 - $Id: 731e5b87ba42146a687c29995d2dfd8b4e40b325 $
    ...
    {
        $result = array();
        $result['server_info'] = $this->db_resource->server_info;
        $result['server_version'] = $this->db_resource->server_version;
        $result['client_info'] = $this->db_resource->client_info;
        $result['client_version'] = $this->db_resource->client_version;
        return $result;
    }
